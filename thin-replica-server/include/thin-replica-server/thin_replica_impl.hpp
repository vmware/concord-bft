...
class ThinReplicaImpl {
 ...
 private:
  ...
  void syncAndSendEventGroups(ServerContextT* context,
                              ...
                              ServerWriterT* stream,
                              std::shared_ptr<kvbc::KvbAppFilter>& kvb_filter) {
    uint64_t current_oldest_start = kvb_filter->firstTagSpecificPublicEventGroupId(getClientId(context));
    ...
    auto end = kvb_filter->lastTagSpecificPublicEventGroupId(getClientId(context));
    if (!end) {
      std::stringstream msg;
  ...
  std::tuple<grpc::Status, std::shared_ptr<SubUpdateBuffer>> subscribeToLiveUpdates(
    ...
    if (request->has_events()) {
      ...
      auto last_eg_id = kvb_filter->lastTagSpecificPublicEventGroupId(client_id);
      LOG_INFO(
          logger_,
