...
TEST(InstallReadyMsg, base_methods) {
  ...
  std::unique_ptr<SigManager> sigManager(createSigManager(config.replicaId,
                                                          ...
                                                          replicaInfo));
  std::unique_ptr<InstallReadyMsg> msg(
      InstallReadyMsg::create(senderId, seqNum, version, concordUtils::SpanContext{spanContext}));
  EXPECT_EQ(msg->idOfGeneratedReplica(), senderId);
  EXPECT_EQ(msg->seqNum(), seqNum);
  EXPECT_EQ(msg->getVersion(), version);

  testMessageBaseMethods(*msg.get(), MsgCode::InstallReady, senderId, spanContext);
