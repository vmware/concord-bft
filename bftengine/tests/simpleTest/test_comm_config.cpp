...
std::unordered_map<NodeNum, NodeInfo> TestCommConfig::SetUpConfiguredNodes(
  ...
  if (!config_file_parser.Parse()) {
    LOG_FATAL(logger_,
              "Failed to parse configuration file: " << config_file_name);
    exit(-1);
  }
  num_of_clients =
      static_cast<uint16_t>(config_file_parser.Count(CLIENTS_CONFIG));
  num_of_replicas =
      static_cast<uint16_t>(config_file_parser.Count(REPLICAS_CONFIG));
...
std::unordered_map<NodeNum, NodeInfo> TestCommConfig::SetUpDefaultNodes(
  ...
  for (int i = 0; i < (num_of_replicas + num_of_clients); i++)
    nodes.insert({i,
                  NodeInfo{ip,
                           static_cast<uint16_t>(base_port_ + i * 2),
                           i < num_of_replicas}});
  return nodes;
}
...
std::unordered_map<NodeNum, NodeInfo> TestCommConfig::SetUpNodes(
  ...
  if (config_file_name.empty())
    return SetUpDefaultNodes(
        node_id, ip, port, num_of_clients, num_of_replicas);
  else
    return SetUpConfiguredNodes(is_replica,
                                config_file_name,
                                node_id,
                                ip,
                                port,
...
PlainUdpConfig TestCommConfig::GetUDPConfig(
  ...
  string ip;
  uint16_t port;
  std::unordered_map<NodeNum, NodeInfo> nodes = SetUpNodes(is_replica,
                                                           node_id,
                                                           ip,
                                                           port,
                                                           num_of_clients,
...
PlainTcpConfig TestCommConfig::GetTCPConfig(
  ...
  PlainTcpConfig ret_val(
      ip, port, buf_length_, nodes, num_of_replicas - 1, node_id);
  return ret_val;
}
