 ...
 *  @brief Contains helper functions for the InMemoryDBClient and InMemoryDBClientIterator classes.
 *
 ...
 *  The map does not contain references to the keys and values. They are always copied into the store.
 *  Functions are included for creating, using, and destroying iterators to navigate through the map.
 *
 */
 ...
 *  @param _outValue Reference to where the value gets stored if the lookup is successful.
 ...
 *  @param _iter Pointer to object of class IDBClientIterator that needs to be freed.
...
/** @brief Services a write request to the In Memory database by adding a key value pair to the map.
 *  If the map already contains the key, it replaces the value with the data referred to by _value.
 ...
 *  @return Moves the iterator to the start of the map and returns the first key value pair of the map.
 */
...
/** @brief Returns the key value pair of the key which is greater than or equal to _searchKey.
 *  Returns the first key value pair whose key is not considered to go before _searchKey. Also,
 *  moves the iterator to this position.
 *  @param _searchKey Key to search for.
 *  @return Key value pair of the key which is greater than or equal to _searchKey.
 */
