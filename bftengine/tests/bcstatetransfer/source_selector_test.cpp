...
TEST(source_selector_test, has_no_source_on_construction) {
  ASSERT_FALSE(SourceSelector(replicas, kRetransmissionTimeoutMs, kReplicaReplacementTimeoutMs).hasSource());
}
...
TEST(source_selector_test, zero_preferred_replica_on_construction) {
  ASSERT_EQ(
      SourceSelector(replicas, kRetransmissionTimeoutMs, kReplicaReplacementTimeoutMs).numberOfPreferredReplicas(), 0);
}

TEST(source_selector_test, no_replica_is_preferred_on_construction) {
  const auto source_selector = SourceSelector(replicas, kRetransmissionTimeoutMs, kReplicaReplacementTimeoutMs);
...
TEST(source_selector_test, leave_initial_state_when_the_fetching_timestamp_is_not_zero) {
  auto source_selector = SourceSelector(replicas, kRetransmissionTimeoutMs, kReplicaReplacementTimeoutMs);
  ASSERT_TRUE(source_selector.isReset());
  source_selector.setFetchingTimeStamp(GL, kSampleCurrentTimeMs);
  ASSERT_FALSE(source_selector.isReset());
}
