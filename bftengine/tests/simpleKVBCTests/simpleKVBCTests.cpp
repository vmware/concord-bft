...
struct SimpleBlock {
  ...
  // block->items[i].key[k]); 		printf("   val=");
...
struct SimpleConditionalWriteHeader {
  ...
  static SimpleConditionalWriteHeader* alloc(size_t numOfKeysInReadSet,
                                             size_t numOfWrites) {
  ...
  static size_t size(size_t numOfKeysInReadSet, size_t numOfWrites) {
    return sizeof(SimpleConditionalWriteHeader) +
           numOfKeysInReadSet * sizeof(SimpleKey) +
           numOfWrites * sizeof(SimpleKV);
  }

  size_t size() { return size(numberOfKeysInReadSet, numberOfWrites); }
  ...
  SimpleKV* keyValArray() {
    return (SimpleKV*)(((char*)this) + sizeof(SimpleConditionalWriteHeader) +
                       numberOfKeysInReadSet * sizeof(SimpleKey));
  }
};
...
struct SimpleReadHeader {
  ...
  static SimpleReadHeader* alloc(size_t numOfKeysToRead) {
    size_t size =
        sizeof(SimpleReadHeader) + (sizeof(SimpleKey) * (numOfKeysToRead - 1));
    char* pBuf = new char[size];
    memset(pBuf, 0, size);
  ...
  size_t size() { return size(numberOfKeysToRead); }

  SimpleKey* keysArray() { return ((SimpleKey*)((char*)keys)); }
};
...
struct SimpleGetLastBlockHeader {
  ...
  static size_t size() { return sizeof(SimpleGetLastBlockHeader); }
...
struct SimpleReplyHeader_Read {
  ...
  static size_t size(size_t numOfElements) {
    size_t size = sizeof(SimpleReplyHeader_Read) +
                  (sizeof(SimpleKV) * (numOfElements - 1));
    return size;
  }

  size_t size() { return size(numberOfElements); }
...
class InternalTestsBuilder {
 private:
  ...
  void createAndInsertRandomConditionalWrite() {
    ...
    if (m_lastBlockId > CONFLICT_DISTANCE)
      readVer -= (rand() % CONFLICT_DISTANCE);

    size_t numberOfWrites = (rand() % (MAX_WRITES_IN_REQ - 1)) + 1;
    size_t numberOfKeysInReadSet = (rand() % MAX_READ_SET_SIZE_IN_REQ);

    SimpleConditionalWriteHeader* pHeader = SimpleConditionalWriteHeader::alloc(
        numberOfKeysInReadSet, numberOfWrites);
    ...
    for (BlockId i = readVer + 1; (i <= m_lastBlockId) && !foundConflict; i++) {
      ...
      for (size_t a = 0; (a < numberOfKeysInReadSet) && !foundConflict; a++)
        for (size_t b = 0; (b < currBlock->numberOfItems) && !foundConflict;
             b++) {
          if (memcmp(pReadKeysArray[a].key, currBlock->items[b].key, KV_LEN) ==
              0)
            foundConflict = true;
        }
    }
    ...
    SimpleReplyHeader_ConditionalWrite* pReply =
        SimpleReplyHeader_ConditionalWrite::alloc();
    pReply->h.type = 1;
    if (foundConflict) {
      pReply->succ = false;
  ...
  void createAndInsertRandomRead() {
    ...
    SimpleReplyHeader_Read* pReply =
        SimpleReplyHeader_Read::alloc(numberOfReads);
    pReply->h.type = 2;
    ...
    for (size_t i = 0; i < numberOfReads; i++) {
      ...
      std::map<SimpleKIDPair, SimpleVal>::const_iterator p =
          m_map.lower_bound(kiPair);

      if (p != m_map.end() && (pHeader->readVerion >= p->first.blockId) &&
          (memcmp(p->first.key.key, pHeader->keys[i].key, KV_LEN) == 0)) {
  ...
  void createAndInsertGetLastBlock() {
    ...
    // compute expected reply
    SimpleReplyHeader_GetLastBlockHeader* pReply =
        SimpleReplyHeader_GetLastBlockHeader::alloc();
    pReply->h.type = 3;
    pReply->latestBlock = m_lastBlockId;
...
class InternalCommandsHandler : public ICommandsHandler {
 public:
  virtual bool executeCommand(const Slice command,
    ...
    SimpleConditionalWriteHeader* pCondWrite =
        (SimpleConditionalWriteHeader*)command.data;
    ...
    SimpleReplyHeader_ConditionalWrite* pReply =
        (SimpleReplyHeader_ConditionalWrite*)outReply;
  ...
  virtual bool executeReadOnlyCommand(
    ...
    if (p->type == 2) {
      ...
      SimpleReplyHeader_GetLastBlockHeader* pReply =
          (SimpleReplyHeader_GetLastBlockHeader*)(outReply);
...
static void verifyEmptyBlockchain(IClient* client) {
  ...
  SimpleReplyHeader_GetLastBlockHeader* pReplyData =
      (SimpleReplyHeader_GetLastBlockHeader*)reply.data;
  (void)pReplyData;
