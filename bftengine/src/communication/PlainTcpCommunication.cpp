...
// terms and conditions of the subcomponent's license, anoted in the LICENSE
// file.
...
class AsyncTcpConnection
 ...
 private:
  ...
  void close_socket() {
    LOG_TRACE(_logger,
              "enter, node " << _selfId << ", dest: " << _destId
                             << ", connected: " << connected
                             << ", closed: " << _closed);
    ...
    try {
      ...
      LOG_ERROR(_logger,
                "exception, node " << _selfId << ", dest: " << _destId
                                   << ", connected: " << connected
                                   << ", ex: " << e.what());
    }

    LOG_TRACE(_logger,
              "exit, node " << _selfId << ", dest: " << _destId
                            << ", connected: " << connected
                            << ", closed: " << _closed);
  }
  ...
  bool is_service_message() {
    ...
    switch (msgType) {
      case MessageType::Hello:
        ...
        LOG_DEBUG(_logger,
                  "node: " << _selfId << " got hello from:" << _destId);

        _fOnHellOMessage(_destId, shared_from_this());
        _destIsReplica = check_replica(_destId);
        LOG_DEBUG(
            _logger,
            "node: " << _selfId << " dest is replica: " << _destIsReplica);
        return true;
      default:
        return false;
    }
  ...
  void send_hello() {
    ...
    LOG_DEBUG(
        _logger,
        "sending hello from:" << _selfId << " to: " << _destId
  ...
  void connect_timer_tick(const B_ERROR_CODE &ec) {
    ...
    if (_closed) {
      LOG_DEBUG(_logger,
                "closed, node " << _selfId << ", dest: " << _destId
                                << ", ec: " << ec.message());
    } else {
      if (connected) {
        LOG_DEBUG(_logger,
                  "already connected, node " << _selfId << ", dest: " << _destId
                                             << ", ec: " << ec);
        _connectTimer.expires_at(boost::posix_time::pos_infin);
      } else if (_connectTimer.expires_at() <=
                 deadline_timer::traits_type::now()) {
        LOG_DEBUG(_logger,
                  "reconnecting, node " << _selfId << ", dest: " << _destId
                                        << ", ec: " << ec);
        reconnect();
      } else {
        LOG_DEBUG(_logger,
                  "else, node " << _selfId << ", dest: " << _destId
                                << ", ec: " << ec.message());
      }
  ...
  void connect_completed(const B_ERROR_CODE &err) {
    ...
    if (!socket.is_open()) {
      ...
      LOG_DEBUG(_logger,
                "connected, node " << _selfId << ", dest: " << _destId
                                   << ", res: " << res);
      connected = true;
      _wasError = false;
      _connecting = false;
 ...
 public:
  void connect(string ip, uint16_t port, bool destIsReplica) {
    ...
    LOG_TRACE(_logger,
              "enter, from: " << _selfId << " ,to: " << _destId
                              << ", ip: " << ip << ", port: " << port);

    tcp::endpoint ep(address::from_string(ip), port);
    LOG_DEBUG(_logger,
              "connecting from: " << _selfId << " ,to: " << _destId
    ...
    socket.async_connect(ep,
                         boost::bind(&AsyncTcpConnection::connect_completed,
                                     ...
                                     boost::asio::placeholders::error));
    LOG_TRACE(_logger,
              "exit, from: " << _selfId << " ,to: " << _destId << ", ip: " << ip
                             << ", port: " << port);
  }
  ...
  static ASYNC_CONN_PTR create(io_service *service,
                               ...
                               UPDATE_CONNECTIVITY_FN statusCallback,
                               NodeMap nodes) {
    auto res = ASYNC_CONN_PTR(new AsyncTcpConnection(service,
                                                     onError,
                                                     onHello,
                                                     bufferLength,
                                                     destId,
...
class PlainTCPCommunication::PlainTcpImpl {
 private:
  ...
  concordlogger::Logger _logger =
      concordlogger::Logger::getLogger("concord-bft.tcp");
  ...
  void start_accept(const NodeMap &nodes) {
    ...
    auto conn = AsyncTcpConnection::create(
        ...
        std::bind(&PlainTcpImpl::on_async_connection_error,
                  this,
                  std::placeholders::_1),
        std::bind(&PlainTcpImpl::on_hello_message,
                  this,
                  std::placeholders::_1,
                  std::placeholders::_2),
        _bufferLength,
    ...
    _pAcceptor->async_accept(conn->socket,
                             boost::bind(&PlainTcpImpl::on_accept,
                                         this,
                                         conn,
                                         nodes,
                                         boost::asio::placeholders::error));
 ...
 public:
  ...
  virtual ~PlainTcpImpl() { LOG_TRACE(_logger, "PlainTCPDtor"); }
};
...
ConnectionStatus PlainTCPCommunication::getCurrentConnectionStatus(
...
void PlainTCPCommunication::setReceiver(NodeNum receiverNum,
                                        IReceiver *receiver) {
