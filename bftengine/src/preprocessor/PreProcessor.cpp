...
PreProcessor::PreProcessor(shared_ptr<MsgsCommunicator> &msgsCommunicator,
    ...
    : msgsCommunicator_(msgsCommunicator),
      ...
      preProcessorMetrics_{metricsComponent_.RegisterAtomicCounter("preProcReqReceived"),
                           ...
                           metricsComponent_.RegisterGauge("preProcessingTimeAvg", 0),
...
void PreProcessor::onMessage<ClientBatchRequestMsg>(ClientBatchRequestMsg *msg) {
  ...
  const auto &cid = clientBatch->getCid();
  LOG_DEBUG(logger(),
...
void PreProcessor::onMessage<PreProcessRequestMsg>(PreProcessRequestMsg *msg) {
  ...
  LOG_DEBUG(logger(),
            "Received PreProcessRequestMsg" << KVLOG(reqType, reqSeqNum, senderId, clientId, reqOffsetInBatch));
  ...
  bool registerSucceeded = false;
  {
    ...
    if (reqEntry->reqProcessingStatePtr) {
      ...
      // Release registered client request
...
void AsyncPreProcessJob::release() {
  ...
  preProcessor_.totalPreProcessingTime_.add(durationInNano * 1000000);
