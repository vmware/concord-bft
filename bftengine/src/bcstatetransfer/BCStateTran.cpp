...
void BCStateTran::init(uint64_t maxNumOfRequiredStoredCheckpoints,
  ...
  try {
    ...
    if (psd_->initialized()) {
      ...
      if ((fs == FetchingState::GettingMissingBlocks) && (commitState_.minBlockId > 0)) {
        ConcordAssertEQ(commitState_, fetchState_);
        postProcessingDT_.start();
        blocksPostProcessed_.start();
        postProcessingQ_->push(std::bind(&BCStateTran::postProcessNextBatch, this, commitState_.minBlockId));
      }
    } else {
