    ...
    if (newBlockIsValid) {
      ...
      if (isGettingBlocks) {
        ...
        if (!lastFetchedBlockIdInCycle) {
          ...
          // until all blocks are taken our from ST temporart blockchain and moved to consensus blockchain.
          ...
          //
          // TODO - at phase 2 - putBlock with lastblock == false and send job to post-processing thread. Then continue
          // into next cycle, if there is such (to improve performance)
