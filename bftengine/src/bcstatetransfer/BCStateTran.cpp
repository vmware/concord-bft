...
namespace SimpleBlockchainStateTransfer {
  ...
  IStateTransfer* create(const Config& config, IAppState* const stateApi,
    ...
    impl::BCStateTran* p
      = new impl::BCStateTran(persistentDataStore,
                  config,
                  stateApi);
...
static uint16_t calcMaxNumOfChunksInBlock(uint32_t maxItemSize,
  ...
  if (!isVBlock) {
    uint16_t retVal =
      (maxBlockSize % maxChunkSize == 0) ?
      (maxBlockSize / maxChunkSize) : (maxBlockSize / maxChunkSize + 1);

    return retVal;
  } else {
    uint16_t retVal =
      (maxItemSize % maxChunkSize == 0) ?
...
BCStateTran::BCStateTran(
  ...
  maxNumOfChunksInAppBlock_{ calcMaxNumOfChunksInBlock(maxItemSize_,
                                                       ...
                                                       config.maxChunkSize,
                                                       false) },
  maxNumOfChunksInVBlock_{ calcMaxNumOfChunksInBlock(maxItemSize_,
                                                     config.maxBlockSize ,
                                                     config.maxChunkSize,
                                                     true) },
...
STDigest BCStateTran::checkpointReservedPages(uint64_t checkpointNumber) {
  ...
  LOG_INFO(STLogger, "associating " << pages.size() <<
      " pending pages with checkpoint " <<
      checkpointNumber);
...
void BCStateTran::getDigestOfCheckpoint(uint64_t checkpointNumber,
  ...
  c.writeDigest(reinterpret_cast<char*>(&retVal));
    ...
    LOG_INFO(STLogger, "BCStateTran::getDigestOfCheckpoint - digest="
    << retVal.toString());
...
void BCStateTran::handleStateTransferMessage(char* msg, uint32_t msgLen,
  ...
  case MsgType::CheckpointsSummary:
    if (fs == FetchingState::GettingCheckpointSummaries)
      noDelete = onMessage(reinterpret_cast<CheckpointSummaryMsg*>(msg),
                           msgLen, senderId);
    break;
  case MsgType::FetchBlocks:
    noDelete = onMessage(reinterpret_cast<FetchBlocksMsg*>(msg),
                         msgLen, senderId);
    break;
  case MsgType::FetchResPages:
    noDelete = onMessage(reinterpret_cast<FetchResPagesMsg*>(msg),
                         msgLen, senderId);
    break;
  case MsgType::RejectFetching:
    if (fs == FetchingState::GettingMissingBlocks ||
        fs == FetchingState::GettingMissingResPages)
      noDelete = onMessage(reinterpret_cast<RejectFetchingMsg*>(msg),
                           msgLen, senderId);
    break;
  case MsgType::ItemData:
    if (fs == FetchingState::GettingMissingBlocks ||
        fs == FetchingState::GettingMissingResPages)
      noDelete = onMessage(reinterpret_cast<ItemDataMsg*>(msg),
                           msgLen, senderId);
    break;
  default:
    break;
...
bool BCStateTran::checkValidityAndSaveMsgSeqNum(uint16_t replicaId,
  ...
  uint64_t diffMilli =
    ((milliMsgTime > milliNow) ?
      (milliMsgTime - milliNow) : (milliNow - milliMsgTime));
  ...
  lastMsgSeqNumOfReplicas_[replicaId] = msgSeqNum;
    ...
    LOG_INFO(STLogger, "BCStateTran::checkValidityAndSaveMsgSeqNum -"
...
bool BCStateTran::onMessage(const AskForCheckpointSummariesMsg* m,
  ...
  uint64_t fromCheckpoint = std::max(m->minRelevantCheckpointNum,
                                     psd_->getFirstStoredCheckpoint());
  // TODO(GG): really need this condition?
  if (toCheckpoint > maxNumOfStoredCheckpoints_) {
    fromCheckpoint = std::max(fromCheckpoint,
                              toCheckpoint - maxNumOfStoredCheckpoints_ + 1);
  }
...
bool BCStateTran::onMessage(const CheckpointSummaryMsg* m,
  ...
  if (p == summariesCerts.end()) {
    cert = new CheckpointSummaryMsgCert(replicaForStateTransfer_,
                                        replicas_.size(), fVal_,
                                        fVal_ + 1, myId_);
...
bool BCStateTran::onMessage(const FetchResPagesMsg* m,
  ...
  if (vblock == nullptr) {
        LOG_INFO(STLogger, "Creating a new vblock: checkpointNum="
...
/*
      LOG_INFO(STLogger,  endl << "  m->requestMsgSeqNum=" << m->requestMsgSeqNum <<
      ...
      LOG_INFO(STLogger,  "COND (currentSourceReplica_ != replicaId)=" << (currentSourceReplica_ != replicaId));
      LOG_INFO(STLogger,  "COND (m->requestMsgSeqNum != lastMsgSeqNum_)=" << (m->requestMsgSeqNum != lastMsgSeqNum_));
      LOG_INFO(STLogger,  "COND (m->blockNumber > lastRequiredBlock)=" << (m->blockNumber > lastRequiredBlock));
      LOG_INFO(STLogger,  "COND (m->blockNumber < firstRequiredBlock)=" << (m->blockNumber < firstRequiredBlock));
      LOG_INFO(STLogger,  "COND (m->blockNumber + MAX_NUMBER_OF_CHUNKS_IN_SINGLE_BATCH + 1 < lastRequiredBlock)=" << (m->blockNumber + MAX_NUMBER_OF_CHUNKS_IN_SINGLE_BATCH + 1 < lastRequiredBlock));
      LOG_INFO(STLogger,  "COND (m->dataSize + totalSizeOfPendingItemDataMsgs > maxPendingDataFromSourceReplica_)=" << (m->dataSize + totalSizeOfPendingItemDataMsgs > maxPendingDataFromSourceReplica_));
*/
...
void BCStateTran::clearPendingItemsData(uint64_t untilBlock) {
    LOG_INFO(STLogger, "BCStateTran::clearPendingItemsData - untilBlock="
    << untilBlock);
...
bool BCStateTran::getNextFullBlock(
  ...
  while ((it != pendingItemDataMsgs.end()) &&
    ...
    if (totalNumberOfChunks != msg->totalNumberOfChunksInBlock ||
      msg->chunkNumber > totalNumberOfChunks ||
      blockSize > maxSize) {
      badData = true;
      break;
    }
...
void BCStateTran::EnterGettingCheckpointSummariesState() {
  Assert(preferredReplicas_.empty());
  LOG_INFO(STLogger, "Go to state  GettingCheckpointSummaries"
      " (because preferredReplicas_.size()==0)");
...
void BCStateTran::processData() {
  ...
  while (true) {
    ...
    const uint64_t diffMilli =
      ((currentSourceReplica_ == NO_REPLICA) ||
       (currTime < timeMilliCurrentSourceReplica_))
    ...
    const bool newBlock = getNextFullBlock(
        nextRequiredBlock_, badDataFromCurrentSourceReplica,
        lastChunkInRequiredBlock, buffer_, actualBlockSize,
        !isGettingBlocks);
    ...
    if (newBlock && isGettingBlocks) {
      ...
      newBlockIsValid =
        checkBlock(nextRequiredBlock_, digestOfNextRequiredBlock,
      ...
      << " newBlockIsValid=" << newBlockIsValid);
    ...
    if (newBlockIsValid && isGettingBlocks) {
      ...
      Assert(lastChunkInRequiredBlock >= 1 && actualBlockSize > 0);
            ...
            LOG_INFO(STLogger, "add block " << nextRequiredBlock_
    ...
    //////////////////////////////////////////////////////////////////////////
      ...
      // Completion
            ...
            LOG_INFO(STLogger, "Calling onTransferringComplete for checkpoint "
        << cp.checkpointNum);
    ...
    //////////////////////////////////////////////////////////////////////////
      ...
      if (newSourceReplica ||
        ...
        sendFetchBlocksMsg(
          ...
          nextRequiredBlock_, lastChunkInRequiredBlock);
      }
...
bool BCStateTran::checkConsistency(bool checkAllBlocks) {
  ...
  if (checkAllBlocks && lastReachableBlockNum > 0) {
    for (uint64_t currBlock = lastReachableBlockNum - 1;
         currBlock >= 1;
         currBlock--) {
      STDigest currDigest;
...
void BCStateTran::computeDigestOfBlock(
  const uint64_t blockNum, const char* block, const uint32_t blockSize,
  STDigest* outDigest) {
  /*
