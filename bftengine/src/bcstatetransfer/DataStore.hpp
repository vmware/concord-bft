...
class DataStore {
 public:
  ...
  virtual set<uint16_t> getReplicas() const = 0;

  virtual void setMyReplicaId(uint16_t id) = 0;
  virtual uint16_t getMyReplicaId() const = 0;

  virtual void setFVal(uint16_t fVal) = 0;
  virtual uint16_t getFVal() const = 0;
  ...
  virtual uint32_t getNumberOfReservedPages() const = 0;
  ...
  virtual uint64_t getLastStoredCheckpoint() const = 0;

  virtual void setFirstStoredCheckpoint(uint64_t c) = 0;
  virtual uint64_t getFirstStoredCheckpoint() const = 0;
  ...
  virtual void getPendingResPage(
                    uint32_t inPageId, char* outPage, uint32_t pageLen) const = 0;
  virtual uint32_t numOfAllPendingResPage() = 0;
  virtual set<uint32_t> getNumbersOfPendingResPages() const = 0;
  ...
  virtual void getResPage(uint32_t inPageId,
                   uint64_t inCheckpoint, uint64_t* outActualCheckpoint) const = 0;
  virtual void getResPage(uint32_t inPageId, uint64_t inCheckpoint,
      uint64_t* outActualCheckpoint, char* outPage, uint32_t copylength) const = 0;
  virtual void getResPage(uint32_t inPageId, uint64_t inCheckpoint,
       uint64_t* outActualCheckpoint, STDigest* outPageDigest, char* outPage,
       uint32_t copylength) const = 0;
  ...
  virtual ResPagesDescriptor* getResPagesDescriptor(uint64_t inCheckpoint) const = 0;
  ...
  virtual bool getIsFetchingState() const = 0;

  virtual void setCheckpointBeingFetched(const CheckpointDesc& c) = 0;
  virtual CheckpointDesc getCheckpointBeingFetched() const = 0;
  ...
  virtual uint64_t getFirstRequiredBlock() const = 0;

  virtual void setLastRequiredBlock(uint64_t i) = 0;
  virtual uint64_t getLastRequiredBlock() const = 0;
