...
void DBDataStore::serializePrunedBlocksDigests(std::ostream& os,
                                               std::vector<std::pair<BlockId, STDigest>>& digests) const {
  Serializable::serialize(os, digests.size());
  for (size_t i{0}; i < digests.size(); ++i) {
    Serializable::serialize(os, digests[i].first);
    Serializable::serialize(os, digests[i].second.getForUpdate(), BLOCK_DIGEST_SIZE);
  }
}
void DBDataStore::deserializePrunedBlocksDigests(std::istream& is,
  ...
  for (size_t i{0}; i < size; ++i) {
    ...
    digests.emplace_back(std::make_pair(blockId, digest));
  }
}
void DBDataStore::setPrunedBlocksDigests(std::vector<std::pair<BlockId, STDigest>>& digests) {
  std::ostringstream oss;
