...
// TODO(GG): we won't need "MaxNumberOfReplicas" as soon as we start working on
// reconfiguration
...
static_assert(kWorkWindowSize == 2 * checkpointWindowSize,
              "kWorkWindowSize != 2 * checkpointWindowSize");

constexpr uint16_t maxLegalConcurrentAgreementsByPrimary =
    16;  // TODO(GG): check the value in config
         // (maxConcurrentAgreementsByPrimary should be <=
...
static_assert(kWorkWindowSize > MaxConcurrentFastPaths + checkpointWindowSize,
              "Violation of kWorkWindowSize > MaxConcurrentFastPaths + "
              "checkpointWindowSize");
static_assert(
    maxLegalConcurrentAgreementsByPrimary < MaxConcurrentFastPaths,
...
constexpr uint32_t maxExternalMessageSize =
    64 * 1000;  // TODO(GG): some message types may need different values
...
constexpr int minTimeBetweenStatusRequestsMilli =
    70;  // TODO(GG): config/dynamicVal ?

constexpr double factorForMinTimeBetweenStatusRequestsMilli =
    0.5;  // TODO(GG): explain , use config

constexpr int sendStatusPeriodMilli =
    0;  // if 0, this value is taken from config
...
static_assert(!forceViewChangeProtocolEnabled ||
                  !forceViewChangeProtocolDisabled,
              "");

constexpr int viewChangeTimeoutMilli =
    0;  //  80 * 1000; // 1 * 60 * 1000; // 20 * 1000; // if 0, this value is
        //  taken from config
...
constexpr bool dynamicCollectorForPartialProofs =
    true;  // if false, then the primary will be the collector

constexpr bool dynamicCollectorForExecutionProofs =
    false;  // if false, then the primary will be the collector

///////////////////////////////////////////////////////////////////////////////
// State Transfer
///////////////////////////////////////////////////////////////////////////////

constexpr int timeToWaitBeforeStartingStateTransferInMainWindowMilli =
    2000;  // TODO(GG): move to configuration??
...
constexpr float ControllerWithSimpleHistory_debugDowngradeFactor =
    0.85F;  //  0.0F;

constexpr float
    ControllerWithSimpleHistory_debugUpgradeFactorForFastOptimisticPath = 0.95F;

constexpr float ControllerWithSimpleHistory_debugUpgradeFactorForFastPath =
    0.85F;
