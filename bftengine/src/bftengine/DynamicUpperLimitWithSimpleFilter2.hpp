...
namespace bftEngine
{
	namespace impl
	{
		...
		{
		public:
			DynamicUpperLimitWithSimpleFilter(T initialUpperLimit, int16_t s, T maxUpperLimit, T minUpperLimit,
				int16_t evalPeriod, int16_t resetPoint, double maxIncreasingFactor, double maxDecreasingFactor)
				: numOfS{ s }, maxLimit{ maxUpperLimit }, minLimit{ minUpperLimit },
				evalPeriod{ evalPeriod }, resetPoint{ resetPoint },
				maxIncreasing{ maxIncreasingFactor }, maxDecreasing{ maxDecreasingFactor }
			{
			...
			virtual void add(T x)
			{
				...
				if (((T)newLimit) > maxVal)	currentUpperLimit = maxVal;
				else if (((T)newLimit) < minVal) currentUpperLimit = minVal;
				else currentUpperLimit = (T)newLimit;

				if (avgAndVar.numOfElements() >= resetPoint)
					avgAndVar.reset();
			}

			virtual T upperLimit()
			{
