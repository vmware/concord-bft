...
void NullStateTransfer::createCheckpointOfCurrentState(
...
void NullStateTransfer::getDigestOfCheckpoint(uint64_t checkpointNumber,
  ...
  LOG_WARN_F(GL,
             "State digest is only based on sequence number (becuase state "
             "transfer module has not been loaded)");
  ...
  DigestUtil::compute(
      (char*)&checkpointNumber, sizeof(checkpointNumber), (char*)&d, sizeof(d));
...
void NullStateTransfer::startCollectingState() {
  ...
  LOG_ERROR_F(GL,
              "Replica is not able to collect state from the other replicas "
              "(becuase state transfer module has not been loaded)");
  errorReport = true;
}
...
bool NullStateTransfer::loadReservedPage(uint32_t reservedPageId,
                                         uint32_t copyLength,
...
void NullStateTransfer::saveReservedPage(uint32_t reservedPageId,
                                         uint32_t copyLength,
...
void NullStateTransfer::handleStateTransferMessage(char* msg,
                                                   uint32_t msgLen,
                                                   uint16_t senderId) {
