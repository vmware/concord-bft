...
class SimpleClientImp : public SimpleClient, public IReceiver {
 public:
  ...
  virtual int sendRequestToReadLatestSeqNum(
      uint64_t timeoutMilli, uint64_t& outLatestReqSeqNum) override;
  ...
  virtual void onConnectionStatusChanged(
      const NodeNum node, const ConnectionStatus newStatus) override;
 ...
 protected:
  ...
  MsgsCertificate<ClientReplyMsg, false, false, true, SimpleClientImp>
      replysCertificate;
...
// in this version we assume that the set of replicas is
// 0,1,2,...,numberOfReplicas (TODO(GG): should be changed to support full
...
int SimpleClientImp::sendRequest(bool isReadOnly,
  ...
  ClientRequestMsg* reqMsg = new ClientRequestMsg(
      _clientId, isReadOnly, reqSeqNum, lengthOfRequest, request);
  ...
  while (true) {
    ...
    if (timeoutMilli != INFINITE_TIMEOUT &&
        (uint64_t)absDifference(beginTime, currTime) > timeoutMilli) {
      requestTimeout = true;
      break;
    }
  ...
  if (requestCommitted) {
    ...
    uint64_t durationMilli =
        ((uint64_t)absDifference(getMonotonicTime(), beginTime)) / 1000;
...
int SimpleClientImp::sendRequestToReadLatestSeqNum(
    uint64_t timeoutMilli, uint64_t& outLatestReqSeqNum) {
  Assert(false);  // not implemented yet
  return 0;
}
...
void SimpleClientImp::onConnectionStatusChanged(
...
void SimpleClientImp::sendPendingRequest() {
  ...
  const bool resetReplies =
      (numberOfTransmissions % clientPeriodicResetThresh == 0);

  const bool sendToAll =
      pendingRequest->isReadOnly() || !_primaryReplicaIsKnown ||
      (numberOfTransmissions == clientSendsRequestToAllReplicasFirstThresh) ||
      (numberOfTransmissions > clientSendsRequestToAllReplicasFirstThresh &&
       (numberOfTransmissions % clientSendsRequestToAllReplicasPeriodThresh ==
        0)) ||
      resetReplies;
  ...
  if (sendToAll) {
    ...
    // int stat =
    _communication->sendAsyncMessage(
        _knownPrimaryReplica, pendingRequest->body(), pendingRequest->size());
    // TODO(GG): handle errors (print and/or ....)
  }
}

class SeqNumberGeneratorForClientRequestsImp
    : public SeqNumberGeneratorForClientRequests {
...
uint64_t SeqNumberGeneratorForClientRequestsImp::
    generateUniqueSequenceNumberForRequest() {
  std::chrono::time_point<std::chrono::system_clock> n =
      std::chrono::system_clock::now();

  uint64_t milli = std::chrono::duration_cast<std::chrono::milliseconds>(
                       n.time_since_epoch())
                       .count();
...
SeqNumberGeneratorForClientRequests* SeqNumberGeneratorForClientRequests::
