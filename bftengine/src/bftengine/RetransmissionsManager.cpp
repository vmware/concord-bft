...
class RetransmissionsLogic {
 public:
  RetransmissionsLogic(uint16_t maxOutNumOfSeqNumbers)
      : maxOutSeqNumbers{maxOutNumOfSeqNumbers}, lastStable{0} {}
  ...
  void clearPendingRetransmissions()  // NB: should be used before moving to a
                                      // new view
  {
  ...
  void processSend(Time time,
    ...
    if (trackedItem->seqNumber != msgSeqNum || ignorePreviousAcks) {
      ...
      // TODO(GG): if ignorePreviousAcks==true, we may already have related
      // retransmissions in pendingRetransmissionsHeap (make sure that this is
      // okay)
    ...
    if (waitTimeMilli >=
        PARM::maxTimeBetweenRetranMilli)  // maxTimeBetweenRetranMilli is
                                          // treated as "infinite"
    {
      trackedItem->ackOrAbort = true;
      return;
    ...
    if (pendingRetransmissionsHeap.size() >=
        PARM::maxNumberOfConcurrentManagedTransmissions)
      makeRoom();
  ...
  void processAck(Time time,
    ...
    if ((trackedItem->seqNumber == msgSeqNum) &&
        (trackedItem->ackOrAbort == false) && (!(time < tmp))) {
      trackedItem->ackOrAbort = true;
  ...
  void getSuggestedRetransmissions(
      Time currentTime,
    ...
    while (true) {
      ...
      if (!t->ackOrAbort && (t->seqNumber > lastStable) &&
          (t->seqNumber <= lastStable + maxOutSeqNumbers)) {
 ...
 protected:
  struct TrackedItem {
    ...
    TrackedItem(uint16_t rId, uint16_t mType)
        : replicaId{rId}, msgType{mType} {}
   ...
   public:
    ...
    void add(uint64_t responseTimeMilli) {
      const uint64_t hours24 =
          24 * 60 * 60 * 1000;  // TODO(GG): ?? we only wanted safe conversion
                                // from uint64_t to double
      ...
      if (numOfVals % PARM::evalPeriod == 0) {
        const uint64_t maxVal =
            std::min((uint64_t)PARM::maxTimeBetweenRetranMilli,
                     retranTimeMilli * PARM::maxIncreasingFactor);
        const uint64_t minVal =
            std::max((uint64_t)PARM::minTimeBetweenRetranMilli,
                     retranTimeMilli / PARM::maxDecreasingFactor);
  ...
  inline TrackedItem* getTrackedItem(uint16_t replicaId,
    ...
    std::unordered_map<uint64_t, TrackedItem*>::const_iterator it =
        trackedItemsMap.find(itemId);
  ...
  inline ReplicaInfo* getReplicaInfo(uint16_t replicaId, uint16_t msgType) {
    ...
    std::unordered_map<uint64_t, ReplicaInfo*>::const_iterator it =
        replicasMap.find(itemId);
  ...
  void makeRoom() {
    ...
    while (pendingRetransmissionsHeap.size() >=
           PARM::maxNumberOfConcurrentManagedTransmissions) {
      const PendingRetran oldItem = pendingRetransmissionsHeap.top();
      pendingRetransmissionsHeap.pop();
      if (!oldItem.item->ackOrAbort && oldItem.item->seqNumber > lastStable)
        itemIgnored++;
    }
    if (itemIgnored > 0) {
      // TODO(GG): warning
...
class RetranProcResultInternalMsg : public InternalMessage {
 ...
 public:
  ...
  virtual ~RetranProcResultInternalMsg() override {
    std::forward_list<RetSuggestion>* p =
...
void RetransmissionsManager::add(const Event& e) {
  ...
  if (setOfEvents->size() < PARM::sufficientNumberOfMsgsToStartBkProcess)
    return;

  bool succ = tryToStartProcessing();

  if (!succ &&
      (setOfEvents->size() >= PARM::maxNumberOfMsgsInThreadLocalQueue)) {
...
bool RetransmissionsManager::tryToStartProcessing() {
  ...
  class RetransmissionsProcessingJob : public SimpleThreadPool::Job {
   ...
   public:
    RetransmissionsProcessingJob(
        InternalReplicaApi* const r,
    ...
    virtual void execute() override {
      ...
      std::forward_list<RetSuggestion>* suggestedRetransmissions =
          new std::forward_list<RetSuggestion>();
