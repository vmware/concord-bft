...
namespace bftEngine
{
	namespace impl
	{
		...
		void PartialExecProofsSet::addSelf(PartialExecProofMsg* m, Digest& merkleRoot, std::set<FullExecProofMsg*> fullExecProofs)
		{
		...
		public:
			MerkleExecSignatureInternalMsg(InternalReplicaApi* const internalReplicaApi, ViewNum viewNumber, SeqNum seqNumber, uint16_t signatureLength, const char* signature)
				: replicaApi(internalReplicaApi)
			{
		...
		void PartialExecProofsSet::tryToCreateFullProof()
		{
			...
			if ((participatingReplicas.size() == (numOfRquiredPartialProofs - 1)) && (myPartialExecProof != nullptr))
			{
				...
				AsynchExecProofCreationJob* j = new AsynchExecProofCreationJob(replicaApi, thresholdVerifier(), acc, expectedDigest, myPEP->seqNumber(), myPEP->viewNumber());
