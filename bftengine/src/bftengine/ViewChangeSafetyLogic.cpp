...
///////////////////////////////////////////////////////////////////////////////
// Internal methods and types  used by the implementation of
// ViewChangeSafetyLogic
///////////////////////////////////////////////////////////////////////////////

static int compareSeqNumbers(const void* a,
                             const void* b)  // used to sort sequence numbers
{
  SeqNum x = *((SeqNum*)a);
  SeqNum y = *((SeqNum*)b);
...
bool operator==(
    const FastElem& lhs,
    const FastElem& rhs)  // TODO(GG): make sure that this method is used
                          // correctly (should be called from the unordered_map)
{
  return lhs.prePrepreDigest() == rhs.prePrepreDigest();
}

struct FastElemHash {
  size_t operator()(FastElem const& s) const noexcept {
    return s.e->prePrepreDigest.hash();
  }
};
...
struct SlowElemCompare {
  bool operator()(const SlowElem& lhs, const SlowElem& rhs) const {
    ...
    if (leftCV != rightCV) {
      ...
      // (2) we have 2 different valid PreparedCertificate (this means that more
      // than f replicas are malicious and/or the crypto is broken)
...
static bool checkSlowPathCertificates(
    std::set<SlowElem, SlowElemCompare>&
        slowPathCertificates)  // for debug only
{
  ...
  for (SlowElem e : slowPathCertificates) {
    if (!lastElement.isNull())  // if this is not the first element
    {
      ...
      if (e.certificateView() == lastElement.certificateView()) {
        ...
        LOG_WARN_F(
            GL,
            "Found two conflicting prepared certificate for SeqNum %" PRIu64,
            e.seqNum());
      }
    }
...
ViewChangeSafetyLogic::ViewChangeSafetyLogic(
    ...
    : N(n),
      F(f),
      C(c),
      preparedCertVerifier(preparedCertificateVerifier),
      nullDigest(digestOfNull) {
  Assert(N == (3 * F + 2 * C + 1));
}

// TODO(GG): consider to optimize this method
SeqNum ViewChangeSafetyLogic::calcLBStableForView(
    ViewChangeMsg** const viewChangeMsgsOfPendingView) const {
  ...
  for (uint16_t i = 0; i < N; i++) {
    ...
    if (n == 0) {  // if this is the first message
      ...
      Assert(v ==
             vc->newView())  // all VC messages should refer to the same view
    }
...
void ViewChangeSafetyLogic::computeRestrictions(
  ...
  // TODO(GG): optimize the restricted range (e.g., add lastPrepared to each VC
  // - the max of all VC msgs can be used as a better upper bound)
  ...
  for (uint16_t i = 0; i < N; i++) {
    ...
    ViewChangeMsg::ElementsIterator* iter =
        new ViewChangeMsg::ElementsIterator(vc);

    iter->goToAtLeast(lowerBound);
    if (iter->end())  // no relevant elements in message
      delete iter;    // TODO(GG): avoid this deletion (by adding new ctor to
                      // ViewChangeMsg::ElementsIterator)
    else
      VCIterators.push_back(iter);
  }

  const bool noElements =
      VCIterators.empty();  // (useful when we don't have requests, and we still
                            // need to change view)
  ...
  if (noElements) {
    ...
    // TODO(GG): patch (we should fix the "stable point bug" (and this patch
    // will not be needed)). TODO(GG): for simplicity, the patch assumes that:
    // kWorkWindowSize == 2 * checkpointWindowSize
...
bool ViewChangeSafetyLogic::computeRestrictionsForSeqNum(
  ...
  for (SlowElem slow : slowPathCertificates) {
    ...
    if (valid) {
      ...
      LOG_WARN_F(GL,
                 "An invalid prepared certificate for SeqNum %" PRIu64
                 " is ignored",
                 s);
    }
  }
  ...
  if (!selectedSlow.isNull())
    minRelevantFastView = selectedSlow.certificateView() + 1;
  ...
  while ((!VCIterators.empty()) && (currentIdx <= IdxOfMaxLiveIterator)) {
    ...
    if (end || elem->seqNum > upperBound)  // if currIter is no longer needed
    {
      ...
      Assert(VCIterators.size() ==
             (IdxOfMaxLiveIterator + 1));  // TODO(GG): delete
    } else {
      currentIdx++;
    }
  }
