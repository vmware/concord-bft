...
void SimpleController::onStartingSlowCommit(SeqNum n) {
  ...
  if (onlyOptimisticFast) {
    if (currentFirstPath == CommitPath::OPTIMISTIC_FAST &&
        numberOfSlowCommitPaths == (dThreshold))
      currentFirstPath = CommitPath::SLOW;
  } else {
    if (currentFirstPath == CommitPath::OPTIMISTIC_FAST &&
        numberOfSlowCommitPaths == dThreshold)
      currentFirstPath = CommitPath::FAST_WITH_THRESHOLD;
    else if (currentFirstPath == CommitPath::FAST_WITH_THRESHOLD &&
             numberOfSlowCommitPaths == (dThreshold * 2))
      currentFirstPath = CommitPath::SLOW;
  }
}
