...
#define RSA_STANDARD OAEP<SHA256>
		...
		bool RSAKeysGenerator::generateKeys(string& outPublicKey, string& outPrivateKey)
		{
			outPrivateKey.clear();
		...
		void SecretSharingOperations::splitBinaryString(uint16_t threshold, uint16_t nShares, string binaryData, const char *seed, string* outBinaryStringArray, uint16_t lenOutBinaryStringArray)
		{
			...
			CryptoPP::SecretSharing* secretSharing = new CryptoPP::SecretSharing(rng, threshold, nShares, channelSwitch);
		...
		void SecretSharingOperations::recoverBinaryString(uint16_t threshold, string inBinaryStringArray[], string& outBinaryData)
		{
			VERIFY(threshold > 1);
			VERIFY(threshold <= 1000);

			CryptoPP::SecretRecovery recovery(threshold, new CryptoPP::StringSink(outBinaryData));
			...
			{
				strSources[i].reset(new CryptoPP::StringSource(inBinaryStringArray[i], false));
				strSources[i]->Pump(4);
				strSources[i]->Get(channel, 4);
				strSources[i]->Attach(new CryptoPP::ChannelSwitch(recovery, string((char *)channel.begin(), 4)));
			}
		...
		void SecretSharingOperations::splitHexString(uint16_t threshold, uint16_t nShares, string hexData, const char *seed, string* outHexStringArray, uint16_t lenOutHexStringArray)
		{
			VERIFY(threshold <= nShares);
			VERIFY(threshold > 1);
			VERIFY(nShares > 1);
			...
			string binData;
			CryptoPP::StringSource ss(hexData, true, new CryptoPP::HexDecoder(new CryptoPP::StringSink(binData)));

			for (uint16_t i = 0; i < nShares; i++) outHexStringArray[i].clear();

			splitBinaryString(threshold, nShares, binData, seed, outHexStringArray, nShares); // we use outHexStringArray to store the binary strings (to avoid additional allocation of strings)

			for (uint16_t i = 0; i < nShares; i++)
			{
				string binString = outHexStringArray[i];
				outHexStringArray[i].clear();
				CryptoPP::StringSource ss((const byte*)binString.data(), binString.length(), true, new CryptoPP::HexEncoder(new CryptoPP::StringSink(outHexStringArray[i])));
			}
		}

		void SecretSharingOperations::recoverHexString(uint16_t threshold, string inHexStringArray[], string& outHexData)
		{
			VERIFY(threshold > 1);
			VERIFY(threshold <= 1000);
			...
			for (uint16_t i = 0; i < threshold; i++)
			{
				...
				CryptoPP::StringSource ss(inHexStringArray[i], true, new CryptoPP::HexDecoder(new CryptoPP::StringSink(binaryStringArray[i])));
			}
			...
			CryptoPP::StringSource ss(binData, true, new CryptoPP::HexEncoder(new CryptoPP::StringSink(outHexData)));
