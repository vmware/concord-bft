...
class ClientReplyMsg : public MessageBase {
  ...
  static_assert(sizeof(ClientReplyMsgHeader::msgType) ==
                    sizeof(MessageBase::Header),
                "");
  static_assert(sizeof(ClientReplyMsgHeader::reqSeqNum) == sizeof(ReqId), "");
  static_assert(sizeof(ClientReplyMsgHeader::currentPrimaryId) ==
                    sizeof(ReplicaId),
                "");
  static_assert(sizeof(ClientReplyMsgHeader) == 16,
                "ClientRequestMsgHeader is 16B");
  // TODO(GG): more asserts

 public:
  ClientReplyMsg(ReplicaId primaryId, ReqId reqSeqNum, ReplicaId replicaId);

  ClientReplyMsg(ReplicaId replicaId,
                 ReqId reqSeqNum,
                 char* reply,
  ...
  uint32_t maxReplyLength() const {
    return internalStorageSize() - sizeof(ClientReplyMsgHeader);
  }
  ...
  static bool ToActualMsgType(NodeIdType myId,
                              MessageBase* inMsg,
