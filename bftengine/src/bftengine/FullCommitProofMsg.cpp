...
bool FullCommitProofMsg::ToActualMsgType(const ReplicasInfo& repInfo,
                                         MessageBase* inMsg,
  ...
  if (t->size() < (sizeof(FullCommitProofMsgHeader) + thresholSignatureLength))
    return false;

  // TODO(GG): TBD - check something about the collectors identity (and in other
  // similar messages)
