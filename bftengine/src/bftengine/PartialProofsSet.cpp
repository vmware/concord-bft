...
namespace bftEngine
{
	namespace impl
	{
		...
		PartialProofsSet::PartialProofsSet(InternalReplicaApi* const rep)
			: replica(rep),
			numOfRquiredPartialProofsForFast((3 * rep->getReplicasInfo().fVal()) + rep->getReplicasInfo().cVal() + 1),
			numOfRquiredPartialProofsForOptimisticFast((3 * rep->getReplicasInfo().fVal()) + (2 * rep->getReplicasInfo().cVal()) + 1),
			seqNumber(0),
		...
		PartialProofsSet::~PartialProofsSet()
		{
			resetAndFree();
		}
		...
		void PartialProofsSet::resetAndFree()
		{
			...
			expectedDigest.makeZero();
			timeOfSelfPartialProof = MinTime;
			if (thresholdAccumulatorForFast) thresholdVerifier(CommitPath::FAST_WITH_THRESHOLD)->release(thresholdAccumulatorForFast);
			thresholdAccumulatorForFast = nullptr;
			if (thresholdAccumulatorForOptimisticFast) thresholdVerifier(CommitPath::OPTIMISTIC_FAST)->release(thresholdAccumulatorForOptimisticFast);
			thresholdAccumulatorForOptimisticFast = nullptr;
		}
		...
		void PartialProofsSet::addSelfMsgAndPPDigest(PartialCommitProofMsg* m, Digest& digest)
		{
			...
			// NOTE: Accumulator might not be created yet, but the thresholdAccumulator() method creates it for us transparently if needed
			thresholdAccumulator(m->commitPath())->setExpectedDigest(reinterpret_cast<unsigned char *>(digest.content()), DIGEST_SIZE);
		...
		bool PartialProofsSet::addMsg(PartialCommitProofMsg* m)
		{
			...
			{
				if (participatingReplicasInOptimisticFast.count(repId) > 0) return false; // check that replica id has not been added yet

				if (participatingReplicasInOptimisticFast.size() < numOfRquiredPartialProofsForOptimisticFast - 1)
				{
			...
			else
			{
				...
				if (participatingReplicasInFast.size() < numOfRquiredPartialProofsForFast - 1)
				{
		...
		void PartialProofsSet::addImp(PartialCommitProofMsg* m, CommitPath cPath)
		{
			if (cPath == CommitPath::OPTIMISTIC_FAST)
			{
				...
				if ((participatingReplicasInOptimisticFast.size() == (numOfRquiredPartialProofsForOptimisticFast - 1)) && (selfPartialCommitProof != nullptr))
				{
			...
			else
			{
				...
				if ((participatingReplicasInFast.size() == (numOfRquiredPartialProofsForFast - 1)) && (selfPartialCommitProof != nullptr))
				{
					tryToCreateFullProof();
				}
			}
		...
		bool PartialProofsSet::addMsg(FullCommitProofMsg* m)
		{
			...
			bool succ = thresholdVerifier(myPCP->commitPath())->verify((const char*)&expectedDigest, sizeof(Digest),
		...
		FullCommitProofMsg* PartialProofsSet::getFullProof()
		{
		...
		public:
			...
			virtual void execute()
			{
				...
				{
					FullCommitProofMsg* fcpMsg = new FullCommitProofMsg(me->getReplicasInfo().myId(), view, seqNumber, bufferForSigComputations, (uint16_t)sigLength);
		...
		void PartialProofsSet::tryToCreateFullProof()
		{
			...
			if (cPath == CommitPath::OPTIMISTIC_FAST)
			{
				ready = (participatingReplicasInOptimisticFast.size() == (numOfRquiredPartialProofsForOptimisticFast - 1));
				thresholdAccumulator = thresholdAccumulatorForOptimisticFast;
			}
			else
			{
				ready = (participatingReplicasInFast.size() == (numOfRquiredPartialProofsForFast - 1));
...

			{
				...
				AsynchProofCreationJob* j = new AsynchProofCreationJob(replica, thresholdVerifier(cPath), acc, expectedDigest, myPCP->seqNumber(), myPCP->viewNumber());
