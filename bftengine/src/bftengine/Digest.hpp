...
class Digest {
 public:
  ...
  static void calcCombination(
      const Digest& inDigest,
      int64_t inDataA,
      int64_t inDataB,
      Digest& outDigest)  // TODO(GG): consider to change this function (TBD -
                          // check security)
  {
...
static_assert(DIGEST_SIZE >= sizeof(uint64_t),
              "Digest size should be >= sizeof(uint64_t)");
