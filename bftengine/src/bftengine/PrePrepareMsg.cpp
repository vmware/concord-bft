...
uint32_t getRequestSizeTemp(
    const char* request);  // TODO(GG): change - call directly to object
...
bool PrePrepareMsg::ToActualMsgType(const ReplicasInfo& repInfo,
                                    MessageBase* inMsg,
  ...
  // NB: the actual expected sender is verified outside this class (becuase in
  // some cases, during view-change protocol,  this message may sent by a
  // non-primary replica to the primary replica).
  ...
  if ((tmp->firstPath() == CommitPath::FAST_WITH_THRESHOLD) &&
      (repInfo.cVal() == 0))
    return false;
  ...
  if (tmp->b()->numberOfRequests >= tmp->b()->endLocationOfLastRequest)
    return false;
  ...
  const uint32_t requestSize =
      (tmp->b()->endLocationOfLastRequest - prePrepareHeaderPrefix);
...
void PrePrepareMsg::finishAddingRequests() {
  ...
  const uint32_t requestSize =
      (b()->endLocationOfLastRequest - prePrepareHeaderPrefix);
  DigestUtil::compute(requestBuffer, requestSize, (char*)&d, sizeof(Digest));
