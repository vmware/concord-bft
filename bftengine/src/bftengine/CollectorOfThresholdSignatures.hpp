...
namespace bftEngine
{
	namespace impl
	{
		...
		{
		public:
			CollectorOfThresholdSignatures(void* cnt)
			{
				this->context = cnt;
			}
			...
			bool addMsgWithPartialSignature(PART* partialSigMsg, ReplicaId repId)
			{
				...
				if ((combinedValidSignatureMsg != nullptr) || (replicasInfo.count(repId) > 0))
					return false;
			...
			{
				if (combinedValidSignatureMsg != nullptr || candidateCombinedSignatureMsg != nullptr)
					return false;
			...
			{
				return (combinedValidSignatureMsg != nullptr);
			}

			void onCompletionOfSignaturesProcessing(SeqNum seqNumber, ViewNum  view, const std::set<ReplicaId>& replicasWithBadSigs) // if we found bad signatures
			{
			...
			void onCompletionOfSignaturesProcessing(SeqNum seqNumber, ViewNum  view,
				const char* combinedSig, uint16_t combinedSigLen) // if we compute a valid combined signature
			{
				Assert(expectedSeqNumber == seqNumber);
				Assert(expectedView == view);
				Assert(processingSignaturesInTheBackground);
				...
				combinedValidSignatureMsg = ExternalFunc::createCombinedSignatureMsg(context, seqNumber, view, combinedSig, combinedSigLen);
			}
		...
		protected:
			...
			void trySendToBkThread()
			{
				...
				if (processingSignaturesInTheBackground || expectedSeqNumber == 0)
					return;
				...
				if (candidateCombinedSignatureMsg != nullptr)
				{
					...
					CombinedSigVerificationJob* bkJob =
						new CombinedSigVerificationJob(ExternalFunc::thresholdVerifier(context),
				...
				else if (numberOfUnknownSignatures >= numOfRequiredSigs)
				{
					...
					SignaturesProcessingJob* bkJob = new SignaturesProcessingJob(ExternalFunc::thresholdVerifier(context), &ExternalFunc::incomingMsgsStorage(context), expectedSeqNumber, expectedView, expectedDigest, numOfRequiredSigs, context);
					...
					for (std::pair<uint16_t, RepInfo>&& info : replicasInfo)
					{
						...
						if (numOfPartSigsInJob == numOfRequiredSigs)
							break;
					}
			...
			public:
				...
				SignaturesProcessingJob(
					IThresholdVerifier* const thresholdVerifier,
			...
			public:
				CombinedSigVerificationJob(
					IThresholdVerifier* const thresholdVerifier,
					IncomingMsgsStorage* const replicaMsgsStorage,
					SeqNum seqNum, ViewNum  view, Digest& digest,
					char* const combinedSigBody, uint16_t combinedSigLength, void* cnt) :
					verifier{ thresholdVerifier },
					repMsgsStorage{ replicaMsgsStorage },
					expectedSeqNumber{ seqNum },
					expectedView{ view },
