...
namespace bftEngine
{
	namespace impl
	{
		...
		bool CheckpointInfo::isCheckpointCertificateComplete() const
		{
			return checkpointCertificate->isComplete();
		}

		CheckpointMsg* CheckpointInfo::selfCheckpointMsg() const
		{
			return checkpointCertificate->selfMsg();
		}

		void CheckpointInfo::tryToMarkCheckpointCertificateCompleted()
		{
			checkpointCertificate->tryToMarkComplete();
		}

		bool CheckpointInfo::checkpointSentAllOrApproved() const
		{
			return sentToAllOrApproved;
		}

		Time CheckpointInfo::selfExecutionTime()
		{
			return executed;
		}
		...
		void CheckpointInfo::setCheckpointSentAllOrApproved()
		{
			sentToAllOrApproved = true;
		}
		...
		void CheckpointInfo::init(CheckpointInfo& i, void* d)
		{
			...
			i.checkpointCertificate = new MsgsCertificate<CheckpointMsg, true, true, true, CheckpointMsgCmp>(
				numOfReps, F, 2 * F + C + 1, myId);

		}

		void CheckpointInfo::free(CheckpointInfo& i)
		{
			i.resetAndFree();
		}

		void CheckpointInfo::reset(CheckpointInfo& i)
		{
			i.resetAndFree();
		}
