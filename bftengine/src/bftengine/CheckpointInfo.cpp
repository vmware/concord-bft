...
namespace bftEngine {
namespace impl {
bool CheckpointInfo::CheckpointMsgCmp::equivalent(CheckpointMsg* a,
                                                  CheckpointMsg* b) {
  return (a->seqNumber() == b->seqNumber()) &&
         (a->digestOfState() == b->digestOfState());
}
...
void CheckpointInfo::init(CheckpointInfo& i, void* d) {
  ...
  i.checkpointCertificate =
      new MsgsCertificate<CheckpointMsg, true, true, true, CheckpointMsgCmp>(
