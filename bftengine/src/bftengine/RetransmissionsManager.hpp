...
class RetransmissionsParams  // Parameters // TODO(GG): some of the parameters
                             // should be taken from the replica configuration
                             // (TBD)
{
 public:
  ...
  static_assert(
      maxTransmissionsPerMsg >= 2,
      "This functionality is not needed when maxTransmissionsPerMsg < 2");
...
class RetransmissionsManager {
 public:
  ...
  void onSend(uint16_t replicaId,
              SeqNum msgSeqNum,
              uint16_t msgType,
  ...
  void OnProcessingComplete();  // TODO(GG): should only be called by internal
                                // code ...
 ...
 protected:
  ...
  SimpleThreadPool* const
      pool;  // TODO(GG): not needed (use InternalReplicaApi*)
  IncomingMsgsStorage* const
      incomingMsgs;  // TODO(GG): not needed (use InternalReplicaApi*)
