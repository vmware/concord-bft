...
class ReplicaImp : public InternalReplicaApi, public ReplicaForStateTransfer {
 ...
 protected:
  ...
  bool tryToEnterView();
  void onNewView(std::vector<std::shared_ptr<PrePrepareMsg>>& prePreparesForNewView);
 ...
 private:
  ...
  struct Recorders {
    ...
    DEFINE_SHARED_RECORDER(send, MAX_VALUE_NANOSECONDS, Unit::NANOSECONDS);
    DEFINE_SHARED_RECORDER(executeReadOnlyRequest, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(executeWriteRequest, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(executeRequestsInPrePrepareMsg, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(executeRequestsAndSendResponses, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(prepareAndSendResponses, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(advanceActiveWindowMainLog, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(numRequestsInPrePrepareMsg, 2500, Unit::COUNT);
    DEFINE_SHARED_RECORDER(requestsQueueOfPrimarySize,
                           // Currently, hardcoded to 700 in ReplicaImp.cpp
                           701,
                           Unit::COUNT);
    DEFINE_SHARED_RECORDER(onSeqNumIsStable, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(onTransferringCompleteImp, MAX_VALUE_NANOSECONDS, Unit::NANOSECONDS);

    // Only updated by the primary
    DEFINE_SHARED_RECORDER(consensus, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);

    DEFINE_SHARED_RECORDER(timeInActiveView, MAX_VALUE_SECONDS, Unit::SECONDS);
    DEFINE_SHARED_RECORDER(timeInStateTransfer, MAX_VALUE_SECONDS, Unit::SECONDS);
    DEFINE_SHARED_RECORDER(checkpointFromCreationToStable, MAX_VALUE_SECONDS, Unit::SECONDS);

    // Measurements related to PrePrepare creation and sending
    DEFINE_SHARED_RECORDER(removeDuplicatedRequestsFromQueue, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(buildPrePrepareMessage, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(startConsensusProcess, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(finishAddingRequestsToPrePrepareMsg, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(addAllRequestsToPrePrepare, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(addSelfMsgPrePrepare, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(prePrepareWriteTransaction, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(broadcastPrePrepare, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(sendPreparePartialToSelf, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
    DEFINE_SHARED_RECORDER(sendPartialProofToSelf, MAX_VALUE_MICROSECONDS, Unit::MICROSECONDS);
  };
