...
class ReplicaImp : public InternalReplicaApi, public IReplicaForStateTransfer {
 protected:
  ...
  // the last SeqNum used to generate a new SeqNum (valid when this replica is
  // the primary)
  ...
  // last SeqNum executed  by this replica (or its affect was transferred to
  // this replica)
  ...
  SequenceWithActiveWindow<kWorkWindowSize, 1, SeqNum, SeqNumInfo, SeqNumInfo>*
      mainLog;
  ...
  Time timeOfLastStateSynch;  // last time the replica received a new state (via
                              // the state transfer mechanism)
  Time timeOfLastViewEntrance;  // last time the replica entered to a new view

  //
  ViewNum
      lastAgreedView;  // latest view number v such that the replica received
                       // 2f+2c+1 ViewChangeMsg messages with view >= v
 ...
 public:
  ...
  virtual void changeStateTransferTimerPeriod(
 ...
 protected:
  static std::unordered_map<uint16_t, PtrToMetaMsgHandler>
  createMapOfMetaMsgHandlers();

  template <typename T>
  void metaMessageHandler(MessageBase* msg);
  template <typename T>
  void metaMessageHandler_IgnoreWhenCollectingState(
      MessageBase* msg);  // TODO(GG): rename
  ...
  void MoveToHigherView(
      ViewNum nextView);  // also sends the ViewChangeMsg message
  void GotoNextView();
  ...
  void sendCommitPartial(
      SeqNum);  // TODO(GG): the argument should be a ref to SeqNumInfo
  ...
  virtual IThresholdVerifier* getThresholdVerifierForOptimisticCommit()
      override {
