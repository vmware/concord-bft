...
SimpleThreadPool::SimpleThreadPool(unsigned char numOfThreads)
    : numberOfThreads(numOfThreads), p(new InternalDataOfSimpleThreadPool) {
...
void SimpleThreadPool::stop(bool executeAllJobs) {
  ...
  for (int i = 0; i < numberOfThreads; i++) threadJoin(p->threadsArray[i]);
