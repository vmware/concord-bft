...
ControllerWithSimpleHistory::ControllerWithSimpleHistory(uint16_t C,
  ...
  if (isPrimary) onBecomePrimary(initialView, initialSeq);
}
...
void ControllerWithSimpleHistory::onBecomePrimary(ViewNum v, SeqNum s) {
  ...
  if (s % EvaluationPeriod == 0)
    ...
    nextFirstRelevantSeqNum =
        ((s / EvaluationPeriod) * EvaluationPeriod) + EvaluationPeriod + 1;
...
void ControllerWithSimpleHistory::onEndOfEvaluationPeriod() {
  ...
  if (currentFirstPath == CommitPath::SLOW) {
    ...
    if (ControllerWithSimpleHistory_debugUpgradeEnabled) {
      ...
      const float factorForFastPath =
          ControllerWithSimpleHistory_debugUpgradeFactorForFastPath;  //  0.85F;

      if (cyclesWithFullCooperation >=
          (factorForFastOptimisticPath * EvaluationPeriod))
        currentFirstPath = CommitPath::OPTIMISTIC_FAST;
      else if (!onlyOptimisticFast &&
               (cyclesWithFullCooperation + cyclesWithPartialCooperation >=
                (factorForFastPath * EvaluationPeriod)))
        currentFirstPath = CommitPath::FAST_WITH_THRESHOLD;
    }
  } else {
    Assert(currentFirstPath == CommitPath::OPTIMISTIC_FAST ||
           currentFirstPath == CommitPath::FAST_WITH_THRESHOLD);
    ...
    const float factor =
        ControllerWithSimpleHistory_debugDowngradeFactor;  // 0.85F; //  0.0F;

    if (ControllerWithSimpleHistory_debugDowngradeEnabled &&
        (successfulFastPaths < (factor * EvaluationPeriod))) {
      downgraded = true;
  ...
  if (!downgraded) {
    ...
    const uint32_t relMin = (uint32_t)((1 - maxUpdateInTimeToStartSlowPath) *
                                       currentTimeToStartSlowPathMilli);
    const uint32_t relMax = (uint32_t)((1 + maxUpdateInTimeToStartSlowPath) *
                                       currentTimeToStartSlowPathMilli);
...
void ControllerWithSimpleHistory::onSendingPrePrepare(SeqNum n,
