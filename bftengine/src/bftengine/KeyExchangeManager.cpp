...
void KeyExchangeManager::publishClientsPublicKeys(const std::string& keys) {
  ...
  LOG_DEBUG(ON_CHAIN_LOG, "Sends clients keys, cid: " << cid << " payload size " << keys.size());
  client_->sendRequest(
...
void KeyExchangeManager::sendInitialKey(std::optional<std::string> clientsPublicKeys) {
  ...
  auto ret = std::async(std::launch::async, [this, clientsPublicKeys]() {
    ...
    // In order to prevent race on the client, we'll wait untill we know, that all replicas finished processing the key
    // exchange requests.
