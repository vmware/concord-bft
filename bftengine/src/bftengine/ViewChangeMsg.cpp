...
void ViewChangeMsg::addElement(const ReplicasInfo& repInfo,
  ...
  // TODO(GG): we should make sure that this assert will never be violated (by
  // calculating the maximum  size of a ViewChangeMsg message required for the
  // actual configuration)
  Assert((size_t)(requiredSpace + repInfo.mySigManager().getMySigLength()) <=
         (size_t)internalStorageSize());
  ...
  if (hasPreparedCertificate) {
    PreparedCertificate* pCert =
        (PreparedCertificate*)(body() + b()->locationAfterLast +
                               sizeof(Element));

    pCert->certificateView = certificateView;
    pCert->certificateSigLength = certificateSigLength;

    char* pSig = (char*)(body() + b()->locationAfterLast + sizeof(Element) +
                         sizeof(PreparedCertificate));
...
bool ViewChangeMsg::ToActualMsgType(const ReplicasInfo& repInfo,
                                    MessageBase* inMsg,
  ...
  uint16_t sigLen =
      repInfo.mySigManager().getSigLength(t->idOfGeneratedReplica());
  if (dataLength < sizeof(ViewChangeMsgHeader))
    dataLength = sizeof(ViewChangeMsgHeader);
...
bool ViewChangeMsg::checkElements(uint16_t sigSize) const {
  ...
  while ((remainingBytes >= sizeof(Element)) &&
         (numOfActualElements < numberOfElements())) {
    numOfActualElements++;
    ...
    if (pElement->hasPreparedCertificate) {
      ...
      const int16_t s =
          sizeof(PreparedCertificate) + pCert->certificateSigLength;

      if (remainingBytes < s) return false;

      // we don't check the certificate here (becuase in many cases it is not
...
ViewChangeMsg::ElementsIterator::ElementsIterator(const ViewChangeMsg* const m)
    : msg{m} {
...
bool ViewChangeMsg::ElementsIterator::getCurrent(Element*& pElement) {
  ...
  if (pElement->hasPreparedCertificate) {
    ...
    PreparedCertificate* pCert =
        (PreparedCertificate*)(msg->body() + currLoc + sizeof(Element));
    Assert(remainingbytes >= sizeof(Element) + sizeof(PreparedCertificate) +
                                 pCert->certificateSigLength);
  }
