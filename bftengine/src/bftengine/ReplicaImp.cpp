...
namespace bftEngine
{
	namespace impl
	{
		...
		std::unordered_map<uint16_t, PtrToMetaMsgHandler> ReplicaImp::createMapOfMetaMsgHandlers()
		{
			...
			r[MsgCode::PrePrepare] = &ReplicaImp::metaMessageHandler<PrePrepareMsg>;
			...
			r[MsgCode::Request] = &ReplicaImp::metaMessageHandler<ClientRequestMsg>;
			...
			r[MsgCode::ClientGetTimeStamp] = &ReplicaImp::metaMessageHandler<ClientGetTimeStampMsg>;
			r[MsgCode::CollectStablePoint] = &ReplicaImp::metaMessageHandler<CollectStablePointMsg>;
			r[MsgCode::LocalCommitSet] = &ReplicaImp::metaMessageHandler<LocalCommitSetMsg>;
			return r;
		}
			...
			const size_t messageLength)
		{
            ...
            if (orderingMsgs && pMsg->type() == MsgCode::Request) {
		...
		void ReplicaImp::onBatchMessage(ClientRequestMsg *m)
		{
			...
			if (readOnly)
			{
				ClientReplyMsg reply(myReplicaId, reqSeqNum, (char*)&reqSeqNum, sizeof(uint64_t));
			...
			if (seqNumberOfLastReply < reqSeqNum)
			{
				if (isCurrentPrimary())
				{
					//if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum) && (requestsQueueOfPrimary.size() < std::max(5000, 2 * maxBatchSize))) // TODO(GG): use config/parameter
					if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum))
					{
						std::pair<std::uint16_t, std::uint64_t> key = std::make_pair(clientId, reqSeqNum);
						if (reqTime.find(key) == reqTime.end()) {
							ClientReplyMsg repMsg(myReplicaId, reqSeqNum, (char*)&reqSeqNum, sizeof(uint64_t));
							repMsg.setPrimaryId(currentPrimary());
				    		send(&repMsg, clientId);

                            reqTime[key] = getMonotonicTime();
		...
		void ReplicaImp::onMessage(ClientRequestMsg *m)
		{
			...
			if (seqNumberOfLastReply < reqSeqNum)
			{
				if (isCurrentPrimary())
				{
					if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum)) // TODO(GG): use config/parameter
					{
						requestsQueueOfPrimary.push(m);
						tryToSendPrePrepareMsg(true);
						return;
		...
		void ReplicaImp::tryToSendPrePrepareMsg(bool batchingLogic)
		{
			...
			//if (requestsInQueue < maxBatchSize && absDifference(getMonotonicTime(), localStablePoint) / 1000 < 100) return;
			if (requestsInQueue < maxBatchSize) return;
            ...
            */
			...
			while (nextRequest != nullptr && nextRequest->size() <= pp->remainingSizeForRequests() && pp->numberOfRequests() < maxBatchSize)
			{
        ...
        void ReplicaImp::onNewMessage(ClientRequestMsg *m)
		{
                        metric_received_client_requests_.Get().Inc();
			const NodeIdType clientId = m->clientProxyId();
			const bool readOnly = m->isReadOnly();
			...
			if (seqNumberOfLastReply < reqSeqNum)
			{
				if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum))
				{
					...
					std::pair<std::uint16_t, std::uint64_t> key = std::make_pair(clientId, reqSeqNum);
					auto it = localCommitSet.find(key);
					if (it != localCommitSet.end()) {
						delete it->second;
					} else {
						reqTime[key] = getMonotonicTime();
					}
					localCommitSet[key] = m;
				...
				{
					LOG_INFO_F(GL, "NewClientRequestMsg is ignored becuase request is old or replica has another pending request from the same client");
				}
			}
			else if (seqNumberOfLastReply == reqSeqNum)
			{
				LOG_INFO_F(GL, "NewClientRequestMsg has already been executed - retransmit reply to client");
			...
			{
				LOG_INFO_F(GL, "NewClientRequestMsg is ignored becuase request is old");
			}
        ...
        PrePrepareMsg* ReplicaImp::generatePrePrepareMsg() {
			...
			if (requestsQueueOfPrimary.size() == 0) return nullptr;
        ...
        void ReplicaImp::onMessage(CollectStablePointMsg* msg)
		{
			...
			for (uint16_t i = 0; i < msg->numberOfRequests(); i++) {
				auto it = localCommitSet.find(std::make_pair(ptr[i].clientId, ptr[i].seqNumber));
				if (it != localCommitSet.end()) {
					it->second->setAsReadyOnly();
				}
			}

			LocalCommitSetMsg m(myReplicaId, msg->prevStablePoint(), msg->nextStablePoint());
			for (auto it = localCommitSet.begin(); it != localCommitSet.end();) {
				Time ts = it->second->timeStamp();
				if (!it->second->isReadOnly() && ts > msg->prevStablePoint() && ts <= msg->nextStablePoint() && m.remainSize() >= it->second->size()) {
					m.addRequest(it->second);
					it++;
				} else if (ts <= msg->prevStablePoint()) {
					delete it->second;
                    localCommitSet.erase(it++);
				} else {
					it++;
				}
			}
		...
		void ReplicaImp::onMessage(LocalCommitSetMsg* msg)
		{
			...
			char* p = msg->requests();
			for (uint16_t i = 0; i < msg->numberOfRequests(); i++) {
				ClientRequestMsg m((ClientRequestMsgHeader*)p);
				std::pair<std::uint16_t, std::uint64_t> key = std::make_pair(m.clientProxyId(), m.requestSeqNum());
				auto it = localCommitSet.find(key);
				if (it == localCommitSet.end()) localCommitSet[key] = new ClientRequestMsg(&m);
				else if (it->second->timeStamp() < m.timeStamp()) {
					delete it->second;
					it->second = new ClientRequestMsg(&m);
				}
				p += m.size();
			}
			localCommitMsgs[msg->senderId()] = true;

			if (localCommitMsgs.size() == 2 * fVal) {
				for (auto it = localCommitSet.begin(); it != localCommitSet.end();) {
					Time ts = it->second->timeStamp();
					if (ts > localStablePoint && ts <= localNextStablePoint) {
						requestsQueueOfPrimary.push(it->second);
						localCommitSet.erase(it++);
					} else if (ts <= localStablePoint) {
						delete it->second;
                        localCommitSet.erase(it++);
					} else {
						it++;
		...
		void ReplicaImp::onMessage(ClientGetTimeStampMsg* msg)
		{
		...
		void ReplicaImp::tryToStartSlowPaths()
		{
			...
			{
				LOG_INFO_F(GL, "Replica::tryToStartSlowPaths() : minSeqNum(%" PRId64 ") > lastStableSeqNum(%" PRId64 ") + kWorkWindowSize", minSeqNum, lastStableSeqNum);
				return;
			}
...
//			initAllocator();
			...
			repsInfo = new ReplicasInfo(myReplicaId, *sigManager, numOfReplicas, fVal, cVal, config.dynamicCollectorEnabled, dynamicCollectorForExecutionProofs);
			...
			if (commitDuration > 0)
			{
			  timeManager = new TimeManager(thresholdSignerForSlowPathCommit);
			}
			internalThreadPool.start(maxConcurrentAgreementsByPrimary); // TODO(GG): use configuration
		}
...
//			freeAllocator();
		...
		void ReplicaImp::start()
		{
            ...
            if (commitDuration > 0) {
				std::thread oThread([this] {processOrderMessages(); });
				orderingThread.swap(oThread);
			}
			startSyncEvent.set();
		}
        ...
        void ReplicaImp::processOrderMessages()
		{
			fprintf(stderr, "Ordering Running");
			...
			while (!mainThreadShouldStop)
			{
				...
				{
					LOG_WARN_F(GL, "Unknown message msgtype=%d", m->type());
					delete m;
				}
			}
		}
		...
		void ReplicaImp::executeRequestsInPrePrepareMsg(PrePrepareMsg* ppMsg)
		{
			...
			while (reqIter.getAndGoToNext(requestBody)) 
			{
				...
				//TODO:BATCH
				if (!supportDirectProofs && actualReplyLength != 0 && commitDuration == 0)
				{
					send(replyMsg, clientId);
				}
