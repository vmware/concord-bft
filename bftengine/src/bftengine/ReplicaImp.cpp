...
void ReplicaImp::onSeqNumIsSuperStable(SeqNum superStableSeqNum) {
  LOG_INFO(GL,
           "Reached to n/n checkpoint in " << (getMonotonicTime() - timeOfLastStableSeqNum).count()
                                           << " after stable checkpoint");
...
void ReplicaImp::start() {
  ...
  std::shared_ptr<ISecureStore> backupsec(new KeyManager::FileSecureStore(
      ReplicaConfigSingleton::GetInstance().GetKeyViewFilePath(), config_.replicaId, "_backed"));
