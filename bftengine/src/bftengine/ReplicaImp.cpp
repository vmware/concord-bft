...
namespace bftEngine
{
	namespace impl
	{
		...
		{
			Logger::printWarn("Node %d received invalid message from Node %d (type==%d)", (int)myReplicaId, (int)msg->senderId(), (int)msg->type());
		...
		void ReplicaImp::send(MessageBase* m, NodeIdType dest)
		{
			...
			if (m->type() == MsgCode::Checkpoint)
			{
				...
				if (c->digestOfState().isZero()) Logger::printWarn("Debug: checkpoint digest is zero");
			}
		...
		void ReplicaImp::sendRaw(char* m, NodeIdType dest, uint16_t type, MsgSize size)
		{
			...
			if (errorCode != 0)
			{
				Logger::printError("In ReplicaImp::sendRaw - communication->sendAsyncMessage returned error %d for message type %d", errorCode, (int)type);
			}
		}
		...
		void ReplicaImp::onMessage(ClientRequestMsg *m)
		{
			...
			Logger::printInfo("Node %d received ClientRequestMsg (clientId=%d reqSeqNum=%" PRIu64 ", readOnly=%d) from Node %d", myReplicaId, clientId, reqSeqNum, readOnly ? 1 : 0, senderId);

			if (stateTransfer->isCollectingState())
			{
				Logger::printInfo("ClientRequestMsg is ignored becuase this replica is collecting missing state from the other replicas");
				delete m;
				return;
			}
			...
			{
				Logger::printInfo("ClientRequestMsg is invalid");
			...
			{
				Logger::printInfo("ClientRequestMsg is ignored becuase current view is inactive");
				delete m;
				return;
			}
			...
			if (seqNumberOfLastReply < reqSeqNum)
			{
				if (isCurrentPrimary())
				{
					...
					{
						Logger::printInfo("ClientRequestMsg is ignored becuase: request is old, OR primary is current working on a request from the same client, OR queue contains too many requests");
					}
				}
				else // not the current primary
				{
					if (clientsManager->noPendingAndRequestCanBecomePending(clientId, reqSeqNum))
					{
						...
						Logger::printInfo("Sending ClientRequestMsg to current primary");
					}
					else
					{
						Logger::printInfo("ClientRequestMsg is ignored becuase request is old or replica has another pending request from the same client");
					}
				}
			}
			else if (seqNumberOfLastReply == reqSeqNum)
			{
				Logger::printInfo("ClientRequestMsg has already been executed - retransmit reply to client");
			...
			{
				Logger::printInfo("ClientRequestMsg is ignored becuase request is old");
			}
		...
		void ReplicaImp::tryToSendPrePrepareMsg(bool batchingLogic)
		{
			...
			Logger::printInfo("Sending PrePrepareMsg (seqNumber=%" PRId64 ", requests=%d, size=%d",
		...
		void ReplicaImp::onMessage(PrePrepareMsg* msg)
		{
			...
			Logger::printInfo("Node %d received PrePrepareMsg from node %d for seqNumber %" PRId64 " (size=%d)",
			...
			if (!currentViewIsActive() && viewsManager->waitingForMsgs() && msgSeqNum > lastStableSeqNum)
			{
				...
				{
					Logger::printInfo("Node %d adds PrePrepareMsg for seqNumber %" PRId64 " to viewsManager", (int)myReplicaId, msgSeqNum);
					tryToEnterView();
				}
				else
				{
					Logger::printInfo("Node %d does not add PrePrepareMsg for seqNumber %" PRId64 " to viewsManager", (int)myReplicaId, msgSeqNum);
				}
		...
		void ReplicaImp::tryToStartSlowPaths()
		{
			...
			for (SeqNum i = minSeqNum; i <= maxSeqNum; i++)
			{
				...
				Logger::printInfo("Primary initiates slow path for seqNum=%" PRId64 " (currTime=%lld timeOfPartProof=%lld", i, currTime, timeOfPartProof);
		...
		void ReplicaImp::onMessage(StartSlowCommitMsg* msg)
		{
			...
			Logger::printInfo("Node %d received StartSlowCommitMsg for seqNumber %" PRId64 "", myReplicaId, msgSeqNum);
			...
			if (relevantMsgForActiveView(msg))
			{
				...
				{
					Logger::printInfo("Node %d starts slow path for seqNumber %" PRId64 "", myReplicaId, msgSeqNum);
		...
		void ReplicaImp::sendPreparePartial(SeqNumInfo& seqNumInfo)
		{
			...
			if (seqNumInfo.getSelfPreparePartialMsg() == nullptr && seqNumInfo.hasPrePrepareMsg() && !seqNumInfo.isPrepared())
			{
				...
				Logger::printInfo("Sending PreparePartialMsg for seqNumber %" PRId64 "", pp->seqNumber());
		...
		void ReplicaImp::sendCommitPartial(const SeqNum s)
		{
			...
			Logger::printInfo("Sending CommitPartialMsg for seqNumber %" PRId64 "", s);
		...
		void ReplicaImp::onMessage(PartialCommitProofMsg* msg)
		{
			...
			Logger::printInfo("Node %d received PartialCommitProofMsg (size=%d) from node %d for seqNumber %" PRId64 "",
		...
		void ReplicaImp::onMessage(FullCommitProofMsg* msg)
		{
			...
			Logger::printInfo("Node %d received FullCommitProofMsg message for seqNumber %" PRId64 "",
		...
		void ReplicaImp::onMessage(PreparePartialMsg* msg)
		{
			...
			if (relevantMsgForActiveView(msg))
			{
				...
				Logger::printInfo("Node %d received PreparePartialMsg from node %d for seqNumber %" PRId64 "", (int)myReplicaId, (int)msgSender, msgSeqNum);
			...
			if (!msgAdded)
			{
				Logger::printInfo("Node %d ignored the PreparePartialMsg from node %d (seqNumber %" PRId64 ")", (int)myReplicaId, (int)msgSender, msgSeqNum);
				delete msg;
			}
		}
		...
		void ReplicaImp::onMessage(CommitPartialMsg* msg)
		{
			...
			if (relevantMsgForActiveView(msg))
			{
				...
				Logger::printInfo("Node %d received CommitPartialMsg from node %d for seqNumber %" PRId64 "", (int)myReplicaId, (int)msgSender, msgSeqNum);
			...
			if (!msgAdded)
			{
				Logger::printInfo("Node %d ignored the CommitPartialMsg from node %d (seqNumber %" PRId64 ")", (int)myReplicaId, (int)msgSender, msgSeqNum);
				delete msg;
			}
		}
		...
		void ReplicaImp::onMessage(PrepareFullMsg* msg)
		{
			...
			if (relevantMsgForActiveView(msg))
			{
				...
				Logger::printInfo("Node %d received PrepareFullMsg for seqNumber %" PRId64 "", (int)myReplicaId, msgSeqNum);
			...
			if (!msgAdded)
			{
				Logger::printInfo("Node %d ignored the PrepareFullMsg from node %d (seqNumber %" PRId64 ")", (int)myReplicaId, (int)msgSender, msgSeqNum);
				delete msg;
			}
		}
		...
		void ReplicaImp::onMessage(CommitFullMsg* msg)
		{
			...
			if (relevantMsgForActiveView(msg))
			{
				...
				Logger::printInfo("Node %d received CommitFullMsg for seqNumber %" PRId64 "", (int)myReplicaId, msgSeqNum);
			...
			if (!msgAdded)
			{
				Logger::printInfo("Node %d ignored the CommitFullMsg from node %d (seqNumber %" PRId64 ")", (int)myReplicaId, (int)msgSender, msgSeqNum);
				delete msg;
			}
		}
		...
		void ReplicaImp::onMessage(CheckpointMsg* msg)
		{
			...
			if ((msgSeqNum > lastStableSeqNum) && (msgSeqNum <= lastStableSeqNum + kWorkWindowSize))
			{
				...
				if (msgAdded) Logger::printInfo("Node %d added Checkpoint message from node %d for seqNumber %" PRId64 "",
			...
			if (msgIsStable && msgSeqNum > lastExecutedSeqNum)
			{
				...
				if (pos == tableOfStableCheckpoints.end() || pos->second->seqNumber() < msgSeqNum)
				{
					...
					Logger::printInfo("Node %d added stable Checkpoint message to tableOfStableCheckpoints (message from node %d for seqNumber %" PRId64 ")", (int)myReplicaId, (int)msgSenderId, msgSeqNum);
					...
					if ((uint16_t)tableOfStableCheckpoints.size() >= fVal + 1)
					{
						...
						Logger::printInfo("numRelevant=%d    numRelevantAboveWindow=%d", (int)numRelevant, (int)numRelevantAboveWindow);
			...
			const std::forward_list<RetSuggestion>* const suggestedRetransmissions)
		{
			...
			for (const RetSuggestion& s : *suggestedRetransmissions)
			{
				...
				case MsgCode::PrePrepare:
				{
					...
					Logger::printInfo("Replica %d retransmits to replica %d PrePrepareMsg with seqNumber %" PRId64 "", (int)myId, (int)s.replicaId, s.msgSeqNum);
				}
				break;
				case MsgCode::PartialCommitProof:
				{
					...
					Assert(msgToSend != nullptr);
					sendRetransmittableMsgToReplica(msgToSend, s.replicaId, s.msgSeqNum);
					Logger::printInfo("Replica %d retransmits to replica %d PartialCommitProofMsg with seqNumber %" PRId64 "", (int)myId, (int)s.replicaId, s.msgSeqNum);
				}
				break;
				/*  TODO(GG): do we want to use acks for FullCommitProofMsg ?
				*/
				case MsgCode::StartSlowCommit:
				{
					...
					sendRetransmittableMsgToReplica(msgToSend, s.replicaId, s.msgSeqNum);
					delete msgToSend;
					Logger::printInfo("Replica %d retransmits to replica %d StartSlowCommitMsg with seqNumber %" PRId64 "", (int)myId, (int)s.replicaId, s.msgSeqNum);
				}
				break;
				case MsgCode::PreparePartial:
				{
					...
					Assert(msgToSend != nullptr);
					sendRetransmittableMsgToReplica(msgToSend, s.replicaId, s.msgSeqNum);
					Logger::printInfo("Replica %d retransmits to replica %d PreparePartialMsg with seqNumber %" PRId64 "", (int)myId, (int)s.replicaId, s.msgSeqNum);
				}
				break;
				case MsgCode::PrepareFull:
				{
					...
					Assert(msgToSend != nullptr);
					sendRetransmittableMsgToReplica(msgToSend, s.replicaId, s.msgSeqNum);
					Logger::printInfo("Replica %d retransmits to replica %d PrepareFullMsg with seqNumber %" PRId64 "", (int)myId, (int)s.replicaId, s.msgSeqNum);
				}
				break;
				...
				case MsgCode::CommitPartial:
				{
					...
					Assert(msgToSend != nullptr);
					sendRetransmittableMsgToReplica(msgToSend, s.replicaId, s.msgSeqNum);
					Logger::printInfo("Replica %d retransmits to replica %d CommitPartialMsg with seqNumber %" PRId64 "", (int)myId, (int)s.replicaId, s.msgSeqNum);
				}
				break;
				...
				case MsgCode::CommitFull:
				{
					...
					Assert(msgToSend != nullptr);
					sendRetransmittableMsgToReplica(msgToSend, s.replicaId, s.msgSeqNum);
					Logger::printInfo("Replica %d retransmits to replica %d CommitFullMsg with seqNumber %" PRId64 "", (int)myId, (int)s.replicaId, s.msgSeqNum);
				}
				break;
		...
		void ReplicaImp::onMessage(ReplicaStatusMsg* msg)
		{
			...
			Logger::printInfo("Node %d received ReplicaStatusMsg from node %d", (int)myReplicaId, (int)msgSenderId);
		...
		void ReplicaImp::onMessage(ViewChangeMsg* msg)
		{
			...
			Logger::printInfo("Node %d received ViewChangeMsg (generatedReplicaId=%d, newView=%" PRId64 ", lastStable=%" PRId64 ", numberOfElements=%d).",
			...
			Logger::printInfo("ViewChangeMsg add=%d", (int)msgAdded);
			...
			{
				Logger::printInfo("Primary asks to leave view (primary Id=%d , view=%" PRId64 ")", (int)generatedReplicaId, curView);
		...
		void ReplicaImp::onMessage(NewViewMsg* msg)
		{
			...
			Logger::printInfo("Node %d received NewViewMsg message (senderId=%d, newView=%" PRId64 ")",
				(int)myReplicaId, (int)senderId, msg->newView());

			bool added = viewsManager->add(msg);

			Logger::printInfo("NewViewMsg add=%d", (int)added);
		...
		void ReplicaImp::MoveToHigherView(ViewNum nextView)
		{
			...
			Logger::printInfo("**************** In MoveToHigherView (curView=%" PRId64 ", nextView=%" PRId64 ", wasInPrevViewNumber=%d)",
			...
			Logger::printInfo("Sending view change message: new view=%" PRId64 ", wasInPrevViewNumber=%d, new primary=%d, lastExecutedSeqNum=%" PRId64 ", lastStableSeqNum=%" PRId64 "",
		...
		void ReplicaImp::onNewView(const std::vector<PrePrepareMsg*>& prePreparesForNewView)
		{
			...
			Logger::printInfo("**************** In onNewView curView=%" PRId64 " (num of PPs=%d, first safe seq=%" PRId64 ", last safe seq=%" PRId64 ", lastStableSeqNum=%" PRId64 ", lastExecutedSeqNum=%" PRId64 ", stableLowerBoundWhenEnteredToView= %" PRId64 ")",
			...
			Logger::printInfo("**************** Start working in view %" PRId64 "", curView);
		...
		void ReplicaImp::sendCheckpointIfNeeded()
		{
			...
			{
				Logger::printInfo("My Checkpoint message is missing"); // TODO(GG): TBD
				return;
			}

			//Logger::printInfo("Debug - sendCheckpointIfNeeded - 1");

			if (checkInfo.checkpointSentAllOrApproved()) return;

			//Logger::printInfo("Debug - sendCheckpointIfNeeded - 2");
			...
			//Logger::printInfo("Debug - sendCheckpointIfNeeded - 3");
			...
			//Logger::printInfo("Debug - sendCheckpointIfNeeded - 4");
			...
			if (mainLog->insideActiveWindow(lastExecutedSeqNum)) // TODO(GG): condition is needed ?
			{
				...
				//Logger::printInfo("Debug - sendCheckpointIfNeeded - 5");


				if (seqNumInfo.partialProofs().hasFullProof())
				{

					//Logger::printInfo("Debug - sendCheckpointIfNeeded - 6");
			...
			//Logger::printInfo("Debug - sendCheckpointIfNeeded - 7");
...
//				const SeqNum prevLastExecutedSeqNum = lastExecutedSeqNum;
			...
			{
				Logger::printError("Debug Warning: executing onTransferringCompleteImp(newStateCheckpoint) where newStateCheckpoint <= lastExecutedSeqNum");
			}
		...
		void ReplicaImp::onSeqNumIsStableWithoutRefCheckpoint(SeqNum newStableSeqNum)
		{
			...
			Logger::printInfo("onSeqNumIsStableWithoutRefCheckpoint: lastStableSeqNum is now == %" PRId64 "", newStableSeqNum);
		...
		void ReplicaImp::onSeqNumIsStable(SeqNum newStableSeqNum)
		{
			...
			Logger::printInfo("onSeqNumIsStable: lastStableSeqNum is now == %" PRId64 "", newStableSeqNum);
		...
		void ReplicaImp::tryToSendReqMissingDataMsg(SeqNum seqNumber, bool slowPathOnly, uint16_t destReplicaId)
		{
			...
			Logger::printInfo("Node %d trys to request missing data for seqNumber %" PRId64 "", (int)myReplicaId, seqNumber);
			...
			for (ReplicaId destRep = firstRepId; destRep <= lastRepId; destRep++)
			{
				...
				Logger::printInfo("Node %d sends ReqMissingDataMsg to %d - seqNumber %" PRId64 " , flags=%X",
		...
		void ReplicaImp::onMessage(ReqMissingDataMsg* msg)
		{
			...
			Logger::printInfo("Node %d received ReqMissingDataMsg message from Node %d - seqNumber %" PRId64 " , flags=%X",
			...
			{
				Logger::printInfo("Node %d ignores the ReqMissingDataMsg message from Node %d", (int)myReplicaId, (int)msgSender);
			}
		...
		void ReplicaImp::onViewsChangeTimer(Time cTime, Timer& timer) // TODO(GG): review/update logic
		{
			...
			if (autoPrimaryUpdateEnabled && currentViewIsActive())
			{
				...
				if (diffMilli > timeout) {
					...
					Logger::printInfo("**************** Node %d initiates automatic view change in view %" PRId64 " (%" PRIu64  " milli seconds after start working in the previous view)", myReplicaId, curView, diffMilli);
			...
			if (currentViewIsActive())
			{
				...
				{
					Logger::printInfo("**************** Node %d asks to leave view %" PRId64 " (%" PRIu64  " milli seconds after receiving a client request)", myReplicaId, curView, diffMilli3);
			...
			else // not currentViewIsActive()
			{
				...
				if ((diffMilli1 > viewChangeTimeout) &&
					(diffMilli2 > viewChangeTimeout))
				{
					Logger::printInfo("**************** Node %d asks to jump to view %" PRId64 " (%" PRIu64  " milli seconds after receiving 2f+2c+1 view change msgs for view %" PRId64 ")", myReplicaId, curView, diffMilli2);
		...
		void ReplicaImp::onMessage(SimpleAckMsg* msg)
		{
			if (retransmissionsLogicEnabled)
			{
				...
				Logger::printInfo("Node %d received SimpleAckMsg message from node %d for seqNumber %" PRId64 " and type %d", myReplicaId, msg->senderId(), msg->seqNumber(), (int)relatedMsgType);
...
//			freeAllocator();
		...
		void ReplicaImp::processMessages()
		{
			...
			while (!mainThreadShouldStop)
			{
				...
				{
					Logger::printWarn("Unknown message");
					delete m;
				}
			}
		}
		...
		void ReplicaImp::executeReadWriteRequests(const bool requestMissingInfo) 
		{
			...
			while (lastExecutedSeqNum < lastStableSeqNum + kWorkWindowSize)
			{
				...
				{
					Logger::printInfo("executeReadWriteRequests - Asking for missing information about %" PRId64 "", lastExecutedSeqNum + 1);
