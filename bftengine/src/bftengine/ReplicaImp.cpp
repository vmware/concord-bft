...
std::unordered_map<uint16_t, PtrToMetaMsgHandler>
ReplicaImp::createMapOfMetaMsgHandlers() {
  std::unordered_map<uint16_t, PtrToMetaMsgHandler> r;

  r[MsgCode::Checkpoint] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<CheckpointMsg>;
  r[MsgCode::CommitPartial] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<
          CommitPartialMsg>;
  r[MsgCode::CommitFull] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<CommitFullMsg>;
  r[MsgCode::FullCommitProof] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<
          FullCommitProofMsg>;
  r[MsgCode::NewView] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<NewViewMsg>;
  r[MsgCode::PrePrepare] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<PrePrepareMsg>;
  r[MsgCode::PartialCommitProof] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<
          PartialCommitProofMsg>;
  r[MsgCode::PartialExecProof] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<
          PartialExecProofMsg>;
  r[MsgCode::PreparePartial] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<
          PreparePartialMsg>;
  r[MsgCode::PrepareFull] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<PrepareFullMsg>;
  r[MsgCode::ReqMissingData] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<
          ReqMissingDataMsg>;
  r[MsgCode::SimpleAckMsg] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<SimpleAckMsg>;
  r[MsgCode::StartSlowCommit] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<
          StartSlowCommitMsg>;
  r[MsgCode::ViewChange] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<ViewChangeMsg>;
  r[MsgCode::Request] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<
          ClientRequestMsg>;
  r[MsgCode::ReplicaStatus] =
      &ReplicaImp::metaMessageHandler_IgnoreWhenCollectingState<
          ReplicaStatusMsg>;
...
void ReplicaImp::send(MessageBase* m, NodeIdType dest) {
  ...
  if (m->type() == MsgCode::Checkpoint) {
    ...
    if (c->digestOfState().isZero())
      LOG_WARN_F(GL, "Debug: checkpoint digest is zero");
  }
...
void ReplicaImp::sendRaw(char* m,
  ...
  if (dest == ALL_OTHER_REPLICAS) {
    for (ReplicaId d : repsInfo->idsOfPeerReplicas()) sendRaw(m, d, type, size);
    return;
  }
...
void ReplicaImp::recvMsg(void*& item, bool& external) {
  ...
  while (!newMsg) {
    ...
    if (!newMsg)
      timersScheduler
          .evaluate();  // TODO(GG): make sure that we don't check timers too
...
void ReplicaImp::MsgReceiver::onNewMessage(const NodeNum sourceNode,
  ...
  MessageBase::Header* msgBody =
      (MessageBase::Header*)std::malloc(messageLength);
  memcpy(msgBody, message, messageLength);

  NodeIdType n =
      (uint16_t)sourceNode;  // TODO(GG): make sure that this casting is okay
...
void ReplicaImp::MsgReceiver::onConnectionStatusChanged(
...
void ReplicaImp::onMessage(ClientRequestMsg* m) {
  ...
  if (stateTransfer->isCollectingState()) {
    LOG_INFO_F(GL,
               "ClientRequestMsg is ignored becuase this replica is collecting "
               "missing state from the other replicas");
    delete m;
    return;
  }
  ...
  const bool sentFromReplicaToNonPrimary =
      repsInfo->isIdOfReplica(senderId) && !isCurrentPrimary();

  if (invalidClient ||
      sentFromReplicaToNonPrimary)  // TODO(GG): more conditions (make sure that
                                    // a request of client A cannot be generated
                                    // by client B!=A)
  {
  ...
  if (!currentViewIsActive()) {
    LOG_INFO_F(GL,
               "ClientRequestMsg is ignored becuase current view is inactive");
    delete m;
    return;
  }

  const ReqId seqNumberOfLastReply =
      clientsManager->seqNumberOfLastReplyToClient(clientId);

  if (seqNumberOfLastReply < reqSeqNum) {
    if (isCurrentPrimary()) {
      if (clientsManager->noPendingAndRequestCanBecomePending(clientId,
                                                              reqSeqNum) &&
          (requestsQueueOfPrimary.size() <
           700))  // TODO(GG): use config/parameter
      {
        ...
        LOG_INFO_F(GL,
                   "ClientRequestMsg is ignored becuase: request is old, OR "
                   "primary is current working on a request from the same "
                   "client, OR queue contains too many requests");
      }
    } else  // not the current primary
    {
      if (clientsManager->noPendingAndRequestCanBecomePending(clientId,
        ...
        send(m, currentPrimary());  // TODO(GG): add a mechanism that
                                    // retransmits (otherwise we may start
                                    // unnecessary view-change )

        LOG_INFO_F(GL, "Sending ClientRequestMsg to current primary");
      } else {
        LOG_INFO_F(GL,
                   "ClientRequestMsg is ignored becuase request is old or "
                   "replica has another pending request from the same client");
      }
    }
  } else if (seqNumberOfLastReply == reqSeqNum) {
    LOG_INFO_F(GL,
               "ClientRequestMsg has already been executed - retransmit reply "
               "to client");

    ClientReplyMsg* repMsg =
...
void ReplicaImp::tryToSendPrePrepareMsg(bool batchingLogic) {
  ...
  if (primaryLastUsedSeqNum + 1 >
      lastExecutedSeqNum + maxConcurrentAgreementsByPrimary)
    return;  // TODO(GG): should also be checked by the non-primary replicas

  if (requestsQueueOfPrimary.empty()) return;

  // remove irrelevant requests from the head of the requestsQueueOfPrimary (and
  // update requestsInQueue)
  ClientRequestMsg* first = requestsQueueOfPrimary.front();
  while (first != nullptr &&
         !clientsManager->noPendingAndRequestCanBecomePending(
             first->clientProxyId(), first->requestSeqNum())) {
    delete first;
    requestsQueueOfPrimary.pop();
    first = (!requestsQueueOfPrimary.empty() ? requestsQueueOfPrimary.front()
                                             : nullptr);
  }
  ...
  if (batchingLogic && (concurrentDiff >= 2)) {
    ...
    const size_t maxReasonableMinBatchSize =
        350;  // TODO(GG): use param from configuration

    if (minBatchSize > maxReasonableMinBatchSize)
      minBatchSize = maxReasonableMinBatchSize;
  }
  ...
  if ((primaryLastUsedSeqNum % kWorkWindowSize) ==
      0)  // TODO(GG): do we want to update batchingFactor when the view is
          // changed
  {
  ...
  Assert(
      primaryLastUsedSeqNum <=
      lastExecutedSeqNum +
          MaxConcurrentFastPaths);  // because maxConcurrentAgreementsByPrimary
                                    // <  MaxConcurrentFastPaths
  ...
  Assert(
      (cVal != 0) ||
      (firstPath !=
       CommitPath::FAST_WITH_THRESHOLD));  // assert: (cVal==0) --> (firstPath
                                           // !=
                                           // CommitPath::FAST_WITH_THRESHOLD)

  controller->onSendingPrePrepare(primaryLastUsedSeqNum, firstPath);

  PrePrepareMsg* pp = new PrePrepareMsg(
      myReplicaId, curView, primaryLastUsedSeqNum, firstPath, false);
  ...
  while (nextRequest != nullptr &&
    ...
    requestsQueueOfPrimary.pop();
    nextRequest =
        (requestsQueueOfPrimary.size() > 0 ? requestsQueueOfPrimary.front()
                                           : nullptr);
  }
...
bool ReplicaImp::relevantMsgForActiveView(const T* msg) {
  ...
  if (currentViewIsActive() && (msgViewNum == curView) &&
...
void ReplicaImp::onMessage(PrePrepareMsg* msg) {
  ...
  if (!currentViewIsActive() && viewsManager->waitingForMsgs() &&
    ...
    Assert(!msg->isNull());  // we should never send (and never accept) null
                             // PrePrepare message
  ...
  if (relevantMsgForActiveView(msg) && (msg->senderId() == currentPrimary())) {
    ...
    if (seqNumInfo.addMsg(msg)) {
      ...
      if (msg->firstPath() != CommitPath::SLOW &&
          !seqNumInfo
               .slowPathStarted())  // TODO(GG): make sure we correctly handle a
                                    // situation where StartSlowCommitMsg is
                                    // handled before PrePrepareMsg
      {
        sendPartialProof(seqNumInfo);
      } else {
        seqNumInfo.startSlowPath();
...
void ReplicaImp::tryToStartSlowPaths() {
  if (!isCurrentPrimary() || stateTransfer->isCollectingState() ||
      !currentViewIsActive())
    return;  // TODO(GG): consider to stop the related timer when this method is
             // not needed (to avoid useless invocations)

  const SeqNum minSeqNum = lastExecutedSeqNum + 1;

  if (minSeqNum > lastStableSeqNum + kWorkWindowSize) {
    LOG_INFO_F(GL,
               "Replica::tryToStartSlowPaths() : minSeqNum > lastStableSeqNum "
               "+ kWorkWindowSize");
    return;
  }
  ...
  for (SeqNum i = minSeqNum; i <= maxSeqNum; i++) {
    ...
    if (seqNumInfo.partialProofs()
            .hasFullProof() ||           // already has a full proof
        seqNumInfo.slowPathStarted() ||  // slow path has already  started
        seqNumInfo.partialProofs().getSelfPartialCommitProof() ==
            nullptr ||  // did not start a fast path
        (!seqNumInfo.hasPrePrepareMsg()))
      continue;  // slow path is not needed

    const Time timeOfPartProof =
        seqNumInfo.partialProofs().getTimeOfSelfPartialProof();

    if (absDifference(currTime, timeOfPartProof) <
        controller->timeToStartSlowPathMilli() * 1000)
    ...
    StartSlowCommitMsg* startSlow =
        new StartSlowCommitMsg(myReplicaId, curView, i);
...
void ReplicaImp::tryToAskForMissingInfo() {
  ...
  Assert(maxSeqNumTransferredFromPrevViews <=
         lastStableSeqNum + kWorkWindowSize);

  const bool recentViewChange =
  ...
  if (!recentViewChange) {
    ...
    maxSeqNum = std::min(minSeqNum + searchWindow - 1,
                         lastStableSeqNum + kWorkWindowSize);
  } else {
  ...
  for (SeqNum i = minSeqNum; i <= maxSeqNum; i++) {
    ...
    if (t != MinTime && (t < curTime)) {
      ...
      if (diffMilli >= dynamicUpperLimitOfRounds->upperLimit())
        lastRelatedSeqNum = i;
    }
  }
...
void ReplicaImp::sendPartialProof(SeqNumInfo& seqNumInfo) {
  ...
  if (!partialProofs.hasFullProof()) {
    ...
    // send PartialCommitProofMsg (only if, from my point of view, at most
    // MaxConcurrentFastPaths are in progress)
    if (seqNum <= lastExecutedSeqNum + MaxConcurrentFastPaths) {
      // TODO(GG): improve the following code (use iterators instead of a simple
      // array)
...
void ReplicaImp::sendPreparePartial(SeqNumInfo& seqNumInfo) {
  ...
  if (seqNumInfo.getSelfPreparePartialMsg() == nullptr &&
      seqNumInfo.hasPrePrepareMsg() && !seqNumInfo.isPrepared()) {
    PrePrepareMsg* pp = seqNumInfo.getPrePrepareMsg();
...
void ReplicaImp::sendCommitPartial(const SeqNum s) {
  ...
  CommitPartialMsg* c = CommitPartialMsg::create(
      curView, s, myReplicaId, d, thresholdSignerForSlowPathCommit);
  seqNumInfo.addSelfCommitPartialMsgAndDigest(c, d);

  if (!isCurrentPrimary())
    sendRetransmittableMsgToReplica(c, currentPrimary(), s);
}
...
void ReplicaImp::onMessage(PartialCommitProofMsg* msg) {
  ...
  if (relevantMsgForActiveView(msg)) {
    ...
    if (msgSeqNum > lastExecutedSeqNum) {
      ...
      if (pps.addMsg(msg)) {
        // GG: The following block is not needed (pps.hasFullProof() will always
        // be false) because we create the combined signature in another thread.
        // TODO(GG): verify
        if (pps.hasFullProof()) {
...
void ReplicaImp::onMessage(FullCommitProofMsg* msg) {
  ...
  if (relevantMsgForActiveView(msg)) {
    ...
    if (!pps.hasFullProof() &&
        pps.addMsg(msg))  // TODO(GG): consider to verify the signature in
                          // another thread
    {
      Assert(seqNumInfo.hasPrePrepareMsg());
...
void ReplicaImp::onMessage(CommitFullMsg* msg) {
  ...
  if (relevantMsgForActiveView(msg)) {
    ...
    if (fcp != nullptr) {
      send(fcp, msgSender);  // TODO(GG): do we really want to send this message
                             // ? (msgSender already has a CommitFullMsg for the
                             // same seq number)
    } else if (commitFull != nullptr) {
      // nop
    } else {
...
void ReplicaImp::onPrepareCombinedSigSucceeded(SeqNum seqNumber,
  ...
  if (fcp != nullptr)
    return;  // don't send if we already have FullCommitProofMsg
...
void ReplicaImp::onCommitCombinedSigSucceeded(SeqNum seqNumber,
  ...
  bool askForMissingInfoAboutCommittedItems =
      (seqNumber > lastExecutedSeqNum + maxConcurrentAgreementsByPrimary);
...
void ReplicaImp::onMessage(CheckpointMsg* msg) {
  ...
  if (msgIsStable && msgSeqNum > lastExecutedSeqNum) {
    ...
    if (pos == tableOfStableCheckpoints.end() ||
      ...
      CheckpointMsg* x =
          new CheckpointMsg(msgSenderId, msgSeqNum, msgDigest, msgIsStable);
      ...
      if ((uint16_t)tableOfStableCheckpoints.size() >= fVal + 1) {
        ...
        while (tableItrator != tableOfStableCheckpoints.end()) {
          if (tableItrator->second->seqNumber() <= lastExecutedSeqNum) {
            ...
            numRelevant++;
            if (tableItrator->second->seqNumber() >
                lastStableSeqNum + kWorkWindowSize)
        ...
        if (numRelevantAboveWindow >= fVal + 1) {
          ...
          if (mainLog->insideActiveWindow(lastExecutedSeqNum))
            timeOfLastCommit =
                mainLog->get(lastExecutedSeqNum).lastUpdateTimeOfCommitMsgs();
...
bool ReplicaImp::handledByRetransmissionsManager(const ReplicaId sourceReplica,
  ...
  if (sourcePrimary && ((msgType == MsgCode::PrePrepare) ||
                        (msgType == MsgCode::StartSlowCommit)))
    return true;

  const bool dstPrimary = (destReplica == primaryReplica);

  if (dstPrimary && ((msgType == MsgCode::PreparePartial) ||
                     (msgType == MsgCode::CommitPartial)))
    return true;

  //  TODO(GG): do we want to use acks for FullCommitProofMsg ?

  if (msgType == MsgCode::PartialCommitProof) {
    const bool destIsCollector = repsInfo->getCollectorsForPartialProofs(
        destReplica, curView, seqNum, nullptr, nullptr);
...
void ReplicaImp::onRetransmissionsProcessingResults(
  ...
  if (stateTransfer->isCollectingState() || (relatedViewNumber != curView) ||
      (!currentViewIsActive()))
    return;
  ...
  for (const RetSuggestion& s : *suggestedRetransmissions) {
    if ((s.msgSeqNum <= lastStableSeqNum) ||
        (s.msgSeqNum > lastStableSeqNum + kWorkWindowSize))
      continue;
    ...
    switch (s.msgType) {
      ...
      case MsgCode::PartialCommitProof: {
        SeqNumInfo& seqNumInfo = mainLog->get(s.msgSeqNum);
        PartialCommitProofMsg* msgToSend =
            seqNumInfo.partialProofs().getSelfPartialCommitProof();
        Assert(msgToSend != nullptr);
        sendRetransmittableMsgToReplica(msgToSend, s.replicaId, s.msgSeqNum);
        LOG_INFO_F(GL,
      ...
      case MsgCode::StartSlowCommit: {
        StartSlowCommitMsg* msgToSend =
            new StartSlowCommitMsg(myId, curView, s.msgSeqNum);
        sendRetransmittableMsgToReplica(msgToSend, s.replicaId, s.msgSeqNum);
        delete msgToSend;
        LOG_INFO_F(GL,
...
void ReplicaImp::onMessage(ReplicaStatusMsg* msg) {
  // TODO(GG): we need filter for msgs (to avoid denial of service attack) +
  // avoid sending messages at a high rate.
  // TODO(GG): for some communication modules/protocols, we can also utilize
  ...
  if (lastStableSeqNum > msgLastStable + kWorkWindowSize) {
    CheckpointMsg* checkMsg =
        checkpointsLog->get(lastStableSeqNum).selfCheckpointMsg();
    ...
    const SeqNum beginRange =
        std::max(checkpointsLog->currentActiveWindow().first,
                 msgLastStable + checkpointWindowSize);
    const SeqNum endRange =
        std::min(checkpointsLog->currentActiveWindow().second,
                 msgLastStable + kWorkWindowSize);
  ...
  /////////////////////////////////////////////////////////////////////////
    ...
    if (isCurrentPrimary() || (repsInfo->primaryOfView(curView) ==
                               msgSenderId))  // if the primary is involved
    {
      ...
      {
        // send NewViewMsg
        if (!msg->currentViewHasNewViewMessage() &&
            viewsManager->viewIsActive(curView)) {
        ...
        // TODO(GG): send all VC msgs that can help making progress (needed
        // because the original senders may not send the ViewChangeMsg msgs used
        // by the primary)

        // TODO(GG): if viewsManager->viewIsActive(curView), we can send only
      ...
      } else  // if I am also not in curView --- In this case we take messages
              // from viewsManager
      {
        if (msg->hasListOfMissingPrePrepareMsgForViewChange()) {
          for (SeqNum i = msgLastStable + 1;
            ...
            if (msg->isMissingPrePrepareMsgForViewChange(i)) {
              PrePrepareMsg* prePrepareMsg = viewsManager->getPrePrepare(
                  i);  // TODO(GG): we can avoid sending misleading message by
                       // using the digest of the expected pre prepare message
              if (prePrepareMsg != nullptr) send(prePrepareMsg, msgSenderId);
            }
          }
        }
        ...
        SeqNum beginRange =
            std::max(lastStableSeqNum + 1,
                     msg->getLastExecutedSeqNum() +
                         1);  // Notice that after a view change, we don't have
                              // to pass the PrePrepare messages from the
                              // previous view. TODO(GG): verify
        SeqNum endRange = std::min(lastStableSeqNum + kWorkWindowSize,
                                   msgLastStable + kWorkWindowSize);
...
void ReplicaImp::tryToSendStatusReport() {
  // TODO(GG): in some cases, we can limit the amount of such messages by using
  // information about connection/disconnection (from the communication module)
  // TODO(GG): explain that the current "Status Report" approch is relatively
  // simple (it can be more efficient and sophisticated).

  const Time currentTime = getMonotonicTime();

  const int64_t milliSinceLastTime = (int64_t)(
      absDifference(currentTime,
  ...
  const int64_t dynamicMinTimeBetweenStatusRequestsMilli =
      (int64_t)((double)dynamicUpperLimitOfRounds->upperLimit() *
                factorForMinTimeBetweenStatusRequestsMilli);

  if (milliSinceLastTime < dynamicMinTimeBetweenStatusRequestsMilli) return;

  lastTimeThisReplicaSentStatusReportMsgToAllPeerReplicas =
      currentTime;  // TODO(GG): handle restart/pause !! (restart/pause may
                    // affect time measurements...)
...
void ReplicaImp::onMessage(ViewChangeMsg* msg) {
  ...
  const ReplicaId generatedReplicaId =
      msg->idOfGeneratedReplica();  // Notice that generatedReplicaId may be !=
                                    // msg->senderId()
  ...
  if (maxKnownAgreedView != curView)
    return;  // return, if we can't move to the new view yet

  // Replica now has at least 2f+2c+1 ViewChangeMsg messages with view  >=
  // curView
    ...
    for (SeqNum i = lastStableSeqNum + 1;
      ...
      if (seqNumInfo.getPrePrepareMsg() != nullptr) {
        ...
        Assert(x.prepareFull == nullptr ||
               x.hasAllRequests);  // (x.prepareFull!=nullptr) ==>
                                   // (x.hasAllRequests==true)
        Assert(x.prepareFull == nullptr ||
               x.prepareFull->viewNumber() ==
                   curView);  // (x.prepareFull!=nullptr) ==>
                              // (x.prepareFull->viewNumber() == curView)
...
void ReplicaImp::GotoNextView() {
  ...
  // at this point we don't have enough ViewChangeMsg messages (2f+2c+1) to
  // enter to the new view (because 2f+2c+1 > f+1)
}
...
void ReplicaImp::onNewView(
    const std::vector<PrePrepareMsg*>& prePreparesForNewView) {
  SeqNum firstPPSeq = 0;
  SeqNum lastPPSeq = 0;
  ...
  LOG_INFO_F(GL,
             "**************** In onNewView curView=%" PRId64
             " (num of PPs=%d, first safe seq=%" PRId64
  ...
  Assert(lastExecutedSeqNum >=
         lastStableSeqNum);  // we moved to the new state, only after
                             // synchronizing the state

  timeOfLastViewEntrance =
      getMonotonicTime();  // TODO(GG): handle restart/pause
  ...
  for (size_t i = 0; i < prePreparesForNewView.size(); i++) {
    ...
    Assert(pp->firstPath() ==
           CommitPath::SLOW);  // TODO(GG): don't we want to use the fast path?
...
void ReplicaImp::sendCheckpointIfNeeded() {
  ...
  const SeqNum lastCheckpointNumber =
      (lastExecutedSeqNum / checkpointWindowSize) * checkpointWindowSize;
  ...
  const SeqNum refSeqNumberForCheckpoint =
      lastCheckpointNumber + MaxConcurrentFastPaths;

  if (lastExecutedSeqNum < refSeqNumberForCheckpoint) return;

  // LOG_INFO_F(GL, "Debug - sendCheckpointIfNeeded - 4");

  if (mainLog->insideActiveWindow(
          lastExecutedSeqNum))  // TODO(GG): condition is needed ?
  {
...
void ReplicaImp::onTransferringCompleteImp(SeqNum newStateCheckpoint) {
  ...
  LOG_INFO_F(GL,
             "onTransferringCompleteImp with newStateCheckpoint==%" PRId64 "",
             newStateCheckpoint);
  ...
  if (newStateCheckpoint > lastExecutedSeqNum) {
    ...
    if (newStateCheckpoint > lastStableSeqNum + kWorkWindowSize) {
      ...
      const bool withRefCheckpoint =
          (checkpointsLog->insideActiveWindow(refPoint) &&
    ...
    LOG_ERROR_F(GL,
                "Debug Warning: executing "
                "onTransferringCompleteImp(newStateCheckpoint) where "
                "newStateCheckpoint <= lastExecutedSeqNum");
  }
...
void ReplicaImp::onSeqNumIsStableWithoutRefCheckpoint(SeqNum newStableSeqNum) {
  ...
  LOG_INFO_F(GL,
             "onSeqNumIsStableWithoutRefCheckpoint: lastStableSeqNum is now == "
             "%" PRId64 "",
             newStableSeqNum);
...
void ReplicaImp::onSeqNumIsStable(SeqNum newStableSeqNum) {
  ...
  if (currentViewIsActive() &&
      !stateTransfer->isCollectingState())  // TODO(GG): TBD
  {
    if (currentPrimary() == myReplicaId) tryToSendPrePrepareMsg();
  }
}
...
void ReplicaImp::tryToSendReqMissingDataMsg(SeqNum seqNumber,
...

  {
    ...
    if (t == MinTime && (t < curTime)) {
      ...
      if (diffMilli <
          dynamicUpperLimitOfRounds->upperLimit() / 4)  // TODO(GG): config
        return;
    }
  }
  ...
  const bool routerForPartialProofs =
      repsInfo->isCollectorForPartialProofs(curView, seqNumber);
  ...
  const bool missingBigRequests =
      (!missingPrePrepare) && (!seqNumInfo.hasPrePrepareMsg());
  ...
  for (ReplicaId destRep = firstRepId; destRep <= lastRepId; destRep++) {
    ...
    const bool missingPartialPrepare =
        (routerForPartialPrepare &&
         (!seqNumInfo.preparedOrHasPreparePartialFromReplica(destRep)));

    const bool missingFullPrepare =
        !seqNumInfo.preparedOrHasPreparePartialFromReplica(destRep);

    const bool missingPartialCommit =
        (routerForPartialCommit &&
         (!seqNumInfo.committedOrHasCommitPartialFromReplica(destRep)));

    const bool missingFullCommit =
        !seqNumInfo.committedOrHasCommitPartialFromReplica(destRep);

    const bool missingPartialProof =
        !slowPathOnly && routerForPartialProofs &&
        !partialProofsSet.hasFullProof() &&
        !partialProofsSet.hasPartialProofFromReplica(destRep);

    const bool missingFullProof =
        !slowPathOnly && !partialProofsSet.hasFullProof();

    bool sendNeeded = missingPartialProof || missingPartialPrepare ||
                      missingFullPrepare || missingPartialCommit ||
                      missingFullCommit || missingFullProof;

    if (destIsPrimary && !sendNeeded)
      sendNeeded = missingBigRequests || missingPrePrepare;
...
void ReplicaImp::onMessage(ReqMissingDataMsg* msg) {
  ...
  if ((currentViewIsActive()) && (msgSeqNum > strictLowerBoundOfSeqNums) &&
    ...
    if (msg->getFullCommitProofIsMissing() &&
        seqNumInfo.partialProofs().hasFullProof()) {
      FullCommitProofMsg* fcp = seqNumInfo.partialProofs().getFullProof();
      send(fcp, msgSender);
    }
  } else {
...
void ReplicaImp::onViewsChangeTimer(
    Time cTime, Timer& timer)  // TODO(GG): review/update logic
{
  Assert(viewChangeProtocolEnabled);
  ...
  if (autoPrimaryUpdateEnabled && currentViewIsActive()) {
    const uint64_t timeout =
        (isCurrentPrimary()
             ? (autoPrimaryUpdateMilli)
             : (autoPrimaryUpdateMilli + viewChangeTimeoutMilli));

    const uint64_t diffMilli =
        absDifference(currTime, timeOfLastViewEntrance) / 1000;
  ...
  if (autoIncViewChangeTimer &&
      ((lastViewThatTransferredSeqNumbersFullyExecuted + 1) < curView)) {
    uint64_t factor =
        (curView - lastViewThatTransferredSeqNumbersFullyExecuted);
    viewChangeTimeout =
        viewChangeTimeout * factor;  // TODO(GG): review logic here
  }

  if (currentViewIsActive()) {
    if (isCurrentPrimary()) return;

    const Time timeOfEarliestPendingRequest =
        clientsManager->timeOfEarliestPendingRequest();

    const bool hasPendingRequest = (timeOfEarliestPendingRequest != MaxTime);

    if (!hasPendingRequest) return;

    const uint64_t diffMilli1 =
        absDifference(currTime, timeOfLastStateSynch) / 1000;
    const uint64_t diffMilli2 =
        absDifference(currTime, timeOfLastViewEntrance) / 1000;
    const uint64_t diffMilli3 =
        absDifference(currTime, timeOfEarliestPendingRequest) / 1000;
  ...
  } else  // not currentViewIsActive()
  {
    ...
    const uint64_t diffMilli1 =
        absDifference(currTime, timeOfLastStateSynch) / 1000;
    const uint64_t diffMilli2 =
        absDifference(currTime, timeOfLastAgreedView) / 1000;

    if ((diffMilli1 > viewChangeTimeout) && (diffMilli2 > viewChangeTimeout)) {
      LOG_INFO_F(GL,
                 "**************** Node %d asks to jump to view %" PRId64
                 " (%" PRIu64
...
void ReplicaImp::commitFullCommitProof(SeqNum seqNum, SeqNumInfo& seqNumInfo) {
  ...
  const bool askForMissingInfoAboutCommittedItems =
      (seqNum >
       lastExecutedSeqNum +
           maxConcurrentAgreementsByPrimary);  // TODO(GG): check this logic

  executeReadWriteRequests(askForMissingInfoAboutCommittedItems);
}

void ReplicaImp::commitAndSendFullCommitProof(SeqNum seqNum,
...
void ReplicaImp::onMessage(SimpleAckMsg* msg) {
  if (retransmissionsLogicEnabled) {
    uint16_t relatedMsgType =
        (uint16_t)msg->ackData();  // TODO(GG): does this make sense ?
...
void ReplicaImp::changeStateTransferTimerPeriod(uint32_t timerPeriodMilli) {
  ...
  if (mainThread.get_id() == std::this_thread::get_id()) {
    stateTranTimer->changePeriodMilli(
        (uint16_t)timerPeriodMilli);  // TODO: check the casting here
  } else {
    // TODO(GG): implement
    Assert(false);
  }
...
ReplicaImp::ReplicaImp(const ReplicaConfig& config,
    ...
    : myReplicaId{config.replicaId},
      ...
      replyBuffer{(char*)std::malloc(maxReplyMessageSize -
                                     sizeof(ClientReplyMsgHeader))},
      stateTransfer{(stateTransferr != nullptr ? stateTransferr
                                               : new NullStateTransfer())},
      maxNumberOfPendingRequestsInRecentHistory{0},
      batchingFactor{1},
  ...
  for (uint16_t i = numOfReplicas; i < numOfReplicas + numOfClientProxies; i++)
    clientsSet.insert(i);

  clientsManager =
      new ClientsManager(myReplicaId, clientsSet, sizeOfReservedPage);
  ...
  int statusReportTimerMilli = (sendStatusPeriodMilli > 0)
                                   ? sendStatusPeriodMilli
                                   : config.statusReportTimerMillisec;
  ;
  Assert(statusReportTimerMilli > 0);

  viewChangeTimerMilli = (viewChangeTimeoutMilli > 0)
                             ? viewChangeTimeoutMilli
                             : config.viewChangeTimerMillisec;
  ...
  if (viewChangeProtocolEnabled) {
    ...
    if (autoPrimaryUpdateEnabled && t > autoPrimaryUpdateMilli)
  ...
  if (retransmissionsLogicEnabled)
    retransmissionsManager = new RetransmissionsManager(
        this, &internalThreadPool, &incomingMsgsStorage, kWorkWindowSize, 0);
  else
    retransmissionsManager = nullptr;
...
ReplicaImp::~ReplicaImp() {
  // TODO(GG): rewrite this method !!!!!!!! (notice that the order may be
  // important here ).
...
void ReplicaImp::executeReadOnlyRequest(ClientRequestMsg* request) {
  ...
  // TODO(GG): TBD - how do we want to support empty replies?
  // (actualReplyLength==0)
...
void ReplicaImp::executeRequestsInPrePrepareMsg(PrePrepareMsg* ppMsg) {
  ...
  while (reqIter.getAndGoToNext(requestBody)) {
    ...
    if (clientsManager->seqNumberOfLastReplyToClient(clientId) >=
        req.requestSeqNum()) {
      ClientReplyMsg* replyMsg = clientsManager->allocateMsgWithLatestReply(
          clientId, currentPrimary());
    ...
    Assert(actualReplyLength > 0);  // TODO(GG): TBD - how do we want to support
                                    // empty replies? (actualReplyLength==0)
  ...
  if ((lastExecutedSeqNum + 1) % checkpointWindowSize == 0) {
    const uint64_t checkpointNum =
        (lastExecutedSeqNum + 1) / checkpointWindowSize;
  ...
  LOG_INFO_F(GL,
             "\nReplica - executeRequestsInPrePrepareMsg() - "
             "lastExecutedSeqNum==%" PRId64 "",
             lastExecutedSeqNum);
...
void ReplicaImp::executeReadWriteRequests(const bool requestMissingInfo) {
  ...
  while (lastExecutedSeqNum < lastStableSeqNum + kWorkWindowSize) {
    ...
    const bool ready =
        (prePrepareMsg != nullptr) && (seqNumInfo.isCommitted__gg());

    if (requestMissingInfo && !ready) {
      LOG_INFO_F(GL,
                 "executeReadWriteRequests - Asking for missing information "
                 "about %" PRId64 "",
