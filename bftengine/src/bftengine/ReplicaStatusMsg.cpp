...
// MaxNumberOfReplicas-1 (should be changed to support dynamic reconfiguration)

MsgSize ReplicaStatusMsg::calcSizeOfReplicaStatusMsg(
...
ReplicaStatusMsg::ReplicaStatusMsg(ReplicaId senderId,
  ...
  Assert(!viewIsActive ||
         !listOfMissingVCForVC);  // viewIsActive --> !listOfMissingVCForVC
  Assert(!viewIsActive ||
         !listOfMissingPPForVC);  // viewIsActive --> !listOfMissingPPForVC
  Assert(viewIsActive ||
         !listOfPPInActiveWindow);  // !viewIsActive --> !listOfPPInActiveWindow
  Assert((listOfPPInActiveWindow ? 1 : 0) + (listOfMissingVCForVC ? 1 : 0) +
             (listOfMissingPPForVC ? 1 : 0) <=
         1);
  ...
  if (size() > sizeof(ReplicaStatusMsg::ReplicaStatusMsgHeader)) {
    ...
    MsgSize listSize =
        size() - sizeof(ReplicaStatusMsg::ReplicaStatusMsgHeader);
    char* p = body() + sizeof(ReplicaStatusMsg::ReplicaStatusMsgHeader);
    memset(p, 0, listSize);
  }
}

bool ReplicaStatusMsg::ToActualMsgType(const ReplicasInfo& repInfo,
                                       MessageBase* inMsg,
  ...
  const bool listOfPPInActiveWindow =
      t->hasListOfPrePrepareMsgsInActiveWindow();
  const bool listOfMissingVCForVC =
      t->hasListOfMissingViewChangeMsgForViewChange();
  const bool listOfMissingPPForVC =
      t->hasListOfMissingPrePrepareMsgForViewChange();

  if (!(!viewIsActive ||
        hasNewChangeMsg))  // if NOT (viewIsActive --> hasNewChangeMsg)
    return false;

  if (!(!viewIsActive || !listOfMissingVCForVC))  // if NOT (viewIsActive -->
                                                  // !listOfMissingVCForVC)
    return false;

  if (!(!viewIsActive || !listOfMissingPPForVC))  // if NOT (viewIsActive -->
                                                  // !listOfMissingPPForVC)
    return false;

  if (!((viewIsActive ||
         !listOfPPInActiveWindow)))  // if NOT (!viewIsActive -->
                                     // !listOfPPInActiveWindow)
    return false;

  if (((listOfPPInActiveWindow ? 1 : 0) + (listOfMissingVCForVC ? 1 : 0) +
       (listOfMissingPPForVC ? 1 : 0)) >= 2)
    return false;

  if (t->size() != calcSizeOfReplicaStatusMsg(listOfPPInActiveWindow,
                                              listOfMissingVCForVC,
                                              listOfMissingPPForVC))
    return false;
...
bool ReplicaStatusMsg::isPrePrepareInActiveWindow(SeqNum seqNum) const {
  ...
  uint8_t* p =
      (uint8_t*)body() + sizeof(ReplicaStatusMsg::ReplicaStatusMsgHeader);
  return ((p[byteIndex] & powersOf2[bitIndex]) != 0);
}

bool ReplicaStatusMsg::isMissingViewChangeMsgForViewChange(
...
bool ReplicaStatusMsg::isMissingPrePrepareMsgForViewChange(
    SeqNum seqNum) const {
...
void ReplicaStatusMsg::setMissingViewChangeMsgForViewChange(
    ReplicaId replicaId) {
  Assert(hasListOfMissingViewChangeMsgForViewChange());
  Assert(replicaId < MaxNumberOfReplicas);
  size_t index = replicaId;
