...
class SimpleClient {
 public:
  ...
  virtual int sendRequestToReadLatestSeqNum(uint64_t timeoutMilli,
                                            uint64_t& outLatestReqSeqNum) = 0;
};

class SeqNumberGeneratorForClientRequests  // users are allowed to generate
                                           // their own sequence numbers (they
                                           // do not have to use this class)
{
 public:
