...
#include "crypto/threshsign/bls/relic/BlsThresholdScheme.h"
...
class ThresholdBlsRelicBenchmark : public IThresholdSchemeBenchmark {
 private:
  ...
  std::vector<std::unique_ptr<IThresholdSigner>> sks;  // secret key shares for all signers
 ...
 protected:
  void generateKeys() {
    ...
    auto [signers, verifierTemp] = factory.newRandomSigners(reqSigners, numSigners);
    sks = std::move(signers);
    verifier = std::move(verifierTemp);
    ...
    sigShareBits = dynamic_cast<BlsThresholdSigner*>(sks[1].get())->requiredLengthForSignedData();
 ...
 public:
  ...
  virtual void signShare(ShareID i) {
    ...
    dynamic_cast<BlsThresholdSigner*>(sks[idx].get())
        ->signData(reinterpret_cast<char*>(msg), msgSize, shares[idx], sigShareBits);
  }
