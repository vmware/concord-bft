...
#include "crypto/threshsign/IPublicParameters.h"
#include "crypto/threshsign/VectorOfShares.h"
...
class ThresholdViabilityTest {
 protected:
  ...
  // Threshold signers
  std::vector<std::unique_ptr<IThresholdSigner>> sks;
 ...
 public:
  void generateKeys() {
    auto [signers, verifierTemp] = makeThresholdFactory()->newRandomSigners(reqSigners, numSigners);
    sks = std::move(signers);
    verif = std::move(verifierTemp);
  }
  ...
  ThresholdSigner* signer(ShareID i) { return dynamic_cast<ThresholdSigner*>(sks[static_cast<size_t>(i)].get()); }
