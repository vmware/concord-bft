...
void runBatchVerificationTest(int k, int n, int maxShares, int numBadShares = 0) {
  ...
  auto [signers, verifierTemp] = BlsThresholdFactory(PublicParametersFactory::getWhatever()).newRandomSigners(k, n);
  BlsThresholdVerifier* verifier = dynamic_cast<BlsThresholdVerifier*>(verifierTemp.get());
  ...
  for (ShareID id = allSubset.first(); allSubset.isEnd(id) == false; id = allSubset.next(id)) {
    ...
    G1T sig = dynamic_cast<BlsThresholdSigner*>(signers[static_cast<size_t>(id)].get())->signData(buf, msgLen);
