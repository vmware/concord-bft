...
class SkvbcPersistenceTest(unittest.TestCase):
    ...
    async def test_view_change_transitions_safely_without_quorum(self, bft_network, tracker):
        ...
        with trio.fail_after(60):  # seconds
            async with trio.open_nursery() as nursery:
                nursery.start_soon(
                    tracker.skvbc.send_indefinite_ops)
    ...
    async def test_read_written_data_after_restart_of_all_nodes(self, bft_network, tracker):
        ...
        (key, val) = await tracker.skvbc.send_write_kv_set()

        bft_network.stop_all_replicas()
        bft_network.start_all_replicas()

        kv_reply = await tracker.skvbc.send_read_kv_set(bft_network.random_client(), key)
    ...
    async def test_checkpoints_saved_and_transferred(self, bft_network, tracker):
        ...
        client, known_key, known_val = \
            await tracker.skvbc.prime_for_state_transfer(stale_nodes={stale_node})
        ...
        kvpairs = await tracker.skvbc.send_read_kv_set(client, known_key)
    ...
    async def _test_st_while_crashing_primary(
        ...
        client, known_key, known_val = \
            await tracker.skvbc.prime_for_state_transfer(stale_nodes={stale_replica},
                                                           checkpoints_num=2)
