...
class BftDebugTool:
    ...
    def __init__(self, num_replicas, builddir, current_test):
        ...
        name = None
        self.output_counters = {i: 0 for i in range(num_replicas)} # used to count output files per specific replica in the same test case
    ...
    def process_output(self, stopped_replica_id, stoppped_proc, testdir):
        ...
        '''
        output_log_format = "%s_skvbc_replica_pid_%d_repid_%d_#%d.%s" # tool name, process id, replica id, counter, postfix
        ...
        if self.name == "HEAPTRACK":
            ...
            heaptrack_proc.wait()
            heaptrack_src_log_name = "heaptrack.skvbc_replica." + str(heaptrack_proc.pid) + ".gz"
            heaptrack_dst_base = output_log_format % (self.name.lower(), heaptrack_proc.pid, stopped_replica_id, counter, "%s")
            heaptrack_dst_gz_path = os.path.join(self.output_log_path, heaptrack_dst_base % "gz")
            heaptrack_dst_log_path = os.path.join(self.output_log_path, heaptrack_dst_base % "log")
            ...
            # We would like to rename the file, so that it contains the replica id as well
            pid = stoppped_proc.pid
            sanitizer_src_file_path = os.path.join(self.output_log_path, self.output_log_prefix + "." + str(pid))
            assert Path(sanitizer_src_file_path).is_file(), f"File {sanitizer_src_file_path} must exist!"
            sanitizer_dst_file_path = os.path.join(self.output_log_path, \
                output_log_format % (self.name.lower(), pid, stopped_replica_id, counter, "log"))
            os.rename(sanitizer_src_file_path, sanitizer_dst_file_path)
