...
class WriteCounter(MonotonicSeqGenerator):
...
class ReqSeqNum(MonotonicSeqGenerator):
    """
    Time based sequence number generator
    """
    def __init__(self):
...
class BftClient(ABC):
    ...
    def __init__(self, config, replicas, ro_replicas=[], time_based_sequences=False):
        self.config = config
        self.replicas = replicas
        self.req_seq_num = ReqSeqNum() if time_based_sequences else BftClient.COUNTER
