...
std::pair<ShareID, G1T> BlsSigshareParser::operator()(const char* sigShare,
                                                      int len) {
  ...
  // Get the sigshare
  G1T sigShareNum(
      reinterpret_cast<const unsigned char*>(sigShare + sizeof(ShareID)),
...
bool BlsAccumulatorBase::verifyShare(ShareID id, const G1T& sigShare) {
  ...
  // logdbg << "Checking signature " << sigShare << " on hash " << hash << "
  // against VK of signer " << id << " " << vk << endl;
  pc_map(e1, hash, vk);
