...
void PrecomputedInverses::precompute() {
  ...
  BNT invOne =
      (fieldOrder - BNT(2)).invertModPrime(fieldOrder).SlowModulo(fieldOrder);

  logdbg << "Precomputing i^-1 mod p, for all signers i < " << invs.size()
         << std::endl;
  for (size_t i = 1; i <= invs.size() - 1; i++) {
    dig_t n = i;

    invs[i] = BNT::invertModPrime(n, fieldOrder);
    // NOTE: Lazy way of testing bn_gcd_ext_dig-based implementation. Commented
    // out to make startup faster.
...
LibraryInitializer::LibraryInitializer() {
  if (core_init() != STS_OK) {
    ...
    throw std::runtime_error(
        "Could not initialize RELIC elliptic curve library");
  }
