...
/**
 * Binary trees that have their left side full but their bottom right side
 * incomplete. e.g., imagine a history tree (Crosby & Wallach) that grows by
 * having leaves added from left to right. As more leaves are added the tree
 * grows in height so as to "encompass" the new leaves. However, the right side
 * of the tree could be "incomplete" (i.e., the number of leaves is not a power
 * of 2)
 *
...
class AlmostCompleteBinaryTree {
 ...
 public:
  /**
   * @param maxLeaves    the max # of leaves that will ever be inserted in the
   * tree
   */
 ...
 public:
  void aggregate() {
    ...
    // We walk the tree level by level from the bottom (leaves) to the top
    // (root) aggregating the sigs and VKs.
    int rootIdx = getRoot();
    while (node != rootIdx) {
      ...
      for (int i = 0; i < levelSize; i += 2) {
        ...
        // If we've reached the first left node that is "not aggregated" on this
        // level that means we're done and we can move up to the next level.
  ...
  const T& getNode(int node) const {
    ...
    if (node < 0 || node > maxNumNodes - 1) {
      logerror << "Accessed node " << node << " is not in [0, " << maxNumNodes
               << ") range" << std::endl;
  ...
  bool isAppendedLeaf(int node) const {
    return node >= firstLeafIndex && node < firstLeafIndex + numLeaves;
  }
   ...
   * case the root will no longer be at index 0, it'll be somewhere on the
   * leftmost path, so we need to compute its index.
   */
  int getRoot() const {
    assertLessThanOrEqual(numLeaves, maxLeaves);
...
/**
 * From "Finding Invalid Signatures in Pairing-Based Batches" by "Law, Laurie;
 * Matt, Brian"
 *
...
class BlsBatchVerifier {
 protected:
  class Share {
   public:
    ...
    Share(ShareID id, const G1T& sig, const G2T& vk)
        : isAggregated(true), id(id), sig(sig), vk(vk) {}
   ...
   public:
    bool operator!=(const Share& s) {
      return isAggregated != s.isAggregated || id != s.id || sig != s.sig ||
             vk != s.vk;
    }
  };
   ...
   public:
    void operator()(const Share& left, const Share& right, Share& parent) {
      if (left.isAggregated) {
        ...
        // If no aggregated left child, there should be no aggregated right
        // child either.
 ...
 public:
  /**
   * @param	ver 	the BlsThresholdVerifier object needed for verifying
   * shares
   */
 ...
 public:
   ...
   * @param   wantBadShares   if true, store the bad shares' IDs in 'shares', if
   * false, stores the good shares' IDs
   * @param   shares      good share or bad share IDs, depending on
   * 'wantBadShares'
   * @param   checkRoot   if we already know the threshold sig failed to
