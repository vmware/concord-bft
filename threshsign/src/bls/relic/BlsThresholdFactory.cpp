...
std::unique_ptr<BlsThresholdKeygenBase> BlsThresholdFactory::newKeygen(
...
IThresholdSigner* BlsThresholdFactory::newSigner(
    ShareID id, const char* secretKeyStr) const {
  return new BlsThresholdSigner(params, id, BNT(std::string(secretKeyStr)));
}

std::tuple<std::vector<IThresholdSigner*>, IThresholdVerifier*>
BlsThresholdFactory::newRandomSigners(NumSharesType reqSigners,
                                      NumSharesType numSigners) const {
  // Need to generate secret keys for the signers
  std::unique_ptr<BlsThresholdKeygenBase> keygen(
      newKeygen(reqSigners, numSigners));

  // Create signers
  std::vector<IThresholdSigner*> sks(
      static_cast<size_t>(numSigners + 1));  // 1-based indices
  ...
  for (ShareID i = 1; i <= numSigners; i++) {
    ...
    verifKeys.push_back(
        dynamic_cast<const BlsPublicKey&>(sks[idx]->getShareVerificationKey()));
  }
