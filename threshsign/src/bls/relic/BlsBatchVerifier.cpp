...
BlsBatchVerifier::BlsBatchVerifier(const BlsThresholdVerifier& ver,
                                   int maxShares)
    : ver(ver),
...
void BlsBatchVerifier::addShare(ShareID id, const G1T& sigShare) {
  const G2T& vk =
      checked_ref_cast<const BlsPublicKey&>(ver.getShareVerificationKey(id))
          .getPoint();

  // MAYDO: We could start aggregating incrementally after a leaf is appended
  // (powers of two + finish it off in batchVerify()). Make sure aggregate()
  // still works incrementally.
...
bool BlsBatchVerifier::batchVerify(const G1T& msg,
  ...
  // thread-safe. Right now is not, cause we push_back().
  assertEqual(shares.size(), 0);
  assertLessThanOrEqual(aggTree.getNumLeaves(), ver.getNumTotalShares());

  // TODO: use trick of not checking right root if left root is valid but their
  // parent is invalid
  this->msg = msg;
  ...
  if (isAggregated == false) {
    ...
    // logwarn << "You seem to be verifying the same batch of shares twice.
    // Hopefully you are testing?" << std::endl;
  }
...
bool BlsBatchVerifier::batchVerifyRecursive(int node,
  ...
  if (verified) {
    logtrace << "Successfully verified node " << node << std::endl;

    // Get all leaves below this valid node and push_back() the share IDs in
    // them
    ...
    logtrace << "Failed verifying node " << node << std::endl;

    if (isLeaf) {
      logtrace << "Found bad share " << share.id << std::endl;
