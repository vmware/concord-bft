...
LagrangeIncrementalCoeffs::LagrangeIncrementalCoeffs(
    NumSharesType numSigners, const BlsPublicParameters& params)
    : fieldOrder(params.getGroupOrder()),
      denoms(static_cast<size_t>(numSigners + 1),
             AccumulatedBNT(fieldOrder.getModulus())),
  ...
  assertEqual(denoms.size(),
              static_cast<std::vector<BLS::Relic::AccumulatedBNT>::size_type>(
                  numSigners + 1));
  assertEqual(denoms.size(), denomSigns.size());
}

/**
 * Functions to compute all n Lagrange coefficients incrementally as signers
 * show up.
 */
bool LagrangeIncrementalCoeffs::addSigner(ShareID newSigner) {
  ...
  if (signers.contains(newSigner)) {
    logwarn << "Tried to add signer #" << newSigner << " more than once"
            << endl;
    return false;
  }
  ...
  // TODO: ALIN: time intensive after a certain |signers| size, might want to
  // split up in separate threads

  // Update denominators for all past signers and add a new denominator for the
  // new signer
  for (ShareID i = signers.first(); signers.isEnd(i) == false;
       i = signers.next(i)) {
...
/**
 * WARNING: This will be slower as it involves division which requires inverting
 * mod p.
 */
void LagrangeIncrementalCoeffs::removeSigner(ShareID badSigner) {
  ...
  // NOTE: PERF: This will lead to a modular reduction every time as the inverse
  // is large.
  ...
  for (ShareID i = signers.first(); signers.isEnd(i) == false;
       i = signers.next(i)) {
    // remove bad signer j from denom(l_i^S) by dividing it with (i - j), where
    // i != j
    size_t ii = static_cast<size_t>(i);
    // NOTE: PERF: This will lead to a modular reduction every time as the
    // inverse is large.
    ...
    if (diff > 0) {
      ...
      denoms[ii].Times(
          pi.get(-diff));  // l_i^S /= (i - j) via marked sign below
      denomSigns[ii] = !denomSigns[ii];
    }
  }
...
void LagrangeIncrementalCoeffs::finalizeCoefficient(ShareID id, BNT& coeff) {
  ...
  assertProperty(finalSign, finalSign == -1 || finalSign == 1)
      assertStrictlyLessThan(numer, fieldOrder.getModulus());

  // Compute Lagrange coefficient as numerator * (denominator)^(-1) mod
  // fieldOrder
  if (finalSign == -1) {
    // We have -coeff, so turn it into a valid field element via (fieldOrder -
    // coeff)
 ...
 *
 * TODO: LATER: Here we are wasting time: to remove signer j from signer i's
 * coefficient, we can multiply the coefficient directly by (i-j) mod p and by
 * (-j)^{-1} mod p, rather than dividing in removeSigner and then inverting the
 * denominator in finalize. We should change the class so that coeffsOut is a
 * member that we update directly in removeSigner() and addSigner(). Thus,
 * finalize() only needs to be called once and we can restrict removeSigner() to
 * only be called after finalize() perhaps (though we have to update the tests
 * then?). addSigner(j) will multiply each coeff by -j (except for coeff j) and
 * divide each coeff i != j by (i-j). Could be a little slower now, unless we
 * use another layer of AccumulatedBNTs.
 */
...
void lagrangeCoeffAccumReduced(const VectorOfShares& signers,
   ...
   * for each i later on.
   */
  for (ShareID j = signers.first(); signers.isEnd(j) == false;
       j = signers.next(j)) {
  ...
  for (ShareID i = signers.first(); signers.isEnd(i) == false;
    ...
    numer = numerFinal;
    // Need to check if it's divisible by i, because numerFull might have been
    // reduced
    if (numer % i == BNT::Zero()) {
    ...
    // Keep track of the denominator's sign (already have numerator sign from
    // above)
    ...
    assertProperty(finalSign, finalSign == -1 || finalSign == 1)
        assertStrictlyLessThan(numer, fieldOrder);
  ...
  for (ShareID i = signers.first(); signers.isEnd(i) == false;
    ...
    for (ShareID j = signers.first(); signers.isEnd(j) == false;
      ...
      if (i > j) {
        ...
        denom.Times(static_cast<dig_t>(
            j - i));  // i.e., -(i-j) and we multiply by -1 below
        modulus.Reduce(denom);
        denomSign = !denomSign;
      }
    }
...
void lagrangeCoeffNaive(const VectorOfShares& signers,
  ...
  // l_i^S(0) = \prod_{j\in S; j \ne i} {(0-j)/(i-j)} (mod ord(G))
#ifndef NDEBUG
  int maxNumeratorBits = 0, maxDenomBits = 0, avgNumeratorBits = 0,
      avgDenomBits = 0;
#endif

  for (ShareID i = signers.first(); signers.isEnd(i) == false;
       i = signers.next(i)) {
    // logtrace << "Computing l_" << i << "(0) relative to other signers..." <<
    // endl;
    ...
    for (ShareID j = signers.first(); signers.isEnd(j) == false;
      ...
      if (i > j) {
        ...
        lagrDenom.Times(static_cast<dig_t>(
            j - i));  // i.e., -(i-j) and we multiply by -1 below
        lagrDenom.Negate();
      }
    }

    // Multiply numerator by (-1)^{signers.size() - 1} (i.e. negate it if number
    // of signers is even)
    if (signers.count() % 2 == 0) {
      lagrNum.Negate();
      // logtrace << "|S| - 1 is odd, multiplying numerator by (-1): " <<
      // lagrNum << endl;
    }
    ...
    // logtrace << "Denominator bits for l_" << i << "(0): " <<
    // bn_bits(lagrDenom) << endl; logtrace << "Numerator bits for l_" << i <<
    // "(0): " << bn_bits(lagrNum) << endl;
#endif
    // Reduce numerator, also takes care of negative numberator.
    lagrNum.SlowModulo(fieldOrder);

    // WARNING: This does NOT work with negative denom when |denom| > fieldOrder
    // so we bring it back to a positive number via a modular reduction.
...
#ifndef NDEBUG
  // The numerator/denom get pretty big! For n = 128, k = 128 (max and avg are
  // around to 700 bits)
  logtrace << "Average numerator bits: "
           << avgNumeratorBits / static_cast<int>(signers.count()) << endl;
  logtrace << "Average denominator bits: "
           << avgDenomBits / static_cast<int>(signers.count()) << endl;
