...
static void fromStringHelper(T& num, const std::string& str) {
  if (str.size() % 2) {
    throw std::runtime_error(
        "Cannot deserialize G1 elliptic curve point from odd-sized hexadecimal "
        "string");
  }
...
void BNT::toBytes(unsigned char* buf, int capacity) const {
  ...
  if (capacity < size) {
    logerror << "Expected buffer of size " << size
             << " bytes for serializing BNT object. You provided " << capacity
             << " bytes" << std::endl;
...
std::string BNT::toString(int base) const {
  ...
  // bn_write_str(reinterpret_cast<unsigned char*>(buf.getBuf()), size, n,
  // base);
...
BNT& BNT::FastModulo(const BNT& m, const BNT& u) {
  /**
   * NOTE: BNT::SlowModulo always uses RELIC's bn_mod_basic, no matter what
   * RELIC compilation flags are used.
   *
   * Montgomery modular reduction functions: FastModulo much slower than
   * SlowModulo bn_mod_monty bn_mod_monty_back bn_mod_monty_basic
   * bn_mod_monty_comba
   * bn_mod_monty_conv
   * bn_mod_pre_monty
   *
   * Barrett modular reduction function: slower than Montgomery, but SlowModulo
   * almost as fast as FastModulo bn_mod_barrt bn_mod_pre_barrt
   *
   *
   * PMERS modular reduction: much much slower than Barrett (20x), FastModulo is
   * insanely slower than SlowModulo bn_mod_pmers bn_mod_pre_pmers
   */
...
// WARNING: Microbenchmarking this must be done on a single thread, so be
// careful!
...
BNT BNT::invertModPrime(const dig_t& a, const BNT& p) {
...
#ifdef INVERT_MICROBENCH
  {
    ...
    // NOTE: This is faster than bn_gcd_ext_lehme (but only works on small dig_t
    // numbers)
...
BNT BNT::invertModPrime(const BNT& p) const {
  // n * inv + [?] * p == 1 <=> n * inv (mod p) + [?] * p (mod p) == 1 (mod p)
  // <=> n * inv (mod p) == 1 (mod p)

  assertTrue(bn_is_prime(p));
  // WARNING: 'bn_gcd_ext_basic' does not seem to work when a < 0, |a| > m
  // (e.g., a = -9, m = 7) but works when |a| < m
  assertFalse(*this < 0 && -(*this) > p);

  // Picked fastest GCD (implemented in various ways in RELIC: basic, Lehmer and
  // Stein)
  BNT gcd, inv;
