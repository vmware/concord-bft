...
void BlsMultisigAccumulator::getFullSignedData(char* outThreshSig,
  ...
  return sigToBytes(reinterpret_cast<unsigned char*>(outThreshSig),
                    threshSigLen);
}

void BlsMultisigAccumulator::sigToBytes(unsigned char* outThreshSig,
                                        int threshSigLen) const {
  ...
  if (reqSigners != totalSigners) {
    ...
    validSharesBits.toBytes(
        reinterpret_cast<unsigned char*>(outThreshSig + sigSize), vectorSize);
  }
}
...
void BlsMultisigAccumulator::aggregateShares() {
  ...
  for (ShareID id = validSharesBits.first(); validSharesBits.isEnd(id) == false;
       id = validSharesBits.next(id)) {
