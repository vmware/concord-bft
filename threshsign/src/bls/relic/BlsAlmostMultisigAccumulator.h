...
class BlsAlmostMultisigAccumulator : public BlsThresholdAccumulator {
 public:
  BlsAlmostMultisigAccumulator(const std::vector<BlsPublicKey>& vks,
                               NumSharesType numSigners);

 protected:
  // The Lagrange coefficients for sigshare i, indexed by the missing signer j
  // (manual indexing is used: i * num_cols + j)
