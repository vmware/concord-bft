...
// TODO: Things like this don't work: std::copy(verifKeys.begin(),
// verifKeys.end(), std::ostream_iterator(loginfo, "\n"));
 ...
 **************/
#define LOG_PREFIX                                                         \
  timeToString() << " " << std::setw(5) << getpid() << " " << std::setw(5) \
                 << getCurrentThreadId() << " " << std::setw(5)

#define LOG_SUFFIX                                                        \
  " " << std::setw(20) << __FUNCTION__ << ":" << std::setw(4) << __LINE__ \
      << " | "
