...
#define XASSERT_START                                                          \
  XASSERT_OSTREAM << "Assertion failed in function '" << __FUNCTION__ << "', " \
                  << __FILE__ << ":" << __LINE__ << ": "
...
/**
 * Here we define all our assert macros, after which we define macros that call
 * them!
 */
...
// Useful when you're checking a complex assertion and you want to see the value
// that failed that assertion. e.g., assertProperty(finalSign, finalSign == -1
// || finalSign == 1)
...
#define XASSERT_False(e)                                               \
  {                                                                    \
    if ((e) != false) {                                                \
      XASSERT_START << "'" << #e << "' is NOT false (i.e., it's true)" \
                    << std::endl;                                      \
      XASSERT_EXIT;                                                    \
    }                                                                  \
  }
...
/**
 * WARNING: You need the (void)arg around every argument 'arg.' On some
 * compilers, not having it will trigger an "error: right operand of comma
...
#define assertInclusiveRange(start, middle, end) \
  XASSERT_NO_EVAL(start) XASSERT_NO_EVAL(middle) XASSERT_NO_EVAL(end)
#define assertStrictlyPositive(x) XASSERT_NO_EVAL(x)
#define assertStrictlyGreaterThan(bigger, smaller) \
  XASSERT_NO_EVAL(bigger) XASSERT_NO_EVAL(smaller)
#define assertStrictlyLessThan(smaller, bigger) \
  XASSERT_NO_EVAL(smaller) XASSERT_NO_EVAL(bigger)
#define assertGreaterThanOrEqual(bigger, smaller) \
  XASSERT_NO_EVAL(bigger) XASSERT_NO_EVAL(smaller)
#define assertLessThanOrEqual(smaller, bigger) \
  XASSERT_NO_EVAL(smaller) XASSERT_NO_EVAL(bigger)
#define assertEqual(first, second) \
  XASSERT_NO_EVAL(first) XASSERT_NO_EVAL(second)
...
#define assertProperty(value, property) \
  XASSERT_NO_EVAL(value) XASSERT_NO_EVAL(property)
...
#define testAssertInclusiveRange(start, middle, end) \
  XASSERT_InclusiveRange(start, middle, end)
#define testAssertStrictlyPositive(x) XASSERT_StrictlyPositive(x)
#define testAssertStrictlyGreaterThan(bigger, smaller) \
  XASSERT_StrictlyGreaterThan(bigger, smaller)
#define testAssertStrictlyLessThan(smaller, bigger) \
  XASSERT_StrictlyLessThan(smaller, bigger)
#define testAssertGreaterThanOrEqual(bigger, smaller) \
  XASSERT_GreaterThanOrEqual(bigger, smaller)
#define testAssertLessThanOrEqual(smaller, bigger) \
  XASSERT_LessThanOrEqual(smaller, bigger)
