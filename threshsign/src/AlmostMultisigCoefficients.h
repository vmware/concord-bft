...
/**
 * NOTE: There should be a way to speed this up even further (only useful for
 * speeding up benchmarks).
 */
...
class AlmostMultisigCoefficientsMap {
 ...
 public:
  /**
   * When storing this object in a std::map, we need its default constructor
   * defined.
   */
  AlmostMultisigCoefficientsMap() : n(0) {}
  AlmostMultisigCoefficientsMap(NumSharesType n)
      : n(n), coeffs(static_cast<size_t>((n + 1) * (n + 1))) {}
 ...
 public:
  void precompute() {
    ...
    // Then, for each participant j remove j and compute the coefficients for
    // each participant i w.r.t. to the set of signers without j
    ...
    logdbg << "Done precomputing Lagrange coefficients for " << n - 1
           << " out of " << n << std::endl;
  }

  virtual void calculateCoefficientsForAll(const VectorOfShares& vec,
                                           ShareID missing) = 0;

  /**
   * NOTE: i and missing are always >= 1
   *
   * Recall that matrix[i][j] = vector[i * row_size + j], where row_size =
   * num_cols and i and j are 0-based indices.
   */
  ...
  /**
   * Returns the coefficient L_i^S(0) for player i w.r.t. to the set S which
   * excludes player 'missing'
   */
  const T& getCoeff(ShareID i, ShareID missing) const {
    // logtrace << "Getting precomputed L_" << i << "^S-{" << missing << "} (n =
    // " << n << ")..." << endl;
...
class AlmostMultisigCoefficients {
 protected:
  /**
   * coeffsMap[n] are the Lagrange coefficients for an n-1 out of n threshold
   * scheme coeffsMap[n].get(i, j) is the Lagrange coefficient L_i(0) for player
   * i's share in the n-1 out of n threshold scheme when player j is the missing
   * one
   */
