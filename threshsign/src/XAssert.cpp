...
bool coredump() {
  XASSERT_OSTREAM
      << "Segfaulting so that you can get a coredump and check the backtrace..."
      << std::endl;
#ifndef _WIN32
  __builtin_trap();
#else
...
/**
 * Makes sure the compiler doesn't evaluate expressions passed in to assertX()
 * calls when NDEBUG is defined. If it does, the program will "segfault"
 * instantly to indicate something's wrong.
 */
class XAssertInitializer {
 ...
 private:
  XAssertInitializer() {
    // If we're in "no debug" mode, make sure assertTrue(expr) doesn't actually
    // evaluate 'expr'!
#ifdef NDEBUG
    ...
    XASSERT_OSTREAM << "Compiler successfully avoids evaluating expressions in "
                       "'assertCall(expr())' calls"
                    << std::endl;
#endif
  }

  bool shouldNotBeCalled() {
    XASSERT_OSTREAM << "Oops, your compiler is not optimizing out expressions "
                       "passed into XAssert calls."
                    << std::endl;
    return coredump();
  }
};
