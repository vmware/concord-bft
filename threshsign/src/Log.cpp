...
std::ostream& coredumpOstream() {
  logerror << "Oops, looks like there's a logic error in your "
              "logdbg/logtrace/log[.*] #define's"
           << std::endl;
  XAssert::coredump();
  return std::cout;
}
...
/**
 * Makes sure the compiler doesn't evaluate expressions passed in to logX <<
 * expr() calls when logging is disabled. If it does, the program will
 * "segfault" instantly to indicate something's wrong.
 */
class LogInitializer {
 ...
 private:
  LogInitializer() {
    // If we're in "no logging" mode, make sure we never evaluate 'expr' we
    // logdbg!
#ifdef NDEBUG
    logdbg << shouldNotBeCalled();
#endif

    // If we're in "no trace" mode, make sure we never evaluate 'expr' we
    // logtrace!
#ifndef TRACE
  ...
  bool shouldNotBeCalled() {
    fprintf(stderr,
            "Oops, your compiler should not be evaluating expressions passed "
            "into optimized out 'log << expr' calls.\n");
