...
bool VectorOfShares::isEnd(ShareID e) const {
  ...
  // Shares are numbered from 1 to MAX_NUM_OF_SHARES and the data bitvector
  // always has size MAX_NUM_OF_SHARES Thus, when e > MAX_NUM_OF_SHARES, we've
  // reached the end. WARNING: Due to C++'s "features", it's important to cast
  // the unsigned type returned by data.size() to a signed type T here!
...
void VectorOfShares::toBytes(unsigned char* buf, int capacity) const {
  ...
  if (byteCount > capacity) {
    throw std::runtime_error(
        "Need more buffer space to serialize VectorOfShares");
  }
  ...
  for (ShareID id1 = first(); isEnd(id1) == false; id1 = next(id1)) {
    ...
    *(buf + byteOffset) =
        static_cast<unsigned char>(*(buf + byteOffset) | mask);
  }
}
...
void VectorOfShares::fromBytes(const unsigned char* buf, int len) {
  ...
  int byteCount = getByteCount();
  if (byteCount > len) {
    logwarn << "Deserializing VectorOfShares of smaller size than "
               "MAX_NUM_OF_SHARES IDs (i.e., "
            << byteCount << " bytes)" << endl;
  }
  ...
  for (int b = 0; b < len; b++) {
    ...
    for (int c = 0; c < 8; c++) {
      if (byte & bitMask) {
        int id1 = b * 8 + (c + 1);
        // logdbg << "bitMask = " << bitMask << ", deserialized ID " << id1 << "
        // from byte " << (int)byte << endl;
        add(id1);
      }
