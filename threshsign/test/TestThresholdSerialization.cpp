...
int RelicAppMain(const Library &lib, const std::vector<std::string> &args) {
  ...
  ShareID id = 0x208419;
  const char secretKeyValue[] = "rsa_private_key: "
  ...
  BlsThresholdSigner origSigner(params, id, secretKey);

  char* outBuf = nullptr;
  int64_t outBufSize = 0;
  origSigner.serialize(outBuf, outBufSize);

  auto* resultSigner =
      (BlsThresholdSigner *)BlsThresholdSigner::deserialize(outBuf, outBufSize);

  bool result = (resultSigner && *resultSigner == origSigner);
