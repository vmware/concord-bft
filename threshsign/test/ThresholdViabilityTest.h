...
class ThresholdViabilityTest {
 ...
 public:
  ...
  virtual GroupType hashMessage(const unsigned char* msg,
                                int msgSize) const = 0;
 ...
 public:
  ...
  std::unique_ptr<IThresholdAccumulator> createAccumulator(
      bool withShareVerification) {
    return std::unique_ptr<IThresholdAccumulator>(
        verif->newAccumulator(withShareVerification));
  }
  ...
  void test(const unsigned char* msg, int msgSize) {
    ...
    loginfo << "Testing " << params.getName() << " on " << reqSigners
            << " out of " << numSigners << " signers" << endl;
    GroupType h = hashMessage(msg, msgSize);

    VectorOfShares signers;
    VectorOfShares::randomSubset(signers, numSigners, reqSigners);

    // Deletes the old accumulator pointer, creates one without share
    // verification
    shareAccum = createAccumulator(verifiesShares);
    testAssertNotNull(accumulator());
    // If we call this before adding the shares, the shares will be verified at
    // addNumById() time
    ...
    for (auto p = points.begin(); p != points.end(); p++) {
      // We test what happens when we call setExpectedDigest() after shares have
      // been added.
      shareAccum = createAccumulator(verifiesShares);
      testAssertNotNull(accumulator());
      ...
      for (ShareID i = signers.first(); signers.isEnd(i) == false;
        ...
        if (i == *p) {
          logtrace << "Calling setExpectedDigest() after adding share " << i
                   << endl;
          shareAccum->setExpectedDigest(msg, msgSize);
        }
      }
