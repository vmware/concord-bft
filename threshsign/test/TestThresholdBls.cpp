...
int RelicAppMain(const Library& lib, const std::vector<std::string>& args) {
  ...
  BLS::Relic::BlsPublicParameters params(
      BLS::Relic::PublicParametersFactory::getWhatever());

  const char* msg = "blaBlaBlaBlaBlaBlaBlaBlaBlaBlaBlaBlaBlaBlaBlaBlaBla";
  for (bool multisig : {true, false}) {
    loginfo << endl;
    loginfo << "Testing threshold signatures (useMultisig = " << multisig << ")"
            << endl;
    loginfo << endl;
    ...
    for (auto it = nk.begin(); it != nk.end(); it++) {
      ...
      t.generateKeys();
      t.test(reinterpret_cast<const unsigned char*>(msg),
