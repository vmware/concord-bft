...
void printTime(const AveragingTimer& t, int numCoeffs, int numIters) {
    std::cout << t.getName() << " (" << numIters << " iters avg): "
                        << t.averageLapTime()/numCoeffs
                        << " microsecs per coefficient"
                        << " / "
                        << t.averageLapTime() << " microsecs in total"
                        << endl;
}
...
int RelicAppMain(const Library& lib, const std::vector<std::string>& args) {
    ...
    if(maxNumShares > MAX_NUM_OF_SHARES) {
        logerror << "MAX_NUM_OF_SHARES = " << MAX_NUM_OF_SHARES << " is smaller need max of " << maxNumShares << endl;
    ...
    for(int n = 128; n <= maxNumShares; n *= 2) {
        ...
        loginfo << "Testing k = " << k << " out of n = " << n
                << " (field order is " << params.getGroupOrder().getBits() << " bits)"<< endl;
