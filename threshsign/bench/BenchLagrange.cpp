...
void printTime(const AveragingTimer& t, int numCoeffs, int numIters) {
  std::cout << t.getName() << " (" << numIters
            << " iters avg): " << t.averageLapTime() / numCoeffs
...
int RelicAppMain(const Library& lib, const std::vector<std::string>& args) {
  ...
  if (maxNumShares > MAX_NUM_OF_SHARES) {
    logerror << "MAX_NUM_OF_SHARES = " << MAX_NUM_OF_SHARES
             << " is smaller need max of " << maxNumShares << endl;
  ...
  for (int n = 128; n <= maxNumShares; n *= 2) {
    ...
    loginfo << "Testing k = " << k << " out of n = " << n << " (field order is "
            << params.getGroupOrder().getBits() << " bits)" << endl;
    ...
    // RELIC does not have sufficient bignum precision; cannot compute
    // coefficients without reducing for n > 512
    if (n < 512) {
    ...
    for (int i = 0; i < numIters; i++) {
      ...
      for (ShareID s = subset.first(); subset.isEnd(s) == false;
           s = subset.next(s)) {
        lagr.addSigner(s);
      }
    ...
    std::cout << "      \\-> Time used to finalize " << k
              << " coeffs: " << tf.averageLapTime() << " microsecs" << endl;
    ...
    for (ShareID i = subset.first(); subset.isEnd(i) == false;
         i = subset.next(i)) {
      size_t idx = static_cast<size_t>(i);
      if (n < 512) {
        if (redCoeffs[idx] != rnCoeffs[idx]) {
          logerror << "ReducedAccumCoeff[" << idx << "] != ReducedNaiveCoeff ["
                   << idx << "]" << endl;
          logerror << redCoeffs[idx] << " != " << rnCoeffs[idx] << endl;
          throw std::runtime_error("Bad coeff");
        }

        if (redCoeffs[idx] != incrCoeffs[idx]) {
          logerror << "ReducedAccumCoeff[" << idx << "] != IncrAccumCoeff ["
                   << idx << "]" << endl;
          logerror << redCoeffs[idx] << " != " << incrCoeffs[idx] << endl;
          throw std::runtime_error("Bad coeff");
        }

        if (redCoeffs[idx] != naiveCoeffs[idx]) {
          logerror << "ReducedAccumCoeff[" << idx << "] != NaiveCoeffs [" << idx
                   << "]" << endl;
          logerror << rnCoeffs[idx] << " != " << naiveCoeffs[idx] << endl;
          throw std::runtime_error("Bad coeff");
        }
      }
    }
  ...
  logperf << "Separate numbers per-coefficient for incremental Lagrange: "
          << endl;
  int n = 256;
  incrCoeffs.resize(static_cast<size_t>(n + 1), BNT(0));
  LagrangeIncrementalCoeffs lagr(n, params);
