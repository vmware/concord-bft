...
class IThresholdSchemeBenchmark {
 protected:
  ...
  AveragingTimer hashT,  // hashing the message to be signed
      sigT,  // a single signing op under the non-threshold scheme (e.g., normal
             // BLS) w/o hashing overhead
      verT,  // a single verification op under the non-threshold scheme (e.g.,
             // normal BLS) w/o hashing overhead
      ...
      aggT;  // aggregating the signature shares, once the coefficients are
             // computed
 ...
 public:
   ...
   * each player gets their share i as the evaluation p(i) of the polynomial
   * p(.) at point i. Since the shared secret key is stored in p(0) no player
   * can have identity 0. (or if they do, then identities need to be mapped to
   * points x_i such that player i's share becomes p(x_i) rather than p(i))
   */
