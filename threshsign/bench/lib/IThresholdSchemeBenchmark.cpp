...
void IThresholdSchemeBenchmark::start() {
  ...
  for (int i = 0; i < numBenchIters; i++) {
    logdbg << "Benchmarking hashing m = " << Utils::bin2hex(msg, msgSize)
           << ", (" << msgSize << " bytes)" << endl;
    ...
    logdbg << "Benchmarking share signing (" << reqSigners << " out of "
           << numSigners << ")..." << endl;
    ...
    // This begins the accumulation of the shares (creates an accumulator for
    // example and adds the shares to it)
    accumulateShares(signers);

    if (hasShareVerify) {
      logdbg << "Benchmarking share verification (" << reqSigners << " out of "
             << numSigners << ")..." << endl;
      ...
      logdbg << "(Skipping over share verification: hasShareVerify is set to "
                "false)"
             << endl;
    }
    ...
    // Compute the lagrange coefficients L_i(0) for each signer i in signers
    // set.
...
void IThresholdSchemeBenchmark::printHeaders(std::ostream& out) {
  out << "library,"
      ...
      << "sigshare_size,"  // might be identical to sig_size; for RSA, this
                           // includes proof \pi
      << "sigshare_time,"  // might be identical to sig_time
      << "lagrange_coeff_time,"  // (time to compute l_i(0) for all i)
      << "lagrange_exp_time,"    // (time to exponentiate the sig shares with
                                 // their l_i(0))
      << "aggregate_time,"  // (given all coeffs and sigshares, time to compute
                            // final sig)
      << "total_share+lagr+agg_time,"  // hash_to_group_time + sigshare_time +
                                       // lagrange_coeff_time +
                                       // lagrange_exp_time + aggregate_time +
                                       // final_aggr_step
...
void IThresholdSchemeBenchmark::printNumbers(std::ostream& out) {
  ...
  auto totalShareAggVerTime =
      sigShareTime + lagrCoeffTime + lagrExpTime + aggTime;

  out << totalShareAggVerTime << ",";

  if (hasShareVerify) {
    // NOTE: Each for loop iteration in start() measures the time to verify all
    // shares.
