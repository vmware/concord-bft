...
class ThresholdBlsRelicBenchmark : public IThresholdSchemeBenchmark {
 private:
  ...
  // to store them in an std::vector: neither std::vector<bn_t> nor
  // std::vector<bn_t*> work.
  ...
  std::vector<BlsPublicKey>
      pks;  // public keys (corresp. to SK shares) for all signers
 ...
 public:
  ThresholdBlsRelicBenchmark(const BlsPublicParameters& p,
    ...
    // WARNING: Players are numbered from 1 to n (pos [0] is irrelevant in these
    // arrays)
    ...
    generateKeys();
    loginfo << "Created new threshold BLS benchmark: numIters = "
            << numBenchIters << ", k = " << k << ", n = " << n
 ...
 protected:
  void generateKeys() {
    const BlsPublicParameters& blsParams =
        dynamic_cast<const BLS::Relic::BlsPublicParameters&>(params);
    ...
    logdbg << "PK size (uncompressed): " << g2_size_bin(pk, 0) << " bytes"
           << endl;
    ...
    // For BLS, sigshare size is just |sig| + ceil(\log_2{numSigners}), but we
    // should probably not count that since we can use the IP address in most
    // application as the identifier.
    sigShareBits = dynamic_cast<BlsThresholdSigner*>(sks[1])
 ...
 public:
  ...
  virtual void verifySingle() {
    ...
    if (gt_cmp(e1, e2) != CMP_EQ) {
      throw std::logic_error(
          "Your single signing or verification code is wrong.");
    }
  }
  ...
  virtual void exponentiateLagrangeCoeff(const VectorOfShares& signers) {
    ...
    dynamic_cast<BlsThresholdAccumulator*>(accum.get())
  ...
  void sanityCheckThresholdSignature(const VectorOfShares& signers) {
    ...
    (void)signers;
    logdbg << "Threshold signature: "
           << Utils::bin2hex(reinterpret_cast<unsigned char*>(threshSig),
                             sigBits)
           << endl;
...
int RelicAppMain(const Library& lib, const std::vector<std::string>& args) {
  ...
  std::vector<std::pair<int, int>> nk =
      Benchmark::getThresholdTestCases(501, false, true, false);
  ...
  for (bool useMultisig : {true, false}) {
    ...
    loginfo << endl;
    loginfo << "Benchmarking "
            << (useMultisig ? "multisig-based" : "Lagrange-based")
            << " BLS threshold signatures" << endl;
    loginfo << endl;
