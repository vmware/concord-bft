...
void printTime(const AveragingTimer& t, bool printAvg = false) {
  logperf << t.numIterations() << " iters of " << t.getName() << ": "
          << t.totalLapTime() << " microsecs" << std::endl;
  if (printAvg)
...
void benchmarkModulo(const BNT& fieldOrder,
  ...
  for (int i = 0; i < numIters; i++) {
    ...
    // We want a to be bigger than p, so that modular reduction does *some*
    // work!
    if (a < fieldOrder || c < fieldOrder) {
      throw std::logic_error("This is surprising.");
    }

    // We want these equal to make sure both SlowModulo and FastModulo return
    // the same result
    if (c != a)
      throw std::logic_error(
          "This benchmark needs c == a. Why did you change it?");
    ...
    if (preFunc != nullptr) {
      ...
      if (c != a)
        throw std::logic_error(
            "FastModulo and SlowModulo returned different results. BNT / RELIC "
            "/ microbench bug?");
    } else {
      t.startLap();
...
void benchModulo(const BNT& fieldOrder, int bigNumIters) {
  ...
  // NOTE: BNT::FastModulo by default uses Monty so we don't need to benchmark
  // it separately
...
int RelicAppMain(const Library& lib, const std::vector<std::string>& args) {
  ...
  //    AveragingTimer tDiv("BNT::DivideBy"); // Don't use this (except rarely
  //    in old Lagrange coefficient implementation)
...
void benchChaPedVerify(const BNT& fieldOrder, int numIters) {
  ...
  loginfo << "Precomputing on message hash took: " << time << " microsecs"
          << endl;
...
void benchChaPedSign(const BNT& fieldOrder, int numIters) {
  const char* message =
      "asdbqu2i12j3891hsdnpnvdsdqd123i12j-u9wenfsdpasd-123j12-3";
