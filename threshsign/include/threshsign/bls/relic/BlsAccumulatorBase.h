...
class BlsAccumulatorBase
    : public ThresholdAccumulatorBase<BlsPublicKey, G1T, BlsSigshareParser> {
 protected:
 ...
 public:
  /**
   * Returns true if shares are verified once the expected message is set via
   * setExpectedDigest() or false otherwise (i.e., when shares are
 ...
 public:
  /**
   * Automagically called when a new signature share is added to the accumulator
   * via add(). In this case does nothing because we defer aggregation to
   * subclasses.
   *
   * @param   id          the signer ID
   * @param   sigShare    a signature share from that signer ID on the expected
   * message.
   */
 ...
 protected:
  /**
   * Verifies a signature share from the specified signer ID on the expected
   * message. Assumes the expected message has been already set in 'hash' via
   * setExpectedDigest() and onExpectedDigestSet(). Automagically called by
   * parent ThresholdAccumulatorBase class when hasShareVerificationEnabled()
   * returns true
   *
   * @param   id          the signer ID
   * @param   sigShare    a signature share from that signer ID on the expected
   * message.
   *
   * @return true if the share verified correctly against the signer's
   * verification key, false otherwise
   */
 ...
 public:
  /**
   * NOTE: Used to fetch the signature as a group element when benchmarking
   * threshold BLS. Assumes caller already called computeLagrangeCoeff(),
   * exponentiateLagrangeCoeff() and aggregateShares(). Otherwise, threshSig
   * will not be computed (i.e., will be G1T::Identity()).
   *
   ...
   * Like getThresholdSignature() assumes the threshold signature has been
   * assembled.
   *
