...
protected:
    ...
    /**
     * When set to true, uses a multisig-based scheme for implementing threshold signatures. Otherwise, uses the secret-sharing-based scheme
     * The multisig-based scheme is much faster when assembling the threshold signature but produces larger signatures that are a bit slower to verify.
     */
    bool useMultisig;
...
public:
     ...
     * @param verifKeysStr the verification keys of individual signers against which their signature shares can be verified
     *
     * @return an IThresholdVerifier object that can be used to verify signature shares, assemble the threshold signature and verify it as well
     */
    virtual IThresholdVerifier * newVerifier(NumSharesType reqSigners, NumSharesType numSigners, const char * publicKeyStr,
