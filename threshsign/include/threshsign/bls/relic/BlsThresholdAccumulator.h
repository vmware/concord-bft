...
class BlsThresholdAccumulator : public BlsAccumulatorBase {
 ...
 public:
  ...
  virtual IThresholdAccumulator* clone() {
    return new BlsThresholdAccumulator(*this);
  }
 ...
 public:
  /**
   * NOTE: Must be virtual because BlsAlmostMultisigAccumulator overrides this
   * to fetch precomputed Lagrange coeffs.
   */
  ...
  /**
   * NOTE: We are now using fast multi exponentiation which computes \sigma =
   * \prod_i \sigma_i ^ \ell_i directly and faster than computing \sigma_i ^
   * \ell_i individually. (Here \sigma denotes the threshold signature, \ell_i
   * is the ith Lagrange coeff and \sigma_i is the ith signature share.) As a
   * result, this function does nothing.
   */
  virtual void aggregateShares() {
    // for(ShareID id = validSharesBits.first(); validSharesBits.isEnd(id) ==
    // false; id = validSharesBits.next(id)) {
