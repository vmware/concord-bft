...
class BNT {
 ...
 public:
  ...
  BNT& Times(const BNT& b) {
    // NOTE: Checked RELIC library and it's safe (and fast) to use same output
    // as input.
    bn_mul(n, n, b);
    return *this;
  }
  BNT& Times(const dig_t d) {
    ...
    // NOTE: Checked in RELIC library, and using same output as input is okay!
  ...
  BNT& SlowModulo(const BNT& m) {
    // TODO: PERF: For MONTGOMERY will need to call more things to convert input
    // to Montgomery form NOTE: Checked RELIC library and it's safe to use same
    // output as input.
    bn_mod_basic(n, n, m);
    return *this;
  }
 ...
 public:
  // Implicitly cast BNT objects to bn_t types so that we can keep the same
  // syntax when calling RELIC functions
  ...
  /**
   * WARNING: Looked at RELIC and it seems that bn_add_imp expects different
   * output than input. We might as well use this inefficient but convenient
   * operator implementation then.
   */
  ...
  /**
   * We just use these for convenience when we have to. We don't want to
   * sacrifice performance by copying bn_t objects around too often!
   */
 public:
  BNT operator*(const BNT& rhs) const {
    BNT mult;
 ...
 * (Note the similarity between all of these. Couldn't figure out a nice way to
 * work around code duplication.)
 */
class G1T {
 public:
  g1_t n;
 ...
 public:
  G1T() {
    g1_null(n);
    g1_new(n);
    // WARNING: G1T::Identity() assumes the default constructor sets this to
    // infinity!
    g1_set_infty(n);
  }
 ...
 public:
  // Implicitly cast G1T objects to g1_t types so that we can keep the same
  // syntax when calling RELIC functions
...
class G2T {
 ...
 public:
  G2T() {
    g2_null(n);
    g2_new(n);
    // WARNING: G2T::Identity() assumes the default constructor sets this to
    // infinity!
    g2_set_infty(n);
  }

  G2T(const G2T& c) : G2T() {
    // WARNING: RELIC asks for non-const, even though it does not modify the
    // args. Thus, we remove the const using a cast.
 ...
 public:
  // Implicitly cast G2T objects to g2_t types so that we can keep the same
  // syntax when calling RELIC functions
...
class GTT {
 ...
 public:
  // Implicitly cast GTT objects to gt_t types so that we can keep the same
  // syntax when calling RELIC functions
