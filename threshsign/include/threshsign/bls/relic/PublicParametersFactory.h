...
class PublicParametersFactory {
 public:
  static BlsPublicParameters getByCurveType(const char* curveName) {
    ...
    if (params.getCurveType() != curveType) {
      throw std::runtime_error(
          "RELIC library is not built for the specified curve");
    }
  ...
  static BlsPublicParameters getWhatever() {
    return BlsPublicParameters(Library::Get().getSecurityLevel(),
                               Library::Get().getCurrentCurve());
  }
  ...
  static BlsPublicParameters getBitSecurityK(int k) {
    // NOTE: RELIC library is compiled with support for one curve at a certain
    // security level, so here we check for that level.
    if (Library::Get().getSecurityLevel() != k) {
      std::ostringstream str;
      str << "The RELIC library you are linked against has a different "
             "security level than "
          << k << " bits: it has " << Library::Get().getSecurityLevel()
          << "-bit security.";
