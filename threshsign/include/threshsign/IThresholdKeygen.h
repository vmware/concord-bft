...
class IThresholdKeygen {
 ...
 public:
  ...
  const SK& getShareSecretKey(ShareID i) const {
    return skShares[static_cast<size_t>(i)];
  }
  const PK& getShareVerificationKey(ShareID i) const {
    return pkShares[static_cast<size_t>(i)];
  }
