...
/**
 * NOTE: This class is NOT the fastest implementation. We just use it when we
 * generate keys.
 */
...
class SecretSharingPolynomial {
 ...
 public:
  // TODO: If you want to eliminate this method (can't right now because C++
  // does not let you call virtual methods in the constructor) then add a
  // functor template parameter for randomCoeff instead of a pure virtual method
  ...
  Integer get(ShareID x) const {
    ...
    // Computes f(i) as f(i)_{old} * x + a_k in reverse order k = { degree + 1,
    // degree, ..., 2, 1, 0 }
