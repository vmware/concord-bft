...
/**
 * NOTE: Can't take msg and length as constructor arguments because, in SBFT,
...
class IThresholdAccumulator {
 ...
 public:
  /**
   * Adds a signature share to the accumulator with or without verifying it,
   * depending on the implementation and on whether setExpectedDigest() has been
   * called beforehand.
   *
   * @param sigShareWithId  the sigShare buffer; contains the ID of the signer.
   * @param len             the length of the sigShare buffer
   *
   * @return  the size of the accumulator (will have increased by one if the
   * added sigShare was validated successfully)
   */
  virtual int add(const char* sigShareWithId, int len) = 0;

  /**
   * Sets the message being threshold-signed. Should only be called with the
   * same message or will throw. This message will be used to verify signature
   * shares against!
   *
   * @param msg
   * @param len
   ...
   * Before that, always returns 0, since shares can't be verified without a
   * digest.
   */
  virtual int getNumValidShares() const = 0;

  /**
   * Computes and returns the final threshold signature on the digest specified
   * in setExpectedDigest(). WARNING: If called twice, might recompute the
   * signature which will be slow.
   *
