...
/**
 * TODO: Threshold accumulators store too much state that could be kept in the
 * parent IThresholdVerifier: reqSigners, totalSigners, vks
 *
 * TODO: Add ParentThresholdVerifier typename to template and take it as arg in
 * constructor. Then access vks, reqSigners and totalSigners from parent
 * verifier.
 */
...
class ThresholdAccumulatorBase : public IThresholdAccumulator {
 protected:
  ...
  // Reference to the verification keys stored in the corresponding
  // IThresholdVerifier object
  const std::vector<VerificationKey>& vks;

  // Pending unverified shares (because setExpectedDigest() has not been called
  // yet)
  ...
  // Bit vectors of pending (unverified) shares and valid shares
  // (|validSharesBits| <= reqSigners)
 ...
 public:
  ...
  /**
   * Adds the share either to the pending shares or valid shares and returns
   * their updated count (depending on hasExpectedDigest()). (We make this
   * public because we call it in our tests/benchmarks)
   */
  virtual int addNumById(ShareID signer, const NumType& sigShare);

 protected:
  /**
   * Called to verify a share when added by addNumById() (and
   * hasExpectedDigest() is true). Or, called by verifyPendingShares() to move
   * pending shares into valid shares when setExpectedDigest() is called.
   *
  ...
  /**
   * Called after the share is verified succesfully if share verification is
   * enabled, or called after the unverified share is added otherwise.
   *
   * For example, multisig accumulators will implement this to algebraically
   * accumulate the newly added share.
   */
  virtual void onNewSigShareAdded(ShareID id, const NumType& sigShare) = 0;

  /**
   * 	Called after setExpectedDigest() is called with a valid digest, but
   *before verifyPendingShares() is called.
   *
   *	For example, the BLS accumulator will want to start precomputing
   *pairings e(H(m), g^{vk_i}), \forall signers i to speed up share
   *verification!
   */
 ...
 protected:
  /**
   * Verifies all the pendingShares and moves the valid ones into validShares
   * for accumulation later. After it returns, the 'signers' bit vector will be
   * for the validShares, not pendingShares.
   */
