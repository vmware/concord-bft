...
public:
    ThresholdAccumulatorBase(const std::vector<VerificationKey>& vks, NumSharesType reqSigners, NumSharesType totalSigners)
        : expectedDigest(nullptr), expectedDigestLen(0),
		  reqSigners(reqSigners), totalSigners(totalSigners), vks(vks),
		  pendingShares(static_cast<size_t>(totalSigners+1)), validShares(static_cast<size_t>(totalSigners+1))
    {}
...
public:
     ...
     * (We make this public because we call it in our tests/benchmarks)
     */
...
protected:
    ...
    /**
     * Called after the share is verified succesfully if share verification is enabled, or called after the unverified share is added otherwise.
     *
