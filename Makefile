...
CONCORD_BFT_DOCKERFILE?=utt-demo/Dockerfile.buildenv
...
CONCORD_BFT_UTT_DEMO_CMF_PATHS?=${CONCORD_BFT_TARGET_SOURCE_PATH}/build/tests/uttDemo/cmf
...
# UDP | TLS | TCP
CONCORD_BFT_CMAKE_TRANSPORT?=UDP
...
CONCORD_BFT_CMAKE_USE_S3_OBJECT_STORE?=FALSE
...
tidy-check: gen_cmake ## Run clang-tidy
	docker run ${CONCORD_BFT_USER_GROUP} ${BASIC_RUN_PARAMS} \
		...
		make -C ${CONCORD_BFT_UTT_DEMO_CMF_PATHS} &> /dev/null && \
