...
class IQueue {
 public:
  ...
  // all concurrently with that queue's destructor after that destructor has begun executing, or at any point after
  // the destructor has begun running (including after the destructor has completed). Furthermore, behavior is
  // undefined if any thread is still waiting on the blocking IQueue::Pop call when the destructor begins running.
  // Code owning IQueue instances should guarantee that there are no outstanding calls to the IQueue's functions and
  // that no thread will start new ones before destroying an instance. Note the ReleaseConsumers function can be used
  // to have the queue unblock and release any threads still waiting on IQueue::Pop calls.
  virtual ~IQueue() {}
  ...
  // become available. This function may block the caller to obtain locks if that is necessary for this operation
  // under the IQueue's implementation.
  ...
  // the queue, this function should block the calling thread and wait until an update that can be popped is
  // available. This function may also block the calling thread for purposes of obtaining any lock(s) needed for these
  // operations. This function gives ownership of an allocated Update to the caller; implementations of IQueue may
  // choose whether they give ownership of an allocated Update they own to the caller or whether they dynamically
  // allocate memory (via malloc/new/std::allocator(s)) to construct the Update object from data the Queue has stored
  // by some other means. If ReleaseConsumers is called, any currently waiting Pop calls will be unblocked, and will
  // return a unique_ptr to null rather than continuing to wait for new updates. Furthermore, a call to
  // ReleaseConsumers will cause any subsequent calls to Pop to return nullptr and will prevent them from blocking
  // their caller.
  virtual std::unique_ptr<T> pop() = 0;

  // Synchronously pop an update from the front of the queue if one is available, block the calling thread and wait
  // till till the timeout milli seconds. Returns a unique_ptr to nullptr if no update is found within the timeout,
  // and a unique_ptr giving ownership of an allocated Update otherwise (this may involve dynamic memory allocation at
  // the discretion of the IQueue implementation).
  virtual std::unique_ptr<T> popTill(std::chrono::milliseconds timeout) = 0;

  // Synchronously pop an update from the front of the queue if one is available, but do not block the calling thread
  // to wait on one if one is not immediately found. Returns a unique_ptr to nullptr if no update is immediately
  // found, and a unique_ptr giving ownership of an allocated Update otherwise (this may involve dynamic memory
  // allocation at the discretion of the IQueue implementation).
