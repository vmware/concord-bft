...
void ConcordClient::subscribe(const SubscribeRequest& request,
                              std::shared_ptr<EventUpdateQueue>& queue,
                              const std::unique_ptr<opentracing::Span>& parent_span) {
  impl_->subscribe(request, queue, parent_span);
}
...
void ConcordClient::getSnapshot(const StateSnapshotRequest& request, std::shared_ptr<SnapshotQueue>& remote_queue) {
  impl_->getSnapshot(request, remote_queue);
}

std::string ConcordClient::getSubscriptionId() const { return impl_->getSubscriptionId(); }
