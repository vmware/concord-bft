...
class Client2 {
 public:
  ...
  void set_socket_options(SocketOptions socket_options) { cli_->set_socket_options(socket_options); }
...
inline bool keep_alive(socket_t sock) {
  ...
  while (true) {
    ...
    if (val < 0) {
      ...
      auto timeout = CPPHTTPLIB_KEEPALIVE_TIMEOUT_SECOND * 1000 + CPPHTTPLIB_KEEPALIVE_TIMEOUT_USECOND / 1000;
...
inline std::pair<std::string, std::string> make_digest_authentication_header(
  ...
  auto field = "Digest username=\"" + username + "\", realm=\"" + auth.at("realm") + "\", nonce=\"" + auth.at("nonce") +
               "\", uri=\"" + req.path + "\", algorithm=" + algo + ", qop=" + qop + ", nc=\"" + nc + "\", cnonce=\"" +
...
inline void Server::set_socket_options(SocketOptions socket_options) { socket_options_ = socket_options; }
...
inline socket_t Server::create_server_socket(const char *host,
  ...
  return detail::create_socket(
      host, port, socket_flags, tcp_nodelay_, socket_options, [](socket_t sock, struct addrinfo &ai) -> bool {
        if (::bind(sock, ai.ai_addr, static_cast<socklen_t>(ai.ai_addrlen))) {
          return false;
        }
...
inline std::shared_ptr<Response> Client::send_with_content_provider(const char *method,
                                                                    const char *path,
                                                                    const Headers &headers,
                                                                    const std::string &body,
                                                                    size_t content_length,
...
inline bool Client::process_socket(Socket &socket, std::function<bool(Stream &strm)> callback) {
...
inline void Client::set_socket_options(SocketOptions socket_options) { socket_options_ = socket_options; }
...
inline bool SSLServer::process_and_close_socket(socket_t sock) {
  auto ssl = detail::ssl_new(sock, ctx_, ctx_mutex_, SSL_accept, [](SSL * /*ssl*/) { return true; });
      ...
      if (parse_www_authenticate(res2, auth, true)) {
        ...
        if (!detail::process_client_socket(socket.sock,
                                           ...
                                           [&](Stream &strm) {
                                             ...
                                             req3.headers.insert(
                                                 make_digest_authentication_header(req3,
                                                                                   auth,
                                                                                   1,
                                                                                   random_string(10),
