...
static bool validateFundamentalFields(const
                                        std::vector<bftEngine::ReplicaConfig>&
                                        configs) {
...
static bool validateConfigStructIntegrity(const
                                            std::vector<
                                            bftEngine::ReplicaConfig>&
                                            configs) {
  uint16_t numReplicas = configs.size();
...
static bool testThresholdCryptosystem(const std::string& name,
                                      const std::vector<IThresholdSigner*>&
                                        signers,
                                      const std::vector<IThresholdVerifier*>&
                                        verifiers,
                                      uint16_t numSigners,
                                      uint16_t threshold) {
...
int main(int argc, char** argv) {
  std::string usageMessage = "Usage:\n"
    ...
    " GenerateConcordKeys\nutility; TestGeneratedKeys expects that"
    " GenerateConcordKeys has already been run\nand produced keyfiles for it to"
    " test. TOTAL_NUMBER_OF_REPLICAS should specify\nhow many replicas there"
    " are in the deployment that TestGeneratedKeys has\ngenerated keys for, and"
    " KEYFILE_PREFIX should be the prefix passed to\nGenerateConcordKeys via"
    " its -o option that all. TestGeneratedKeys expects to\nfind"
    ...
    " inclusive.\n\nTestGeneratedKeys can be run with no options or with the"
    " --help option to view this usage text.\n";
