    "eb3b1a893c5111853bed1cc60fe074a55aa51a975cc3217594ff1479036a05d01c01b2f610"
    "b4"
    "e4adbc629e06dc636b65872072cdf084ee5a7e0a63afe42931025d4e5ed60d069cfa71d91b"
    "c7"
    ...
    for (auto entry : config.publicKeysOfReplicas) {
      ...
      if (id >= numReplicas) {
        std::cout << "TestGeneratedKeys: FAILURE: Entry with invalid replica"
                     " ID ("
                  << id << ") in set of public keys of replicas for replica "
                  << i << ".\n";
        return false;
      }
...
// cryptosystem.
static std::vector<std::vector<uint16_t>> getThresholdSignerCombinationsToTest(
...
static bool testThresholdSignature(const std::string& cryptosystemName,
  ...
  for (auto hash : kHashesToTest) {
    ...
    if (verificationEnabled) {
      try {
        accumulator->setExpectedDigest(
            reinterpret_cast<const unsigned char*>(hash.c_str()), hashLength);
      } catch (std::exception e) {
        std::cout << invalidPublicConfig;
        releaseAccumulator(verifier, accumulator);
        return false;
    ...
    for (auto signerID : signersToTest) {
      ...
      std::string invalidPrivateKey =
          ...
          std::to_string(signerID) + " under the " + cryptosystemName +
          " threshold cryptosystem.\n";
      int sigShareLen;
      try {
      ...
      if (verificationEnabled) {
        ...
        if (((accumulatorSize + 1) <= threshold) &&
          ...
          std::cout << "TestGeneratedKeys: FAILURE: Signature accumulator"
                    ...
                    << signerID << "'s signature for " << cryptosystemName
                    << " threshold cryptosystem.\n";
          releaseAccumulator(verifier, accumulator);
          return false;
        } else {
    ...
    try {
      signatureAccepted =
          verifier->verify(hash.c_str(), hashLength, sigBuf, signatureLength);
    } catch (std::exception e) {
      std::cout << invalidKeyset;
      releaseAccumulator(verifier, accumulator);
      delete[] sigBuf;
    ...
    if (signatureAccepted != (participatingSigners >= threshold)) {
      std::cout << "TestGeneratedKeys: FAILURE: Threshold signer with "
                ...
                << (signatureAccepted ? "accepted" : "rejected")
                << " under the " << cryptosystemName
                << " threshold cryptosystem, which has threshold " << threshold
                << " out of " << numSigners << ".\n";
      return false;
    }
  }
  ...
  for (auto signerCombination : signerCombinationsToTest) {
    ...
    if ((testsCompleted % kTestProgressReportingInterval) == 0) {
      std::cout << "Tested " << testsCompleted << " out of " << totalTests
                << " signer combinations to test on the " << name
                << " threshold"
                   " cryptosystem.\n";
    }
  }
  ...
  for (uint16_t i = 0; i < numSigners; ++i) {
    ...
    if (privKeys.count(key) > 0) {
      ...
      std::cout << "TestGeneratedKeys: FAILURE: Replica " << i
                ...
                << existingKeyHolder << " under the " << name
                << " threshold cryptosystem.\n";
      return false;
    }
  }
...
int main(int argc, char** argv) {
  std::string usageMessage =
      ...
      "GenerateConcordKeys has already been run and produced keyfiles for it "
      "to\n"
      "test. TOTAL_NUMBER_OF_REPLICAS should specify how many replicas there\n"
      "are in the deployment that TestGeneratedKeys has generated keys for, "
      "and\n"
