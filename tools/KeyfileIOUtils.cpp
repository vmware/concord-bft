...
bool outputReplicaKeyfile(uint16_t replicaID,
  ...
  serializeCryptosystemPublicConfiguration(output, commitSys, "Commit",
    "commit");
  serializeCryptosystemPublicConfiguration(output, optSys,
    "Optimistic fast path commit", "optimistic_commit");
...
static bool parseUint16(uint16_t& output,
  ...
  if (map.count(identifier) < 1) {
    if (lineNumbers.count(identifier) < 1) {
      ...
      std::cout << filename << ": line " << lineNumbers.at(identifier)
        << ": expected integer value for " << identifier << ", found list.\n";
    }
    return false;
  }
  ...
  std::string errorMessage = filename + ": line "
    + std::to_string(lineNumbers.at(identifier)) + ": Invalid value given for "
    + identifier + ": " + strVal + " ( expected integer in range ["
    ...
    if (map.count(entry) < 1) {
      if (lineNumbers.count(entry) < 1) {
        ...
        std::cout << filename << ": line " << lineNumbers.at(entry)
          << ": expected string value for " << entry << ", found list.\n";
      }
      return false;
    }
  }
...
static bool deserializeCryptosystemPublicConfiguration(std::unique_ptr<
                                                         Cryptosystem>&
                                                         cryptosystem,
  ...
  if (!Cryptosystem::isValidCryptosystemSelection(type, subtype, numSigners,
    ...
    std::cout << filename << ": line " << identifierLines.at(typeVar) << ":"
      " Invalid configuration for " << name << " cryptosystem (type " << type
      << ", subtype " << subtype << ", threshold of " << threshold << " out of "
      << numSigners << ").\n";
    return false;
  }
  ...
  if (listAssignments.count(verifKeyVar) < 1) {
    if (identifierLines.count(verifKeyVar) < 1) {
      ...
      std::cout << filename << ": line " << identifierLines.at(verifKeyVar)
        << ": expected list for " << verifKeyVar << ", found single value.\n";
    }
    return false;
  }
...
bool parseReplicaKeyfile(std::istream& input,
                         ...
                         std::unordered_map<std::string,
                           std::vector<std::string>>& listAssignments,
  ...
  while (input.peek() != EOF) {
    ...
    if ((line.length() > 2) && (line[0] == '-') && (line[1] == ' ')) {
      ...
      if (value.find_first_of(":") != std::string::npos) {
        std::cout << filename << ": line " << lineNumber << ": Invalid list"
          " entry: contains \":\".\n";
        return false;
      }

      if (!currentList) {
        std::cout << filename << ": line " << lineNumber << ": Unexpected list"
    ...
    // IDENTIFIER:
      ...
      if (identifierLines.count(identifier) > 0) {
        std::cout << filename << ": line " << lineNumber << ": Attempting to"
          " make a new assignment to an identifier that is already in use"
          " (previous assignment on line " << identifierLines[identifier]
          << ").\n";
        return false;
      }
    ...
    // IDENTIFIER: VALUE
      ...
      std::string value = trim(line.substr(colonLoc + 1,
        line.length() - (colonLoc + 1)));
      ...
      if (identifier.length() < 1) {
        std::cout << filename << ": line " << lineNumber << ": Expected"
...
bool inputReplicaKeyfile(std::istream& input,
  ...
  for (size_t i = 0; i < numReplicas; ++i) {
    if (!validateRSAPublicKey(rsaPublicKeys[i])) {
      std::cout << filename << ": line "
        <<  listEntryLines["rsa_public_keys"][i]
  ...
  if (!deserializeCryptosystemPublicConfiguration(slowSys, "slow path commit",
        "slow_commit", valueAssignments, listAssignments, filename,
        identifierLines, listEntryLines, numReplicas)) {
    return false;
  }
  if (!deserializeCryptosystemPublicConfiguration(commitSys, "commit", "commit",
  ...
  if (!deserializeCryptosystemPublicConfiguration(optSys,
        "optimistic fast path commit", "optimistic_commit", valueAssignments,
  ...
  if (!expectEntries({"rsa_private_key", "execution_cryptosystem_private_key",
        ...
        "optimistic_commit_cryptosystem_private_key"}, valueAssignments,
        filename, identifierLines)) {
    return false;
  }
  ...
  for (uint16_t i = 0; i < numReplicas; ++i) {
    config.publicKeysOfReplicas.insert(std::pair<uint16_t, std::string>(i,
      rsaPublicKeys[i]));
  }
