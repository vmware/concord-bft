...
const std::uint32_t BLOCKS_PER_SECOND = 40;
...
concord::messages::PruneRequest ConstructPruneRequest(std::size_t client_idx,
                                                      ...
                                                      std::uint32_t blocks_per_second = BLOCKS_PER_SECOND) {
  ...
  prune_req.blocks_per_second = blocks_per_second;
  return prune_req;
}
...
TEST_F(test_rocksdb, sign_verify_correct) {
  ...
  // Sign and verify a PruneRequest message.
  {
    ...
    request.blocks_per_second = BLOCKS_PER_SECOND;
    for (auto i = 0; i < replica_count; ++i) {
