...
Replica::Replica(ICommunication *comm,
  ...
  bftEngine::bcst::Config stConfig = {
    ...
    replicaConfig_.get("concord.common.bft.st.pedanticChecks", false),
    replicaConfig_.isReadOnly,

#if defined USE_COMM_PLAIN_TCP || defined USE_COMM_TLS_TCP
    replicaConfig_.get<uint32_t>("concord.common.bft.st.maxChunkSize", 30 * 1024 * 1024),
    replicaConfig_.get<uint16_t>("concord.common.bft.st.maxNumberOfChunksInBatch", 256),
#else
    replicaConfig_.get<uint32_t>("concord.common.bft.st.maxChunkSize", 2048),
    replicaConfig_.get<uint16_t>("concord.common.bft.st.maxNumberOfChunksInBatch", 32),
#endif
    replicaConfig_.get<uint32_t>("concord.common.bft.st.maxBlockSize", 30 * 1024 * 1024),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.maxPendingDataFromSourceReplica", 256 * 1024 * 1024),
    replicaConfig_.getmaxNumOfReservedPages(),
    replicaConfig_.getsizeOfReservedPage(),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.gettingMissingBlocksSummaryWindowSize", 600),
    replicaConfig_.get<uint16_t>("concord.common.bft.st.minPrePrepareMsgsForPrimaryAwareness", 10),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.fetchRangeSize", 256),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.RVT_K", 1024),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.refreshTimerMs", 300),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.checkpointSummariesRetransmissionTimeoutMs", 2500),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.maxAcceptableMsgDelayMs", 10000),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.sourceReplicaReplacementTimeoutMs", 0),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.fetchRetransmissionTimeoutMs", 2000),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.maxFetchRetransmissions", 2),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.metricsDumpIntervalSec", 5),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.maxTimeSinceLastExecutionInMainWindowMs", 5000),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.sourceSessionExpiryDurationMs", 5000),
    replicaConfig_.get<uint32_t>("concord.common.bft.st.sourcePerformanceSnapshotFrequencySec", 300),
    replicaConfig_.get("concord.common.bft.st.runInSeparateThread", !replicaConfig_.isReadOnly),
    replicaConfig_.get("concord.common.bft.st.enableReservedPages", true),
    replicaConfig_.get("concord.common.bft.st.enableSourceBlocksPreFetch", true),
    replicaConfig_.get("concord.common.bft.st.enableSourceSelectorPrimaryAwareness", true),
    replicaConfig_.get("concord.common.bft.st.enableStoreRvbDataDuringCheckpointing", true)
  };
