...
uint64_t ReplicaStateSyncImp::execute(logging::Logger& logger, IDbAdapter& bcDBAdapter, uint64_t lastExecutedSeqNum) {
  ...
  BlockId lastReachableBlockId = bcDBAdapter.getLastReachableBlockId();
  ...
  while (lastReachableBlockId && genesisBlockId <= lastReachableBlockId) {
    ...
    lastReachableBlockId = bcDBAdapter.getLastReachableBlockId();
    ++removedBlocksNum;
  }
  LOG_INFO(logger,
           "Inconsistent blockchain block deleted "
               << KVLOG(removedBlocksNum, lastExecutedSeqNum, lastBlockSeqNum, bcDBAdapter.getLastReachableBlockId()));
  return removedBlocksNum;
}
