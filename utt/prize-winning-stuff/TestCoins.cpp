    ...
    for(size_t i = 0; i < w.size(); i++) {
        ...
        for(auto& txout : tx.outs) {
            ...
            for(auto& ow : w) {
                ...
                // i.e., calls TxOut::isMine() which returns std::optional<CoinSecrets> and, if yours, then calls Wallet::makeMine, which rerandomizes everything
            }
        }
    }
...
void test2to2Txn() {
    ...
    Tx tx = Tx::create(
        ...
        {
            std::make_tuple(apk1, out1),
            std::make_tuple(apk2, out2) 
        }
    );
    ...
    testAssertFalse(tx.outs[1].isMine(p, ask1).has_value())
    (void)walletCoin1;

    auto opt2 = tx.outs[1].isMine(p, ask2);
    testAssertTrue(opt2.has_value());
    testAssertFalse(tx.outs[0].isMine(p, ask2).has_value())
