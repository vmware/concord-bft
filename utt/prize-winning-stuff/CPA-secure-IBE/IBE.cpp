...
size_t IBE::Ctxt::getCtxtSize(size_t ptxtSize) { return plusaes::get_padded_encrypted_size(ptxtSize); }
...
IBE::Ctxt IBE::MPK::encrypt(const std::string& pid, const Fr& coinValue, const Fr& vcm_r, const Fr& icm_r) const {
  ...
  // second, encrypt
  plusaes::encrypt_cbc(
      plaintext, ptxt_size, enc_key.getBuf(), IBE::Ctxt::KEY_SIZE, &ctxt.iv, ctxt.buf.getBuf(), ctxt_size, true);
...
std::tuple<Fr, Fr, Fr> IBE::EncSK::decrypt(const IBE::Ctxt& ctxt) const {
  ...
  // NOTE(Alin): I'm a bit confused, but I think I need to pass padding_size=0 so that 'decrypt_cbc' doesn't return the
  // padding in the decrypted plaintext
  size_t padding_size = 0;
  plusaes::decrypt_cbc(ctxt.buf.getBuf(),
                       static_cast<size_t>(ctxt.buf.size()),
                       enc_key.getBuf(),
                       static_cast<size_t>(enc_key.size()),
                       &ctxt.iv,
                       ptxt,
                       ptxt_size,
                       &padding_size);
