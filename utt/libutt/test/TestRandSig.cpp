...
void testCentralized() {
    ...
    // NOTE: For now, we never need to serialize RandSigSK's (e.g., RegAuth is just PKs, because coins are pre-issued in our benchmarks; also, bank is distributed, so serializing RandSigShareSK's instead.)
    //ss << sk_tmp;
    //ss >> sk;
