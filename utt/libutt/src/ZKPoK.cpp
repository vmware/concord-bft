...
PedEqProof::PedEqProof(const CommKey& ck1,
                       const Comm& cm1,
                       const Fr& r_1,
                       const CommKey& ck2,
                       const Comm& cm2,
...
bool PedEqProof::verify(const CommKey& ck1, const Comm& cm1, const CommKey& ck2, const Comm& cm2) const {
  std::vector<G1> X(2, G1::zero());
...
Fr PedEqProof::hash(
    const CommKey& ck1, const Comm& cm1, const CommKey& ck2, const Comm& cm2, const std::vector<G1>& X) const {
  return hashToField(std::string("pedeq-same-msg-diff-rand") + "|" + ck1.toString() + "|" + cm1.toString() + "|" +
                     ck2.toString() + "|" + cm2.toString() + "|" + hashToHex(X) + "");
}

ZKPoK::ZKPoK(const CommKey& ck, const Comm& cm, const Fr& m, const Fr& t) {
...
Fr ZKPoK::hash(const CommKey& ck, const Comm& cm, const G1& R) const {
  return hashToField(std::string("zkpok-ck-size-2") + "|" + ck.toString() + "|" + cm.toString() + "|" + hashToHex(R) +
                     "");
}
