...
void __compute_numerators(std::vector<Fr>& N0, const std::vector<Fr>& allOmegas, const std::vector<size_t>& T, const Fr& num0) {
...
std::vector<Fr> __lagrange_coefficients_naive(const std::vector<Fr>& allOmegas, const std::vector<Fr>& someOmegas, const std::vector<size_t>& T) {
    std::vector<Fr> L;
    size_t N = allOmegas.size();
    assertTrue(Utils::isPowerOfTwo(N));
    assertEqual(someOmegas.size(), T.size());
    assertStrictlyLessThan(someOmegas.size(), N);   // assuming N-1 out of N is "max" threshold, which we don't have to assume (but it would be less efficient to do Lagrange interpolation if you have the evaluations at all N Nth roots of unity)
