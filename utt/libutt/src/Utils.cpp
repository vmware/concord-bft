...
void printMemUsage(const char* headerMessage) {
  size_t vms, rss;
  getMemUsage(vms, rss);
  if (headerMessage != nullptr) {
    loginfo << headerMessage << ": " << endl;
  }

  loginfo << "VMS: " << Utils::humanizeBytes(vms) << endl;
  loginfo << "RSS: " << Utils::humanizeBytes(rss) << endl;
}
...
void getMemUsage(size_t& vm_usage, size_t& resident_set) {
  ...
  vm_usage = vsize;
  resident_set = static_cast<size_t>(rss * page_size_bytes);
#else
  ...
  if (sizeof(pti) == proc_pidinfo(pid, PROC_PIDTASKINFO, 0, &pti, sizeof(pti))) {
    vm_usage = pti.pti_virtual_size;       // / 1024 / PAGE_SIZE_KB;
    resident_set = pti.pti_resident_size;  // / 1024 / PAGE_SIZE_KB;
  } else {
