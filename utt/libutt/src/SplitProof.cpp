...
SplitProof::SplitProof(const Params& p,
                       const Fr& pid_hash,
                       const Fr& s,
                       const Coin& coin,
                       const Fr& a,
...
bool SplitProof::verify(const Params& p,
  ...
  X[0] = Comm::create(p.getCoinCK(), {alpha[0], alpha[1], alpha[2], Fr::zero(), Fr::zero(), alpha[3]}, false).asG1() -
         (c * ccm.asG1());
...
Fr SplitProof::hash(const Params& p,
                    const Nullifier& null,
                    const Comm& rcm,
                    const Comm& ccm,
                    const Comm& vcm,
