...
size_t calcBudget(const Wallet& w) {
    return w.budgetCoin ? w.budgetCoin->getValue() : 0;
}
...
//////////////////////////////////////////////////////////////////////////////////////////////
std::optional<Coin> tryClaimCoin(const Context& ctx, Tx& tx, size_t txoIdx, const std::vector<RandSigShare>& sigShares, const std::vector<size_t>& signerIds, const AddrSK& ask) {
...
void signAndClaimOutputCoins(const Context& ctx, Tx& tx, std::vector<Wallet>& w) {
    ...
    for(size_t txoIdx = 0; txoIdx < tx.outs.size(); txoIdx++) {
        ...
        for(size_t j = 0; j < w.size(); j++) {
            ...
            if(coin.has_value()) {
                foundIdx = j;
                logdbg << "Adding a $" << coin->val << " '"<< coin->getType() << "' coin to wallet #" << j + 1 << " for '" << w[j].ask.pid << "'" << endl;
...
Wallet createWallet(const Context& ctx, const std::string& pid, const std::vector<size_t>& normalCoinVals, size_t budgetCoinVal) {
    ...
    for(const auto& val : normalCoinVals) {
        ...
        ss << c;
        ss >> currCoin;
        assertEqual(currCoin, c)
        assertNotEqual(prevCoin, c);
...
void doRandomPayment_2t2(const Context& ctx, Wallet& w1, Wallet& w2, std::set<std::string>& nullset, std::vector<Wallet>& wallets) {
...
void doRandomPayment_2t1(const Context& ctx, Wallet& w1, Wallet& w2, std::set<std::string>& nullset, std::vector<Wallet>& wallets){
    testAssertGreaterThanOrEqual(w1.coins.size(), 2);
    // ToDo: trivially random
    doPayment_2t1(ctx, w1, w2, 0, 1, nullset, wallets);
}

void doRandomPayment_1t2(const Context& ctx, Wallet& w1, Wallet& w2, std::set<std::string>& nullset, std::vector<Wallet>& wallets){
...
void doRandomPayment_1t1(const Context& ctx, Wallet& w1, Wallet& w2, std::set<std::string>& nullset, std::vector<Wallet>& wallets){
    // ToDo: trivially random
...
void doPayment_2t2(const Context& ctx, Wallet& w1, Wallet& w2, size_t c1, size_t c2, size_t amount, std::set<std::string>& nullset, std::vector<Wallet>& wallets) {
    ...
    w1.coins.erase(std::remove_if(w1.coins.begin(), w1.coins.end(), [](const Coin& c){
        return c.val == 0;
    }), w1.coins.end());
...
void doPayment_2t1(const Context& ctx, Wallet& w1, Wallet& w2, size_t c1, size_t c2, std::set<std::string>& nullset, std::vector<Wallet>& wallets){
...
void doPayment_1t2(const Context& ctx, Wallet& w1, Wallet& w2, size_t c1, size_t amount, std::set<std::string>& nullset, std::vector<Wallet>& wallets){
...
void doPayment_1t1(const Context& ctx, Wallet& w1, Wallet& w2, size_t c1, std::set<std::string>& nullset, std::vector<Wallet>& wallets){
...
void doCoinSplit(const Context& ctx, Wallet& w, size_t c1, size_t amount, std::set<std::string>& nullset, std::vector<Wallet>& wallets){
...
void doCoinMerge(const Context& ctx, Wallet& w, size_t c1, size_t c2, std::set<std::string>& nullset, std::vector<Wallet>& wallets){
    ...
    w.coins.at(c1).val = 0;
    w.coins.at(c2).val = 0;
    w.coins.erase(std::remove_if(w.coins.begin(), w.coins.end(), [](const Coin& c){
        return c.val == 0;
    }), w.coins.end());
