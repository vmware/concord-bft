...
RegistratorIdentity::RegistratorIdentity(const std::string& id, const std::string& rsk, const std::string& rbk, const RegAuthSK& tmp) {
  id_ = id;
...
std::vector<uint8_t> RegistratorIdentity::ComputeRCMSig(const std::vector<uint64_t>& pid_hash, const Commitment& rcm1) const {
    Fr fr_pid;
