...
class BudgetProof {
 ...
 public:
  size_t getSize() const {
    return
        // we only need this when measuring general k-to-m txns; for our purposes, we use just 3-to-3 and 2-to-2, so
        // need for this
        // sizeof(size_t) + (sizeof(size_t) * forMeTxos.size()) +
        sizeof(size_t) + alpha.size() * _fr_size + sizeof(size_t) + beta.size() * _fr_size + _fr_size;
  }

  BudgetProof() {}

  BudgetProof(const std::set<size_t>& forMeTxos,
              const CommKey& regCK,
              const Comm& rcm,
 ...
 public:
  ...
  bool verify(const CommKey& regCK,
              const Comm& rcm,
              const std::vector<CommKey>& ck,
              const std::vector<Comm>& icm) const {
 ...
 public:
  bool operator==(const BudgetProof& o) const {
    return forMeTxos == o.forMeTxos && alpha == o.alpha && beta == o.beta && e == o.e && true;
  }
};
