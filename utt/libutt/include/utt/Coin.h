...
namespace libutt {
    ...
    public:
        ...
        Fr exp_date;    // expiration date; TODO: Fow now, using 32/64-bit UNIX time, since we never check expiration in the current prototype
    ...
    public:
        ...
        Coin(const CommKey& ck, 
            ...
            const Fr& sn, 
            const Fr& val,
            const Fr& type, const Fr& exp_date,
            const AddrSK& ask);
    ...
    public:
        ...
        // TODO(libff): We use Fr(2) rather than Fr(0) here due to a bug in libff: https://github.com/scipr-lab/libff/issues/108
        ...
        // TODO(libff): We use Fr::one() rather than Fr::zero() here due to a bug in libff: https://github.com/scipr-lab/libff/issues/108
         ...
         */
        static Comm augmentComm(const CommKey& coinCK,
            const Comm& ccmTxn, 
            const Fr& type,
            const Fr& exp_date);
    ...
    protected:
        /**
         * Computes the partial coin commitment: g_1^pid g_2^sn g_3^val g^r (assumes r is pre-set) and the value commitment g_3^val g^z, by picking a random z
         */
        void commit();
