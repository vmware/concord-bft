 ...
 *
 * void ZZ_pX::SetMaxLength(long n) -> f.SetMaxLength(n) pre-allocate spaces for n coefficients. The polynomial that f
 * represents is unchanged.
 *
 * clear(ZZ_pX& x) -> x = 0
 * set(ZZ_pX& x) -> x = 1
 * random(ZZ_pX& x, long degBound) -> picks random poly of degree = degBound - 1 (documentation is confusing; it says <
 * degBound, which is not precise; but code generates exactly degBound - 1)
 *
 * rem(r, a, b) -> r = a%b
...
Group multiExp(const std::vector<Group>& bases, const vec_ZZ_p& exps) {
  std::vector<FieldT> expsff;
  convNtlToLibff(exps, expsff);

  long minSize = static_cast<long>(bases.size());
  if (exps.length() < minSize) minSize = exps.length();

  return multiExp<Group>(bases.cbegin(), bases.cbegin() + minSize, expsff.cbegin(), expsff.cbegin() + minSize);
}

template <class Group, class FieldT>
Group multiExp(const std::vector<Group>& bases, const ZZ_pX& exps) {
