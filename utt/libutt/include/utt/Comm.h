...
class CommKey {
 ...
 public:
  size_t getSize() const {
    return sizeof(size_t) + (_g1_size * g.size()) + sizeof(size_t) + (_g2_size * g_tilde.size());
  }

  CommKey() {}

  CommKey(const std::vector<G1>& g) : g(g) {}
 ...
 public:
  ...
  const G2& getGen2() const {
    testAssertFalse(g_tilde.empty());
    return g_tilde.back();
  }
  ...
  bool hasCorrectG2() const {
    ...
    if (g.size() != g_tilde.size()) return false;
 ...
 public:
  bool operator==(const CommKey& ck) const { return g == ck.g && g_tilde == ck.g_tilde; }

  bool operator!=(const CommKey& ck) const { return !operator==(ck); }
};
...
class Comm {
 ...
 public:
  bool operator==(const Comm& other) const {
    if (ped2.has_value() && other.ped2.has_value() && *ped2 != *other.ped2) return false;

    return ped1 == other.ped1;
  }

  bool operator!=(const Comm& other) const { return !operator==(other); }
