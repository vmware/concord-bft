...
namespace libutt {
    ...
    class Tx {
    public:
        bool isSplitOwnCoins;   // true when splitting your own coins; in this case, no budget coins are given as input, to save TXN creation & validation time
        ...
        std::optional<BudgetProof> budget_pi;   // proof that (1) output budget coin has same pid as input coins and (2) that this might be a "self-payment" TXN, so budget should be saved
    ...
    public:
        size_t getSize() const {
           return 
               sizeof(isSplitOwnCoins) + 
               rcm.getSize() +
               regsig.getSize() +
               ins.back().getSize() * ins.size() + sizeof(size_t) + 
               outs.back().getSize() * outs.size() + sizeof(size_t) +
               sizeof(bool) + (budget_pi.has_value() ? budget_pi->getSize(): 0);
        }
        ...
        bool verifySigShare(size_t txoIdx, const RandSigShare& sigShare, const RandSigSharePK& bpkShare) const {
            ...
            if(outs[txoIdx].H.has_value()) {
                // This will be cached here if the client created the TXN, sent it to the replicas, received back sigshares and called this method to verify them
                H = *outs[txoIdx].H;
            } else {
         ...
         * i.e., decrypt the denomination, identity commitment randomness and value commitment randomness from the coin's ciphertext.
         *
         ...
         *
         * WARNING: Assuming there is no need to check the coin signature since it comes from the signed TXN ledger, so it is known to be valid.
         */
        std::optional<Coin> tryClaimCoin(
            const Params& p,
            size_t txoIdx,
            const AddrSK& ask,
            size_t n,
