...
namespace libutt {
    ...
    std::string __groupElemsAsString(const std::vector<Group>& elems) {
        ...
        std::stringstream ss;
        if(elems.size() == 0)
            throw std::runtime_error("will not hash empty vector of group elements");
    ...
    template<class Group>
    void hashToBytes(const Group& elem, const std::string& prefix, const std::string& suffix, unsigned char * hash, size_t capacity) {
        std::stringstream ss;
        ...
        if (capacity < picosha2::k_digest_size)
            throw std::runtime_error("Need a larger hash buffer");
