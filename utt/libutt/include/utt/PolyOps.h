 ...
 * @param   n       # of evaluations, which implicitly determines the primitive Nth root of unity \omega (which must be
 * smallest N of the form 2^k >= n)
...
std::vector<FieldT> poly_inverse_fft(const std::vector<FieldT>& evals, size_t N) {
  ...
  std::transform(poly.begin(), poly.end(), poly.begin(), [&N_inv](const FieldT& f) -> FieldT { return f * N_inv; });
...
std::vector<FieldT> poly_scale(const Fr& c, const std::vector<FieldT>& f) {
  ...
  std::transform(s.begin(), s.end(), s.begin(), [&c](const FieldT& coeff) -> FieldT { return c * coeff; });
...
std::vector<FieldT> poly_negate(const std::vector<FieldT>& f) {
  ...
  std::transform(f_neg.begin(), f_neg.end(), f_neg.begin(), [](const FieldT& c) -> FieldT { return -c; });
...
/**
 * Computes Lagrange coefficients L_i^T(0) for the set of points (\omega_N^i)_{i\in T} in O(k^2) time, where k = |T| and
 * N = 2^k is the smallest power greater than n
 *
...
std::vector<FieldT> poly_from_roots_slow(const vector<FieldT>& roots, size_t startIncl, size_t endExcl) {
  ...
  libfqfft::_polynomial_multiplication_on_fft(
      p, poly_from_roots_slow(roots, startIncl, middle), poly_from_roots_slow(roots, middle, endExcl));
...
bool poly_is_xnc(const vector<FieldT>& p) {
  ...
  if (p[p.size() - 1] != FieldT::one()) return false;

  for (size_t j = 1; j < p.size() - 1; j++) {
    if (p[j] != FieldT::zero()) return false;
  }
 ...
 */
template <class FieldT>
std::tuple<std::vector<FieldT>, std::vector<FieldT>> poly_div_xnc(const vector<FieldT>& a,
