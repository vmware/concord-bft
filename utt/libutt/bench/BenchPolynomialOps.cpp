...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) startSz = static_cast<size_t>(std::stoi(argv[1]));
  ...
  if (argc > 3) {
    ...
    if (numThreads > static_cast<int>(getNumCores())) {
      logerror << "Number of cores for libntl (" << numThreads
               << ") cannot be bigger than # of cores on machine, which is " << getNumCores() << endl;
      return 1;
    }
  ...
  for (size_t i = startSz; i <= startSz; i *= 2) {
    ...
    for (int rep = 0; rep < count; rep++) {
      ...
      if (!noFFT) {
        ...
        if (res.size() != 0) {
          logerror << "The two multiplication functions returned different products" << endl;
          throw std::runtime_error("One of the multiplication implementations is wrong");
        }
      }
      p1.clear();
      p2.clear();
    }
    logperf << "a.size() = " << a.size() << ", b.size() = " << b.size() << ", iters = " << count << endl;
    if (c1.numIterations() == 0) {
      // logperf << " + FFT: skipped"  << endl;
    } else {
      logperf << " + FFT mult: " << (double)c1.averageLapTime() / 1000000 << " seconds." << endl;
    }
    if (c2.numIterations() == 0) {
      // logperf << " + Naive: skipped"  << endl;
    } else {
      logperf << " + Naive mult: " << (double)c2.averageLapTime() / 1000000 << " seconds." << endl;
    }
    logperf << " + NTL mult: " << (double)c4.averageLapTime() / 1000000 << " +/- " << c4.stddev() / 1000000
            << " seconds " << endl;
    logperf << " + NTL (with conv): " << (double)c3.averageLapTime() / 1000000 << " seconds." << endl;
    logperf << endl;
  }
  return 0;
}
