...
void benchAllRootsOfUnity(size_t n, size_t r) {
  ...
  for (size_t id = 0; id < n; id++) {
    ...
    logperf << tn << endl;
    logperf << "Time per 1 exp: " << static_cast<double>(tn.averageLapTime()) / static_cast<double>(n)
            << " microseconds" << endl;
  }
}
...
int main(int argc, char *argv[]) {
  ...
  logperf << tn << endl;
  logperf << "Time per 1 exp: " << static_cast<double>(tn.averageLapTime()) / static_cast<double>(n) << " microseconds"
          << endl;
