...
inline TimeDeltaMicro subtract(
    Time t1, Time t2)  // TODO(GG): replace with absDifference (not sure that this method is really needed)
{
  return (t1 - t2);
}

inline Time addMilliseconds(Time base, uint16_t milliseconds) { return (base + (milliseconds * 1000)); }
