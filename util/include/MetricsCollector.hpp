...
class MetricsCollector {
 public:
  ...
  void takeMetric(MetricType t) { countersCallBacks[t](); }

  void takeMetric(MetricType t, uint64_t val) { gaugesCallBacks[t](val); }

  void takeMetric(MetricType t, const std::string& val) { statusesCallBacks[t](val); }

  void registerCallBack(MetricType t, const CounterHandlerCallback& cb) { countersCallBacks[t] = cb; }
  void registerCallBack(MetricType t, const GaugeHandlerCallback& cb) { gaugesCallBacks[t] = cb; }
  void registerCallBack(MetricType t, const StatusHandlerCallback& cb) { statusesCallBacks[t] = cb; }
