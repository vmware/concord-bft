...
struct Recorders {
  ...
  Recorders(const std::string& selfId, int64_t max_msg_size, int64_t max_queue_size_in_bytes)
      : write_queue_size_in_bytes(
            MAKE_SHARED_RECORDER("write_queue_size_in_bytes", max_queue_size_in_bytes, Unit::BYTES)),
        sent_msg_size(MAKE_SHARED_RECORDER("sent_msg_size", max_msg_size, Unit::BYTES)),
        received_msg_size(MAKE_SHARED_RECORDER("received_msg_size", max_msg_size, Unit::BYTES)) {
  ...
  DEFINE_SHARED_RECORDER(write_queue_len, MAX_QUEUE_LENGTH, Unit::COUNT);
  DEFINE_SHARED_RECORDER(send_time_in_queue, MAX_US, Unit::MICROSECONDS);
  DEFINE_SHARED_RECORDER(read_enqueue_time, MAX_US, Unit::MICROSECONDS);
  DEFINE_SHARED_RECORDER(send_post_to_mgr, MAX_US, Unit::MICROSECONDS);
  DEFINE_SHARED_RECORDER(send_post_to_conn, MAX_US, Unit::MICROSECONDS);
  DEFINE_SHARED_RECORDER(async_write, MAX_US, Unit::MICROSECONDS);
  DEFINE_SHARED_RECORDER(async_read_header_full, MAX_US, Unit::MICROSECONDS);
  DEFINE_SHARED_RECORDER(async_read_header_partial, MAX_US, Unit::MICROSECONDS);
  DEFINE_SHARED_RECORDER(async_read_msg, MAX_US, Unit::MICROSECONDS);
  DEFINE_SHARED_RECORDER(on_connection_authenticated, MAX_US, Unit::MICROSECONDS);
};
